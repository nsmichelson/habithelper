import { SimplifiedTip } from '../types/simplifiedTip';
import { NUTRITION_SIMPLIFIED_TIPS } from './nutritionSimplifiedTips';
import { FITNESS_SIMPLIFIED_TIPS } from './fitnessSimplifiedTips';
import { PRODUCTIVITY_SIMPLIFIED_TIPS } from './productivitySimplifiedTips';
import { RELATIONSHIP_SIMPLIFIED_TIPS } from './relationshipsSimplifiedTips';
import { MINDSET_SIMPLIFIED_TIPS } from './mindsetSimplifiedTips';
import { ORGANIZATION_SIMPLIFIED_TIPS } from './organizationSimplifiedTips';
import { SLEEP_SIMPLIFIED_TIPS } from './sleepSimplifiedTips';

const BASE_SIMPLIFIED_TIPS: SimplifiedTip[] = [
  {
    "tip_id": "e897e203-f6a3-4726-97cf-0be4bf6dade3",
    "micro_version": "Put a few ginger chews in your purse or pockets right now.",
    "summary": "Keep ginger chews or candies on hand for nausea relief.",
    "details_md": "**The Experiment:** Carry ginger chews in your bag and keep a few by your bed. When nausea hits, slowly suck or chew one.\n\n**Why it Works:** Ginger compounds can calm the stomach and reduce nausea; having them pre-positioned makes relief immediate.\n\n**How to Try It:**\n• Stock a few in your purse, desk, and nightstand.\n• If strong ginger is too spicy, try milder ginger drops.\n• Start with small amounts and see what your stomach tolerates.",
    "area": "eating",
    "goals": [
      "reduce_nausea",
      "increase_hydration",
      "nutrient_density"
    ],
    "helps_with": [
      "nausea",
      "vomiting",
      "odor_triggers"
    ],
    "contraindications": [
      "Avoid if ginger worsens heartburn or if your clinician advised limiting ginger (e.g., on certain blood thinners). Large amounts of ginger may interact with blood thinners or increase reflux; limit if advised by your clinician."
    ],
    "mechanisms": [
      "symptom_management",
      "behavioral_strategy",
      "convenience",
      "relief"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "morning",
      "any",
      "feeling_nauseous",
      "waking_up"
    ],
    "where": [
      "any"
    ],
    "involves": [
      "ginger_candy"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "Name your three 'nausea emergency stations' where you'll stash ginger chews:",
    "personalization_type": "multi_text",
    "personalization_config": {
      "items": [
        {
          "label": "Station 1:",
          "placeholder": "e.g., Bedside table drawer"
        },
        {
          "label": "Station 2:",
          "placeholder": "e.g., Car glove compartment"
        },
        {
          "label": "Station 3:",
          "placeholder": "e.g., Work desk drawer"
        }
      ]
    },
    "difficulty": 1,
    "source": "dietitian_reviewed"
  },
  {
    "tip_id": "70bc2cae-2895-4041-9ba3-9cdc8c38ac23",
    "micro_version": "Boil water and steep 2 bags of ginger tea to cool.",
    "summary": "Freeze strong ginger tea into cubes for instant nausea tea.",
    "details_md": "**The Experiment:** Brew a pot of strong ginger tea, freeze in an ice tray, and pop a cube into hot water when queasy.\n\n**Why it Works:** Batch-prepped ginger removes friction on rough days so you can self-soothe fast.\n\n**How to Try It:**\n• Steep sliced fresh ginger or tea bags 10–15 minutes.\n• Freeze in an ice cube tray and store in a zip bag.\n• Use 1–2 cubes in hot water or suck a cube if cold helps.",
    "area": "eating",
    "goals": [
      "reduce_nausea",
      "increase_hydration",
      "nutrient_density"
    ],
    "helps_with": [
      "nausea",
      "vomiting"
    ],
    "contraindications": [
      "Skip if ginger triggers reflux; mind dental sensitivity if sucking frozen cubes. Large amounts of ginger may interact with blood thinners or increase reflux; limit if advised by your clinician. Cool foods quickly, store safely, and reheat leftovers to 165°F; eat within 3–4 days once thawed."
    ],
    "mechanisms": [
      "meal_prep",
      "symptom_management",
      "convenience",
      "relief"
    ],
    "effort": "minimal",
    "time": "15-30min",
    "cost": "$",
    "when": [
      "morning",
      "any",
      "feeling_nauseous",
      "meal_time"
    ],
    "where": [
      "home"
    ],
    "involves": [
      "ginger",
      "water"
    ],
    "requires": [
      "kettle",
      "ice_cube_tray",
      "freezer"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "What fun shape or container will you use for your ginger ice cubes?",
    "personalization_type": "text",
    "personalization_config": {
      "placeholder": "e.g., Star-shaped molds, mini dinosaurs, or heart shapes"
    },
    "difficulty": 1,
    "source": "dietitian_reviewed",
    "areas": [
      "nutrition",
      "organization"
    ]
  },
  {
    "tip_id": "c45987a7-3866-4379-996b-d2e36fefe057",
    "micro_version": "Mix a cup of lemonade and pour it into an ice cube tray.",
    "summary": "Suck on frozen lemon-ginger cubes when liquids are hard to tolerate.",
    "details_md": "**The Experiment:** Freeze diluted lemonade with grated ginger into ice cubes to sip/suck during nausea.\n\n**Why it Works:** Cold, tart flavors are often easier to handle and keep hydration going in tiny sips.\n\n**How to Try It:**\n• Mix water + a splash of lemonade; add optional ginger.\n• Freeze in trays; keep a bag in the freezer.\n• Use during commutes or morning routines.",
    "area": "eating",
    "goals": [
      "reduce_nausea",
      "increase_hydration",
      "nutrient_density"
    ],
    "helps_with": [
      "nausea",
      "dehydration"
    ],
    "contraindications": [
      "Citrus may worsen reflux; rinse mouth after to protect enamel. Large amounts of ginger may interact with blood thinners or increase reflux; limit if advised by your clinician. Citrus can aggravate reflux or tooth enamel; rinse mouth after acidic drinks and avoid if symptoms worsen."
    ],
    "mechanisms": [
      "symptom_management",
      "meal_prep",
      "comfort",
      "relief"
    ],
    "effort": "minimal",
    "time": "5-15",
    "cost": "$",
    "when": [
      "any",
      "feeling_nauseous",
      "commute"
    ],
    "where": [
      "home",
      "work"
    ],
    "involves": [
      "lemonade",
      "ginger",
      "water"
    ],
    "satisfies": [
      "cold"
    ],
    "requires": [
      "ice_cube_tray",
      "freezer"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "Name your frozen cube creation (get creative!):",
    "personalization_type": "text",
    "personalization_config": {
      "placeholder": "e.g., Arctic Ginger Gems, Lemon Ice Rescue, Nausea Ninjas"
    },
    "difficulty": 1,
    "source": "dietitian_reviewed",
    "areas": [
      "nutrition",
      "organization"
    ]
  },
  {
    "tip_id": "c55f0762-08a2-4cb4-91e8-c8d633db4ffa",
    "micro_version": "Place a pack of peppermint gum in your pocket or bag.",
    "summary": "Chew peppermint gum or suck mints to mask odors and ease nausea.",
    "details_md": "**The Experiment:** Use peppermint gum or mints when smells trigger queasiness (kitchen, car, workplace).\n\n**Why it Works:** Peppermint aroma can distract from odor triggers and may soothe the stomach while saliva neutralizes acid.\n\n**How to Try It:**\n• Keep a pack in your bag and car.\n• Try sugar-free gum if managing blood sugar or dental health.\n• Use before entering strong-smell environments.",
    "area": "eating",
    "goals": [
      "reduce_nausea",
      "nutrient_density"
    ],
    "helps_with": [
      "nausea",
      "smell_aversion",
      "heartburn"
    ],
    "contraindications": [
      "Peppermint can worsen reflux; avoid if sensitive. Peppermint may relax the LES and worsen reflux; choose non‑mint flavors if GERD is an issue."
    ],
    "mechanisms": [
      "symptom_management",
      "behavioral_strategy",
      "convenience",
      "relief"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any",
      "odor_trigger",
      "commute",
      "meal_prep_time"
    ],
    "where": [
      "any"
    ],
    "involves": [
      "peppermint_gum",
      "mints"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "What's your 'smell danger zone' where you'll keep emergency mints?",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Car glove box",
        "Work desk",
        "Kitchen drawer",
        "Purse or bag",
        "Coat pocket"
      ],
      "multiple": true
    },
    "difficulty": 1,
    "source": "dietitian_reviewed"
  },
  {
    "tip_id": "cf16c8b8-3424-49e4-8c81-b785a931e910",
    "micro_version": "Slice a lemon or cucumber and put it in your water bottle.",
    "summary": "If plain water tastes bad, flavor it lightly (citrus, mint, flat ginger ale).",
    "details_md": "**The Experiment:** Add lemon/lime, a splash of juice, or use flat ginger ale in small sips to meet hydration goals.\n\n**Why it Works:** Mild flavors reduce aversion and encourage steady sipping, preventing dehydration that worsens nausea.\n\n**How to Try It:**\n• Keep lemon wedges or mint in the fridge.\n• Try flavored ice cubes (juice + water).\n• Use a straw if that makes sipping easier.",
    "area": "eating",
    "goals": [
      "increase_hydration",
      "reduce_nausea",
      "nutrient_density"
    ],
    "helps_with": [
      "dehydration",
      "nausea"
    ],
    "contraindications": [
      "Limit sugary beverages; carbonation may bloat—adjust to comfort. Large amounts of ginger may interact with blood thinners or increase reflux; limit if advised by your clinician. Citrus can aggravate reflux or tooth enamel; rinse mouth after acidic drinks and avoid if symptoms worsen. If managing gestational diabetes, limit added sugars in mixers (use diluted juice). Avoid caffeinated sodas late in the day; prefer caffeine‑free options."
    ],
    "mechanisms": [
      "behavioral_strategy",
      "habit_stacking",
      "convenience",
      "taste"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any",
      "water_bottle_sighting",
      "feeling_thirsty"
    ],
    "where": [
      "any"
    ],
    "involves": [
      "water",
      "lemon",
      "ginger_ale"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "Design your 'water rescue' flavor combo:",
    "personalization_type": "text",
    "personalization_config": {
      "placeholder": "e.g., Lemon + fresh mint = Spa water, Cucumber + lime = Refresher"
    },
    "difficulty": 1,
    "source": "dietitian_reviewed"
  },
  {
    "tip_id": "84271016-fed4-4b53-a1e9-ad80725ee917",
    "micro_version": "Place a box of crackers on your nightstand right now.",
    "summary": "Keep crackers by the bed and nibble before sitting up.",
    "details_md": "**The Experiment:** Eat a few plain crackers or dry cereal before getting out of bed.\n\n**Why it Works:** A little starch on board can blunt the acid surge and blood sugar dip that worsens morning nausea.\n\n**How to Try It:**\n• Pre-pack a zip bag of crackers at night.\n• Nibble slowly and wait 5–10 minutes before standing.\n• Keep a small water bottle nearby for sips.",
    "area": "eating",
    "goals": [
      "reduce_nausea",
      "morning_routine",
      "nutrient_density"
    ],
    "helps_with": [
      "nausea"
    ],
    "contraindications": [
      "Choose gluten‑free options if you have celiac disease; pick lower‑sodium crackers if on a sodium‑restricted plan per your clinician."
    ],
    "mechanisms": [
      "symptom_management",
      "routine_design",
      "relief",
      "convenience"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "morning",
      "waking_up"
    ],
    "where": [
      "home"
    ],
    "involves": [
      "crackers",
      "dry_cereal"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "What's your bedside 'morning rescue' snack and where will you keep it?",
    "personalization_type": "multi_text",
    "personalization_config": {
      "items": [
        {
          "label": "Snack choice:",
          "placeholder": "e.g., Saltines, pretzels, dry cereal"
        },
        {
          "label": "Storage spot:",
          "placeholder": "e.g., Nightstand drawer, bedside basket"
        }
      ]
    },
    "difficulty": 1,
    "source": "dietitian_reviewed"
  },
  {
    "tip_id": "b1fb0db1-90cd-4a87-a688-65510a7f134e",
    "micro_version": "Identify one protein snack (cheese, yogurt) to have tonight.",
    "summary": "Have a protein-rich bedtime snack to ease morning nausea.",
    "details_md": "**The Experiment:** Eat a small protein snack (Greek yogurt, cheese stick, peanut butter on crackers) before bed.\n\n**Why it Works:** Protein slows digestion and stabilizes blood sugar overnight, which may reduce morning queasiness.\n\n**How to Try It:**\n• Choose ~10–15g protein (yogurt, cheese, nut butter).\n• Keep portions small to prevent reflux.\n• Pair with a few crackers if needed.",
    "area": "eating",
    "goals": [
      "reduce_nausea",
      "improve_energy",
      "stable_blood_sugar",
      "nutrient_density"
    ],
    "helps_with": [
      "nausea",
      "overnight_hunger"
    ],
    "contraindications": [
      "Avoid large/fatty meals close to bed if prone to reflux."
    ],
    "mechanisms": [
      "symptom_management",
      "behavioral_strategy",
      "comfort",
      "satiety"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "evening",
      "pre_bed",
      "bedtime"
    ],
    "where": [
      "any"
    ],
    "involves": [
      "greek_yogurt",
      "cheese",
      "peanut_butter",
      "crackers"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "Choose your bedtime protein guardian:",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Greek Yogurt Dream",
        "Cheese & Crackers Duo",
        "PB Toast Comfort",
        "Handful of Almonds",
        "String Cheese Simple"
      ],
      "multiple": false
    },
    "difficulty": 1,
    "source": "dietitian_reviewed",
    "areas": [
      "nutrition",
      "organization"
    ]
  },
  {
    "tip_id": "35f529a1-e92f-403c-ba16-de140af4aeab",
    "micro_version": "Eat 3 prunes or one pear right now.",
    "summary": "Use the 'P' fruits (prunes, pears, plums, pumpkin) for constipation.",
    "details_md": "**The Experiment:** Add one serving of a 'P' fruit daily until regular (e.g., 4–6 prunes or a ripe pear).\n\n**Why it Works:** Fiber and natural sorbitol draw water into the stool and promote movement.\n\n**How to Try It:**\n• Start with one serving and increase as needed.\n• Combine with water to boost effect.\n• Rotate fruits to keep it enjoyable.",
    "area": "eating",
    "goals": [
      "constipation_relief",
      "fiber_intake",
      "nutrient_density"
    ],
    "helps_with": [
      "constipation"
    ],
    "contraindications": [
      "Reduce/stop if stools become too loose."
    ],
    "mechanisms": [
      "symptom_management",
      "nutrition_upgrade",
      "comfort",
      "routine"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any",
      "snack_time"
    ],
    "where": [
      "home",
      "work"
    ],
    "involves": [
      "prunes",
      "pears",
      "plums",
      "pumpkin"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "Pick your 'P' fruit superhero for the day!",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Prunes (The Classic)",
        "Pears (The Gentle Giant)",
        "Plums (The Sweet Solution)",
        "Pumpkin (The Smooth Operator)"
      ],
      "multiple": false
    },
    "difficulty": 1,
    "source": "dietitian_reviewed"
  },
  {
    "tip_id": "7f7df02d-5354-40ef-b35e-22a445d56241",
    "micro_version": "Get the bag of chia/flax out and put a spoon in it.",
    "summary": "Stir chia or ground flax into meals for invisible fiber.",
    "details_md": "**The Experiment:** Add 1 tablespoon of chia or ground flaxseed to oatmeal, yogurt, or smoothies daily.\n\n**Why it Works:** Soluble fiber absorbs water to soften stool and promote regularity; it also adds omega‑3s.\n\n**How to Try It:**\n• Start with 1 tsp and build up to 1 Tbsp to avoid gas.\n• Drink water with added fiber.\n• Store ground flax in the fridge.",
    "area": "eating",
    "goals": [
      "constipation_relief",
      "fiber_intake",
      "omega3_intake",
      "nutrient_density"
    ],
    "helps_with": [
      "constipation",
      "satiety"
    ],
    "contraindications": [
      "Increase gradually to minimize bloating; ensure adequate fluids."
    ],
    "mechanisms": [
      "small_changes",
      "nutrition_upgrade",
      "convenience",
      "habit"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any",
      "breakfast",
      "snack_time"
    ],
    "where": [
      "home",
      "work"
    ],
    "involves": [
      "chia",
      "flaxseed",
      "oatmeal",
      "yogurt"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "Where will you hide your fiber boost today?",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Morning oatmeal",
        "Yogurt parfait",
        "Smoothie blend",
        "Pancake batter",
        "Overnight oats"
      ],
      "multiple": true
    },
    "difficulty": 1,
    "source": "dietitian_reviewed"
  },
  {
    "tip_id": "4e4cc434-fa30-48a8-991f-afde0bb3eaad",
    "micro_version": "Check the fruit bowl for 2 ripe kiwis.",
    "summary": "Try a daily kiwi (or two) to stay regular.",
    "details_md": "**The Experiment:** Eat 1–2 ripe kiwis each day, scooped with a spoon or added to yogurt.\n\n**Why it Works:** Kiwifruit offers fiber and enzymes that can support gentle bowel movement.\n\n**How to Try It:**\n• Keep a few ripening on the counter, then refrigerate.\n• Pair with yogurt or cereal for a quick snack.\n• Choose golden kiwi if you prefer milder flavor.",
    "area": "eating",
    "goals": [
      "constipation_relief",
      "fiber_intake",
      "nutrient_density"
    ],
    "helps_with": [
      "constipation"
    ],
    "contraindications": [
      "Stop if stools become loose; consider oral allergy syndrome if sensitive to kiwi."
    ],
    "mechanisms": [
      "symptom_management",
      "snacks",
      "taste",
      "routine"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any",
      "snack_time"
    ],
    "where": [
      "home",
      "work"
    ],
    "involves": [
      "kiwi",
      "yogurt"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "When's your kiwi time today?",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Morning wake-up",
        "Mid-morning snack",
        "Lunch dessert",
        "Afternoon pick-me-up",
        "Evening treat"
      ],
      "multiple": false
    },
    "difficulty": 1,
    "source": "dietitian_reviewed"
  },
  {
    "tip_id": "29e9e140-16f4-44bf-b190-fe46cc911f51",
    "micro_version": "Set your favorite mug next to the kettle/coffee maker.",
    "summary": "Sip warm water (or warm prune juice) first thing in the morning.",
    "details_md": "**The Experiment:** Drink a mug of warm water upon waking to gently nudge digestion.\n\n**Why it Works:** Warm fluids can stimulate gut motility and are soothing on sensitive mornings.\n\n**How to Try It:**\n• Add lemon if tolerated; avoid if it triggers reflux.\n• Try warm prune juice for extra effect.\n• Pair with light stretching.",
    "area": "eating",
    "goals": [
      "constipation_relief",
      "morning_routine",
      "increase_hydration",
      "nutrient_density"
    ],
    "helps_with": [
      "constipation",
      "nausea"
    ],
    "contraindications": [],
    "mechanisms": [
      "symptom_management",
      "routine_design",
      "comfort",
      "ritual"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "morning",
      "waking_up"
    ],
    "where": [
      "any"
    ],
    "involves": [
      "water",
      "prune_juice",
      "lemon"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "Create your morning 'wake-up sip' ritual:",
    "personalization_type": "multi_text",
    "personalization_config": {
      "items": [
        {
          "label": "Drink choice:",
          "placeholder": "e.g., Warm water with lemon, warm prune juice"
        },
        {
          "label": "Where you'll sip:",
          "placeholder": "e.g., By the window, on the porch, in bed"
        }
      ]
    },
    "difficulty": 1,
    "source": "dietitian_reviewed"
  },
  {
    "tip_id": "a2f0c824-31f2-4a88-b629-bc310a3d5431",
    "micro_version": "Decide exactly which loop/block you will walk after dinner.",
    "summary": "Take a 10‑minute walk after meals to aid digestion.",
    "details_md": "**The Experiment:** Set a 10‑minute timer and walk after lunch or dinner.\n\n**Why it Works:** Light movement leverages gravity and stimulates peristalsis, easing constipation and bloating.\n\n**How to Try It:**\n• Stroll indoors or around the block—keep it gentle.\n• Invite a partner/friend for accountability.\n• Skip right before bed if reflux is an issue—finish 2–3 hours before sleep.",
    "area": "eating",
    "goals": [
      "constipation_relief",
      "heartburn_relief",
      "improve_energy",
      "nutrient_density"
    ],
    "helps_with": [
      "constipation",
      "bloating",
      "heartburn"
    ],
    "contraindications": [
      "Skip walking if you’ve been advised pelvic rest or activity restrictions (e.g., bleeding, placenta previa, risk of preterm labor)."
    ],
    "mechanisms": [
      "behavioral_strategy",
      "routine_design",
      "movement",
      "accountability"
    ],
    "effort": "low",
    "time": "5-15",
    "cost": "$",
    "when": [
      "any",
      "post_meal"
    ],
    "where": [
      "home",
      "work",
      "outdoors"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "Design your post-meal walking adventure:",
    "personalization_type": "multi_text",
    "personalization_config": {
      "items": [
        {
          "label": "Walking route:",
          "placeholder": "e.g., Around the block, office hallways, backyard loops"
        },
        {
          "label": "Walking buddy (optional):",
          "placeholder": "e.g., Partner, dog, podcast companion"
        }
      ]
    },
    "difficulty": 1,
    "source": "dietitian_reviewed",
    "areas": [
      "nutrition",
      "organization"
    ]
  },
  {
    "tip_id": "77f34008-5bb3-4a16-a5c8-d252fb74f85e",
    "micro_version": "Pack a small tin of almonds or gum in your bag.",
    "summary": "Chew gum or munch a few almonds after meals for heartburn relief.",
    "details_md": "**The Experiment:** After eating, chew sugar‑free gum or eat 5–10 raw almonds.\n\n**Why it Works:** Chewing boosts saliva, which can neutralize acid; almonds may buffer stomach acid for some.\n\n**How to Try It:**\n• Keep gum or almonds in your bag or nightstand.\n• Chew 10–20 minutes after meals.\n• Pair with staying upright for at least an hour.",
    "area": "eating",
    "goals": [
      "heartburn_relief",
      "nutrient_density",
      "post_meal_relief"
    ],
    "helps_with": [
      "heartburn",
      "reflux"
    ],
    "contraindications": [
      "Avoid nuts if allergic; mint gum can aggravate reflux for some—try fruit flavors."
    ],
    "mechanisms": [
      "symptom_management",
      "behavioral_strategy",
      "relief",
      "convenience"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any",
      "post_meal",
      "heartburn_flare"
    ],
    "where": [
      "any"
    ],
    "involves": [
      "almonds",
      "gum"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "Pick your heartburn fighter:",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Gum Champion (sugar-free)",
        "Almond Army (5-10 nuts)",
        "Both - Double Defense!"
      ],
      "multiple": false
    },
    "difficulty": 1,
    "source": "dietitian_reviewed"
  },
  {
    "tip_id": "4a9675df-9669-4703-8022-77022718c89e",
    "micro_version": "Heat a mug of water and add 1 tsp honey.",
    "summary": "Try a warm ACV + honey drink for reflux relief.",
    "details_md": "**The Experiment:** Mix 1 tsp apple cider vinegar + 1 tsp honey in warm water and sip slowly.\n\n**Why it Works:** Some find small amounts of ACV tone digestion and reduce symptoms; warmth is soothing.\n\n**How to Try It:**\n• Start with 1 tsp ACV; increase only if helpful.\n• Rinse mouth after to protect enamel.\n• Stop if it worsens symptoms.",
    "area": "eating",
    "goals": [
      "heartburn_relief",
      "increase_hydration",
      "nutrient_density"
    ],
    "helps_with": [
      "heartburn"
    ],
    "contraindications": [
      "May aggravate reflux or damage enamel; avoid if advised by clinician or with esophagitis. Avoid undiluted ACV; may irritate esophagus or enamel. Not a replacement for medical GERD treatment; stop if pain or burning worsens."
    ],
    "mechanisms": [
      "symptom_management",
      "comfort",
      "ritual"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "evening",
      "any"
    ],
    "where": [
      "home"
    ],
    "involves": [
      "apple_cider_vinegar",
      "honey",
      "water"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "When will you try your 'Reflux Remedy' drink?",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Morning wake-up",
        "Before lunch",
        "After dinner",
        "Before bedtime"
      ],
      "multiple": false
    },
    "difficulty": 1,
    "source": "dietitian_reviewed"
  },
  {
    "tip_id": "106146f9-39fd-49c7-8c61-207bffd34b34",
    "micro_version": "Squeeze a wedge of lemon into a glass of water.",
    "summary": "Sip lemon water to test if it soothes your heartburn.",
    "details_md": "**The Experiment:** Add a squeeze of lemon to warm or room‑temp water and sip slowly.\n\n**Why it Works:** For some, mild acidity can support digestion and reduce reflux sensations.\n\n**How to Try It:**\n• Try small amounts first; stop if it burns.\n• Avoid near toothbrushing to protect enamel.\n• Prefer warm over cold if cold triggers cramps.",
    "area": "eating",
    "goals": [
      "heartburn_relief",
      "increase_hydration",
      "nutrient_density"
    ],
    "helps_with": [
      "heartburn"
    ],
    "contraindications": [
      "Citrus can worsen reflux or dental sensitivity—use only if it helps you. Citrus can aggravate reflux or tooth enamel; rinse mouth after acidic drinks and avoid if symptoms worsen."
    ],
    "mechanisms": [
      "symptom_management",
      "comfort",
      "experimentation"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any"
    ],
    "where": [
      "home",
      "work"
    ],
    "involves": [
      "lemon",
      "water"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "What temperature will your lemon water experiment be?",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Warm & Cozy",
        "Room Temperature Neutral",
        "Cool & Refreshing",
        "Hot Tea Style"
      ],
      "multiple": false
    },
    "difficulty": 1,
    "source": "dietitian_reviewed"
  },
  {
    "tip_id": "da73c592-f80c-4d8a-9254-718195c4ec0e",
    "micro_version": "Check your medicine cabinet or add to shopping list.",
    "summary": "Use chewable papaya enzymes as a natural antacid alternative.",
    "details_md": "**The Experiment:** Chew 1–3 papaya enzyme tablets after meals as needed.\n\n**Why it Works:** Papain (from papaya) can support digestion and ease mild indigestion.\n\n**How to Try It:**\n• Choose a brand without extra herbs you don’t need.\n• Use after meals that tend to trigger heartburn.\n• Discuss with your provider if unsure.",
    "area": "eating",
    "goals": [
      "heartburn_relief",
      "nutrient_density",
      "digestive_enzyme_relief"
    ],
    "helps_with": [
      "heartburn",
      "indigestion"
    ],
    "contraindications": [
      "Avoid if allergic to latex/papaya; check with clinician before regular use. Avoid if allergic to latex or papaya. Use packaged enzyme tablets; avoid unripe papaya in large amounts. Check with your clinician before regular use."
    ],
    "mechanisms": [
      "symptom_management",
      "relief",
      "convenience"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any",
      "post_meal",
      "heartburn_flare"
    ],
    "where": [
      "any"
    ],
    "involves": [
      "papaya_enzyme"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "When will you deploy your 'Papaya Power' tablets?",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "After every meal",
        "Only after heavy meals",
        "When heartburn strikes",
        "Before problem foods"
      ],
      "multiple": true
    },
    "difficulty": 1,
    "source": "dietitian_reviewed"
  },
  {
    "tip_id": "7bfa5b1a-a16f-4634-a724-37526d3f8cad",
    "micro_version": "Look up a sushi menu and find the \"Cooked Rolls\" section.",
    "summary": "Craving sushi? Choose veggie or fully cooked rolls.",
    "details_md": "**The Experiment:** Order avocado, cucumber, or tempura shrimp rolls; skip raw fish.\n\n**Why it Works:** You can keep the flavors you love while avoiding the higher risk of raw fish during pregnancy.\n\n**How to Try It:**\n• Ask for cooked fish (eel, shrimp) or veggie rolls.\n• Make sushi bowls at home with nori strips and cooked fillings.\n• Use low‑sodium soy sauce and plenty of ginger/wasabi for the vibe.",
    "area": "eating",
    "goals": [
      "food_safety",
      "craving_satisfaction",
      "nutrient_density"
    ],
    "helps_with": [
      "food_safety_anxiety",
      "cravings"
    ],
    "contraindications": [
      "Avoid raw fish and high‑mercury species; confirm sauces/aioli use pasteurized eggs. Avoid raw fish/shellfish and high‑mercury species (shark, swordfish, king mackerel, tilefish, bigeye tuna). Ensure sauces use pasteurized eggs."
    ],
    "mechanisms": [
      "food_swap",
      "safety_strategy",
      "craving_substitution",
      "safety"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any"
    ],
    "where": [
      "restaurant",
      "home"
    ],
    "involves": [
      "nori",
      "avocado",
      "shrimp_cooked",
      "eel_cooked",
      "rice"
    ],
    "satisfies": [
      "umami"
    ],
    "requires": [
      "rice_cooker (optional)",
      "basic_cooking"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "Build your pregnancy-safe sushi order:",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Avocado Roll",
        "Cucumber Roll",
        "Tempura Shrimp",
        "California Roll (cooked crab)",
        "Veggie Dragon Roll",
        "Eel Roll (cooked)"
      ],
      "multiple": true
    },
    "difficulty": 1,
    "source": "dietitian_reviewed"
  },
  {
    "tip_id": "973ee1d3-d3e2-4754-9f11-f1936a5e867b",
    "micro_version": "Take your fanciest glass out of the cabinet.",
    "summary": "Create a mocktail ritual in your fanciest glass.",
    "details_md": "**The Experiment:** Mix sparkling water with tart cherry or pomegranate juice and citrus; serve in stemware.\n\n**Why it Works:** Honors the social ritual of a drink while keeping pregnancy alcohol‑free.\n\n**How to Try It:**\n• Keep a few favorite juice mixers on hand.\n• Garnish (citrus, herbs) to make it feel special.\n• Use unsweetened juices if watching sugar.",
    "area": "eating",
    "goals": [
      "alcohol_free",
      "healthy_snacking",
      "reduce_sugar",
      "nutrient_density"
    ],
    "helps_with": [
      "social_pressure",
      "evening_cravings"
    ],
    "contraindications": [],
    "mechanisms": [
      "food_swap",
      "ritual_design",
      "novelty",
      "reward",
      "social_norms"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any"
    ],
    "where": [
      "home",
      "restaurant",
      "social_event"
    ],
    "satisfies": [
      "bubbly"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "Name your signature mocktail creation!",
    "personalization_type": "text",
    "personalization_config": {
      "placeholder": "e.g., The Sparkling Sunset, Berry Bliss, Citrus Celebration"
    },
    "difficulty": 1,
    "source": "dietitian_reviewed"
  },
  {
    "tip_id": "fa453cec-f876-446e-87ab-ce2ce540e5ea",
    "micro_version": "Remind yourself: \"Microwave deli meat for 30 seconds.\"",
    "summary": "Heat deli meats until steaming before building your sandwich.",
    "details_md": "**The Experiment:** Microwave or pan‑heat lunch meats to 165°F, cool slightly, then assemble your sub.\n\n**Why it Works:** Heating reduces Listeria risk while letting you enjoy your favorite sandwich.\n\n**How to Try It:**\n• Ask shops to toast/steam your sandwich.\n• At home, microwave meat 30–60 seconds until steaming.\n• Use pasteurized cheeses and add veggies for crunch.",
    "area": "eating",
    "goals": [
      "food_safety",
      "craving_satisfaction",
      "nutrient_density"
    ],
    "helps_with": [
      "food_safety_anxiety",
      "cravings"
    ],
    "contraindications": [
      "Use food thermometer if unsure; avoid cross‑contamination on cutting boards. Heat meats to steaming (165°F) to reduce Listeria risk and avoid cross‑contamination on boards/knives."
    ],
    "mechanisms": [
      "safety_strategy",
      "food_swap",
      "safety",
      "convenience"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any"
    ],
    "where": [
      "home",
      "restaurant"
    ],
    "involves": [
      "turkey",
      "ham",
      "cheese_pasteurized",
      "bread"
    ],
    "satisfies": [
      "savory"
    ],
    "requires": [
      "microwave",
      "skillet",
      "thermometer (optional)"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "Design your safe sandwich strategy:",
    "personalization_type": "multi_text",
    "personalization_config": {
      "items": [
        {
          "label": "Meat choice:",
          "placeholder": "e.g., Turkey, ham, roast beef"
        },
        {
          "label": "Heating method:",
          "placeholder": "e.g., Microwave 45 sec, panini press, skillet"
        }
      ]
    },
    "difficulty": 1,
    "source": "dietitian_reviewed"
  },
  {
    "tip_id": "f9f32fb5-f964-4098-9069-3a39e328afb9",
    "micro_version": "Search for an \"edible cookie dough recipe\" and save it.",
    "summary": "Use edible cookie dough (egg‑free, heat‑treated flour) to satisfy dough cravings.",
    "details_md": "**The Experiment:** Buy or make cookie dough designed to be eaten raw.\n\n**Why it Works:** You can enjoy the flavor/texture without risk from raw eggs or unheated flour.\n\n**How to Try It:**\n• Look for labels: pasteurized/egg‑free + heat‑treated flour.\n• DIY: bake flour 5–10 min at ~350°F; omit eggs.\n• Portion a spoonful into yogurt or freeze as bites.",
    "area": "eating",
    "goals": [
      "food_safety",
      "craving_satisfaction",
      "nutrient_density"
    ],
    "helps_with": [
      "sweet_cravings",
      "food_safety_anxiety"
    ],
    "contraindications": [
      "Only use doughs labeled safe‑to‑eat raw or homemade with heat‑treated flour and no raw egg; portion mindfully if managing blood sugar."
    ],
    "mechanisms": [
      "food_swap",
      "safety_strategy",
      "craving_substitution",
      "novelty"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any"
    ],
    "where": [
      "home"
    ],
    "involves": [
      "edible_cookie_dough"
    ],
    "satisfies": [
      "sweet"
    ],
    "requires": [
      "oven (for DIY)",
      "basic_cooking"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "How will you enjoy your safe cookie dough?",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Straight from the spoon",
        "Mixed in yogurt",
        "Frozen as bites",
        "Blended in a smoothie",
        "On top of ice cream"
      ],
      "multiple": true
    },
    "difficulty": 1,
    "source": "dietitian_reviewed"
  },
  {
    "tip_id": "f27ba582-897d-43ee-8240-e43fcdd52ac1",
    "micro_version": "Mix 1 scoop of regular and 1 scoop of decaf in a jar.",
    "summary": "Switch to half‑caff or decaf (or roasted‑chicory 'coffee') to trim caffeine.",
    "details_md": "**The Experiment:** Mix half regular + half decaf grounds, or try a chicory/dandelion blend.\n\n**Why it Works:** Keeps the comforting ritual with less (or no) caffeine.\n\n**How to Try It:**\n• Transition gradually over a week.\n• Order half‑caff lattes or decaf at cafés.\n• Track total caffeine from tea/chocolate/cola.",
    "area": "eating",
    "goals": [
      "reduce_caffeine",
      "sleep_quality",
      "nutrient_density"
    ],
    "helps_with": [
      "jitters",
      "sleep"
    ],
    "contraindications": [
      "Decaf still contains small amounts of caffeine. Herbal 'coffee' blends vary—avoid unverified herbs (e.g., large amounts of licorice); choose simple roasted chicory/dandelion and confirm with your clinician if unsure."
    ],
    "mechanisms": [
      "food_swap",
      "behavioral_strategy",
      "ritual",
      "comfort"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any"
    ],
    "where": [
      "home",
      "work",
      "restaurant"
    ],
    "involves": [
      "coffee",
      "decaf_coffee",
      "chicory_coffee"
    ],
    "satisfies": [
      "warm_drink"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "Choose your caffeine reduction strategy:",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Half-caff blend (50/50)",
        "Full decaf warrior",
        "Chicory coffee alternative",
        "Gradual weekly reduction",
        "Morning regular, afternoon decaf"
      ],
      "multiple": false
    },
    "difficulty": 1,
    "source": "dietitian_reviewed"
  },
  {
    "tip_id": "8753afeb-59d0-4d5c-9e0d-70b3fa0268c5",
    "micro_version": "Send a message to your doctor asking if this is okay.",
    "summary": "If prenatals make you sick, ask about children's vitamins as a short‑term bridge.",
    "details_md": "**The Experiment:** With clinician approval, use chewable kids’ multivitamins temporarily until nausea improves.\n\n**Why it Works:** Better than nothing when pills won’t stay down; easier on the stomach.\n\n**How to Try It:**\n• Confirm dose/brand with your provider.\n• Keep focusing on folate‑rich foods meanwhile.\n• Resume a full prenatal as soon as tolerated.",
    "area": "eating",
    "goals": [
      "reduce_nausea",
      "nutrient_density",
      "prenatal_vitamin_routine"
    ],
    "helps_with": [
      "nausea",
      "prenatal_vitamin_adherence"
    ],
    "contraindications": [
      "Only with clinician guidance; gummies may lack iron/iodine/choline. Use only with clinician guidance; children’s gummies often lack iron/iodine/choline and are a temporary bridge, not a full replacement."
    ],
    "mechanisms": [
      "safety_strategy",
      "behavioral_strategy",
      "relief",
      "self_efficacy"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any"
    ],
    "where": [
      "any"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "When will you take your temporary vitamin solution?",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "With breakfast",
        "After lunch",
        "With dinner",
        "Before bed",
        "Split morning/evening"
      ],
      "multiple": false
    },
    "difficulty": 1,
    "source": "dietitian_reviewed"
  },
  {
    "tip_id": "bd42db75-c10e-498c-bd8c-8cf85af0f51f",
    "micro_version": "Plan one meal that relies on fruit or eggs instead of veggies.",
    "summary": "If veggies are a no‑go, lean on fruits, beans, eggs, and dairy for nutrients.",
    "details_md": "**The Experiment:** Build meals from tolerated foods that still supply folate, iron, calcium, and protein.\n\n**Why it Works:** You can meet needs without forcing trigger foods; variety returns as nausea lifts.\n\n**How to Try It:**\n• Prioritize fruit smoothies, beans/lentils, eggs, and yogurt.\n• Add a prenatal and 'gap' foods like choline‑rich eggs when possible.\n• Re‑test veggies later in tiny portions.",
    "area": "eating",
    "goals": [
      "reduce_nausea",
      "nutrient_density",
      "nutrient_density",
      "nausea_safe_swaps"
    ],
    "helps_with": [
      "food_aversion",
      "nausea",
      "perfectionism"
    ],
    "contraindications": [
      "Use only pasteurized dairy. If lactose intolerant, choose lactose‑free or fortified non‑dairy alternatives."
    ],
    "mechanisms": [
      "mindset_reframe",
      "nutrition_upgrade",
      "self_compassion",
      "competence"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any"
    ],
    "where": [
      "home",
      "restaurant",
      "work"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "Build your veggie-free nutrient team:",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Fruit smoothies",
        "Bean burritos",
        "Scrambled eggs",
        "Greek yogurt",
        "Cheese & crackers",
        "Hummus & pita"
      ],
      "multiple": true
    },
    "difficulty": 1,
    "source": "dietitian_reviewed"
  },
  {
    "tip_id": "ff2970ca-7637-48bc-84cf-3ecd5383205f",
    "micro_version": "Add \"Zucchini\" or \"Carrots\" to your grocery list.",
    "summary": "Blend extra veggies (and white beans) into pasta sauces and soups.",
    "details_md": "**The Experiment:** Sauté mild veggies/beans, then purée into tomato sauce or soup.\n\n**Why it Works:** Increases fiber, folate, and iron without 'veggie' taste or smell.\n\n**How to Try It:**\n• Use carrots, zucchini, bell peppers, spinach, white beans.\n• Blend smooth; freeze portions for later.\n• Season boldly (garlic, herbs) to keep it craveable.",
    "area": "eating",
    "goals": [
      "fiber_intake",
      "folate_intake",
      "iron_intake",
      "nutrient_density"
    ],
    "helps_with": [
      "aversion",
      "constipation",
      "micronutrient_gaps"
    ],
    "contraindications": [],
    "mechanisms": [
      "meal_prep",
      "nutrition_upgrade",
      "stealth",
      "convenience"
    ],
    "effort": "minimal",
    "time": "15-30min",
    "cost": "$",
    "when": [
      "any"
    ],
    "where": [
      "home"
    ],
    "involves": [
      "tomato_sauce",
      "white_beans",
      "spinach",
      "zucchini",
      "carrot"
    ],
    "requires": [
      "blender",
      "saucepan",
      "freezer",
      "basic_cooking"
    ],
    "features": [
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "What veggies will you hide in your sauce?",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Carrots (sweetness)",
        "Zucchini (invisible)",
        "Bell peppers (flavor)",
        "Spinach (nutrients)",
        "White beans (protein)",
        "Cauliflower (creamy)"
      ],
      "multiple": true
    },
    "difficulty": 1,
    "source": "dietitian_reviewed"
  },
  {
    "tip_id": "7280d3cf-4770-4ed8-baff-5fdeab61c9d6",
    "micro_version": "Put a bag of fresh spinach in the freezer right now.",
    "summary": "Add spinach or peeled zucchini to fruit smoothies (you won't taste it).",
    "details_md": "**The Experiment:** Toss a handful of baby spinach or 1/2 cup peeled zucchini into your smoothie.\n\n**Why it Works:** Boosts folate and fiber invisibly while keeping the flavor you like.\n\n**How to Try It:**\n• Pair with banana/berries to mask color/flavor.\n• Add Greek yogurt or protein powder for staying power.\n• Prep freezer smoothie kits on weekends.",
    "area": "eating",
    "goals": [
      "folate_intake",
      "fiber_intake",
      "protein_intake",
      "nutrient_density"
    ],
    "helps_with": [
      "aversion",
      "constipation",
      "low_energy"
    ],
    "contraindications": [],
    "mechanisms": [
      "simple_swap",
      "meal_prep",
      "stealth",
      "taste"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any"
    ],
    "where": [
      "home"
    ],
    "involves": [
      "spinach",
      "zucchini",
      "banana",
      "berries",
      "yogurt"
    ],
    "requires": [
      "blender",
      "freezer"
    ],
    "features": [
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "Name your secret veggie smoothie blend:",
    "personalization_type": "text",
    "personalization_config": {
      "placeholder": "e.g., Green Monster, Hulk Shake, Sneaky Greenie"
    },
    "difficulty": 1,
    "source": "dietitian_reviewed"
  },
  {
    "tip_id": "1a2a9268-c165-46e3-a411-4c4bbce49c27",
    "micro_version": "Buy a head of cauliflower or a bag of frozen cauliflower.",
    "summary": "Blend cauliflower into mashed potatoes for a stealth veggie upgrade.",
    "details_md": "**The Experiment:** Boil cauliflower with potatoes and mash together (50/50 or 25/75).\n\n**Why it Works:** Adds fiber and vitamins with classic comfort‑food taste.\n\n**How to Try It:**\n• Steam/boil cauliflower until very soft.\n• Mash with potatoes, butter, and seasonings.\n• Try the same trick in mac‑and‑cheese sauces.",
    "area": "eating",
    "goals": [
      "fiber_intake",
      "produce_priority",
      "nutrient_density"
    ],
    "helps_with": [
      "aversion",
      "constipation"
    ],
    "contraindications": [],
    "mechanisms": [
      "simple_swap",
      "nutrition_upgrade",
      "comfort",
      "stealth"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any"
    ],
    "where": [
      "home"
    ],
    "involves": [
      "cauliflower",
      "potatoes"
    ],
    "requires": [
      "pot",
      "masher"
    ],
    "features": [
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "What's your cauliflower-to-potato ratio going to be?",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "25% cauliflower (Beginner)",
        "50/50 (Balanced)",
        "75% cauliflower (Pro level)"
      ],
      "multiple": false
    },
    "difficulty": 1,
    "source": "dietitian_reviewed"
  },
  {
    "tip_id": "bfc4d272-1da0-490f-8145-5e806ae4e5bb",
    "micro_version": "Take out a pot and an egg to be ready for cooking.",
    "summary": "Stir an egg into hot oatmeal for extra protein and choline.",
    "details_md": "**The Experiment:** Whisk an egg into simmering oats on low heat, stirring until creamy and fully set.\n\n**Why it Works:** Turns oats into a higher‑protein, choline‑rich breakfast without tasting 'eggy'.\n\n**How to Try It:**\n• Temper egg with a little hot oats first to prevent curdling.\n• Cook until thick and no liquid egg remains.\n• Top with fruit/nut butter for balance.",
    "area": "eating",
    "goals": [
      "protein_intake",
      "choline_intake",
      "satiety",
      "nutrient_density"
    ],
    "helps_with": [
      "protein_gap",
      "choline_gap",
      "morning_energy"
    ],
    "contraindications": [
      "Ensure egg is fully cooked; avoid if egg‑allergic."
    ],
    "mechanisms": [
      "nutrition_upgrade",
      "simple_swap",
      "satiety",
      "stealth"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any"
    ],
    "where": [
      "home"
    ],
    "involves": [
      "oats",
      "egg",
      "fruit",
      "nut_butter"
    ],
    "requires": [
      "saucepan",
      "whisk",
      "basic_cooking"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "What toppings will crown your protein-packed oatmeal?",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Berries & honey",
        "Banana & peanut butter",
        "Apple & cinnamon",
        "Nuts & maple syrup",
        "Chocolate chips (because why not)"
      ],
      "multiple": true
    },
    "difficulty": 1,
    "source": "dietitian_reviewed"
  },
  {
    "tip_id": "f7b96ddc-e90d-4d21-aaff-ab77c31fd05f",
    "micro_version": "Search for and save a \"Maple Pots de Crème\" recipe.",
    "summary": "Make maple pots de crème for a choline‑rich dessert.",
    "details_md": "**The Experiment:** Bake a simple egg‑yolk custard sweetened with maple syrup until set.\n\n**Why it Works:** Egg yolks are choline‑dense; custard is an easy, craveable way to include them.\n\n**How to Try It:**\n• Use pasteurized dairy; bake until custard reaches ~160°F and sets.\n• Chill for a few hours before serving.\n• Top with berries for freshness.",
    "area": "eating",
    "goals": [
      "choline_intake",
      "healthy_snacking",
      "nutrient_density"
    ],
    "helps_with": [
      "choline_gap",
      "sweet_cravings"
    ],
    "contraindications": [
      "Fully cook custard; avoid if egg/dairy‑allergic."
    ],
    "mechanisms": [
      "nutrition_upgrade",
      "food_swap",
      "indulgence",
      "stealth"
    ],
    "effort": "minimal",
    "time": "5-15",
    "cost": "$",
    "when": [
      "any"
    ],
    "where": [
      "home"
    ],
    "involves": [
      "eggs",
      "milk",
      "maple_syrup",
      "berries"
    ],
    "satisfies": [
      "sweet"
    ],
    "requires": [
      "oven",
      "ramekins",
      "thermometer (optional)",
      "basic_cooking"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "When will you enjoy your fancy custard treat?",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "After dinner dessert",
        "Afternoon indulgence",
        "Weekend brunch treat",
        "Special occasion only"
      ],
      "multiple": false
    },
    "difficulty": 1,
    "source": "dietitian_reviewed"
  },
  {
    "tip_id": "b0e72e31-0479-4961-b9a2-e59f0764aba9",
    "micro_version": "Eat one cheese stick or a yogurt cup right now.",
    "summary": "Double up on dairy: Greek yogurt, hard cheeses, high‑protein milk.",
    "details_md": "**The Experiment:** Swap sour cream for Greek yogurt; add cheese to snacks; choose ultrafiltered milk.\n\n**Why it Works:** Effortless way to boost protein and calcium when meat sounds unappealing.\n\n**How to Try It:**\n• Keep single‑serve Greek yogurt cups handy.\n• Pair cheese with fruit or whole‑grain crackers.\n• Use high‑protein milk in oats/smoothies.",
    "area": "eating",
    "goals": [
      "protein_intake",
      "calcium_intake",
      "nutrient_density"
    ],
    "helps_with": [
      "protein_gap",
      "calcium_gap",
      "nausea_aversion_to_meat"
    ],
    "contraindications": [
      "Use only pasteurized dairy. If lactose intolerant, choose lactose‑free or fortified non‑dairy alternatives."
    ],
    "mechanisms": [
      "simple_swap",
      "nutrition_upgrade",
      "convenience",
      "taste"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any"
    ],
    "where": [
      "home",
      "work"
    ],
    "involves": [
      "greek_yogurt",
      "cheese",
      "milk"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "Pick your dairy all-stars for today:",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Greek yogurt parfait",
        "String cheese snacks",
        "Ultra-filtered milk",
        "Cottage cheese bowl",
        "Cheese & crackers"
      ],
      "multiple": true
    },
    "difficulty": 1,
    "source": "dietitian_reviewed"
  },
  {
    "tip_id": "c414a924-38c0-4385-b249-b13b3d9e39ec",
    "micro_version": "Add \"Chickpea Pasta\" or \"Protein Pancake Mix\" to list.",
    "summary": "Use protein‑fortified pancake mix, pasta, or cereal to quietly raise protein.",
    "details_md": "**The Experiment:** Replace one staple with its higher‑protein version this week.\n\n**Why it Works:** Upgrades familiar foods without extra volume or effort.\n\n**How to Try It:**\n• Try chickpea/lentil pasta or protein pancake mix.\n• Pick cereals with ≥10g protein per serving.\n• Rotate options to prevent taste fatigue.",
    "area": "eating",
    "goals": [
      "protein_intake",
      "satiety",
      "nutrient_density"
    ],
    "helps_with": [
      "protein_gap",
      "energy_crashes"
    ],
    "contraindications": [
      "If you have celiac disease, choose certified gluten‑free options. Check labels for added sugars or artificial sweeteners if they bother you."
    ],
    "mechanisms": [
      "simple_swap",
      "planning",
      "stealth",
      "convenience"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any"
    ],
    "where": [
      "home"
    ],
    "involves": [
      "protein_pasta",
      "protein_pancake_mix",
      "high_protein_cereal"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "Which protein-fortified swap will you try first?",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Protein pancake mix",
        "Chickpea pasta",
        "Lentil pasta",
        "High-protein cereal",
        "Protein bread"
      ],
      "multiple": false
    },
    "difficulty": 1,
    "source": "dietitian_reviewed"
  },
  {
    "tip_id": "4023164b-2b78-4b5f-b7e1-4ec1cad05e92",
    "micro_version": "Get the jar of nut butter out and put a spoon next to it.",
    "summary": "Add protein powder or nut butter to oatmeal/smoothies.",
    "details_md": "**The Experiment:** Stir in one scoop of protein powder or a spoonful of nut butter.\n\n**Why it Works:** Small additions meaningfully increase protein and satiety.\n\n**How to Try It:**\n• Choose a pregnancy‑safe protein powder if using.\n• Blend into smoothies or stir into warm oats.\n• Adjust liquid for desired texture.",
    "area": "eating",
    "goals": [
      "protein_intake",
      "satiety",
      "nutrient_density"
    ],
    "helps_with": [
      "protein_gap",
      "hunger"
    ],
    "contraindications": [
      "Check ingredients if sensitive; some powders contain herbs/sweeteners. Select pregnancy‑appropriate powders without high‑dose herbs/stimulants; verify vitamin A forms/amounts; some sweeteners may cause GI upset."
    ],
    "mechanisms": [
      "nutrition_upgrade",
      "small_changes",
      "convenience",
      "satiety"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any"
    ],
    "where": [
      "home",
      "work"
    ],
    "involves": [
      "protein_powder",
      "nut_butter",
      "oats",
      "milk"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "Create your protein power-up combo:",
    "personalization_type": "multi_text",
    "personalization_config": {
      "items": [
        {
          "label": "Base (oatmeal/smoothie):",
          "placeholder": "e.g., Berry smoothie, cinnamon oatmeal"
        },
        {
          "label": "Protein boost:",
          "placeholder": "e.g., Vanilla protein powder, almond butter"
        }
      ]
    },
    "difficulty": 1,
    "source": "dietitian_reviewed"
  },
  {
    "tip_id": "df05f966-4c0b-4829-91be-a5ddc0171668",
    "micro_version": "Put a bowl of frozen edamame in the microwave.",
    "summary": "Snack on edamame for protein, iron, and folate in one bite.",
    "details_md": "**The Experiment:** Microwave frozen edamame and sprinkle with a little salt.\n\n**Why it Works:** A salty, satisfying snack that covers multiple prenatal nutrients.\n\n**How to Try It:**\n• Buy frozen in‑pod edamame for convenience.\n• Portion into small bowls to grab‑and‑go.\n• Add chili flakes or lemon if you like.",
    "area": "eating",
    "goals": [
      "protein_intake",
      "iron_intake",
      "folate_intake",
      "healthy_snacking",
      "nutrient_density"
    ],
    "helps_with": [
      "protein_gap",
      "anemia_risk",
      "snack_attacks"
    ],
    "contraindications": [
      "Avoid if soy‑allergic. If on thyroid hormone (levothyroxine), separate soy intake and medication by at least 4 hours."
    ],
    "mechanisms": [
      "snacks",
      "simple_swap",
      "taste",
      "convenience"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any"
    ],
    "where": [
      "home",
      "work"
    ],
    "involves": [
      "edamame",
      "salt"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "How will you jazz up your edamame?",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Classic sea salt",
        "Spicy chili flakes",
        "Lemon zest twist",
        "Everything bagel seasoning",
        "Garlic powder kick"
      ],
      "multiple": true
    },
    "difficulty": 1,
    "source": "dietitian_reviewed"
  },
  {
    "tip_id": "ab2ef623-ad42-4162-abe4-2c1e08a1fc24",
    "micro_version": "Drain and rinse a can of white beans.",
    "summary": "Make white‑bean hummus for an iron‑rich dip.",
    "details_md": "**The Experiment:** Blend cannellini beans with olive oil, lemon, garlic, and salt.\n\n**Why it Works:** White beans provide notable iron; lemon adds vitamin C for absorption.\n\n**How to Try It:**\n• Serve with bell peppers and whole‑grain crackers.\n• Batch on Sunday; use all week.\n• Add tahini for creaminess if you like.",
    "area": "eating",
    "goals": [
      "iron_intake",
      "fiber_intake",
      "healthy_snacking",
      "nutrient_density"
    ],
    "helps_with": [
      "anemia_risk",
      "constipation"
    ],
    "contraindications": [
      "Beans can worsen IBS symptoms for some; increase gradually and drink water."
    ],
    "mechanisms": [
      "meal_prep",
      "nutrition_upgrade",
      "taste",
      "convenience"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any"
    ],
    "where": [
      "home"
    ],
    "involves": [
      "white_beans",
      "lemon",
      "olive_oil",
      "tahini"
    ],
    "requires": [
      "blender_or_processor"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "What will you dip in your white bean hummus?",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Bell pepper boats",
        "Carrot sticks",
        "Whole grain crackers",
        "Cucumber rounds",
        "Pita triangles",
        "Celery sticks"
      ],
      "multiple": true
    },
    "difficulty": 1,
    "source": "dietitian_reviewed",
    "areas": [
      "nutrition",
      "organization"
    ]
  },
  {
    "tip_id": "78d7ca48-0fcd-4189-a40c-4e3441557364",
    "micro_version": "Check your cabinet to see if you have a cast iron skillet.",
    "summary": "Cook tomato‑based meals in cast iron for a tiny iron boost.",
    "details_md": "**The Experiment:** Use a cast‑iron skillet or Dutch oven for chili or marinara night.\n\n**Why it Works:** A small amount of iron leaches into acidic foods—an effortless micro‑boost.\n\n**How to Try It:**\n• Keep pans well seasoned to prevent sticking.\n• Avoid storing leftovers in cast iron.\n• Pair with vitamin C‑rich sides for better absorption.",
    "area": "eating",
    "goals": [
      "iron_intake",
      "nutrient_density"
    ],
    "helps_with": [
      "anemia_risk"
    ],
    "contraindications": [
      "If you’ve been advised to limit iron (e.g., hemochromatosis), use standard cookware instead."
    ],
    "mechanisms": [
      "kitchen_hack",
      "nutrition_upgrade",
      "convenience"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any"
    ],
    "where": [
      "home"
    ],
    "requires": [
      "cast_iron_pan"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "What tomato dish will you cook in cast iron?",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Classic marinara",
        "Hearty chili",
        "Shakshuka",
        "Tomato soup",
        "Bolognese sauce"
      ],
      "multiple": false
    },
    "difficulty": 1,
    "source": "dietitian_reviewed"
  },
  {
    "tip_id": "d67a23ae-4f49-416b-8e8d-fec79b5c916f",
    "micro_version": "Pair your next beans/greens meal with a glass of orange juice.",
    "summary": "Pair plant iron with vitamin C; separate iron from dairy/tea/coffee.",
    "details_md": "**The Experiment:** Add citrus, bell pepper, or berries when you eat beans/greens or take iron.\n\n**Why it Works:** Vitamin C increases non‑heme iron absorption; calcium/tannins inhibit it.\n\n**How to Try It:**\n• Squeeze lemon over lentils or spinach.\n• Have strawberries with fortified cereal.\n• Take iron 1–2 hours away from dairy/coffee/tea.",
    "area": "eating",
    "goals": [
      "iron_intake",
      "anemia_prevention",
      "nutrient_density"
    ],
    "helps_with": [
      "anemia_risk"
    ],
    "contraindications": [
      "Use only pasteurized dairy. If lactose intolerant, choose lactose‑free or fortified non‑dairy alternatives."
    ],
    "mechanisms": [
      "education",
      "behavioral_strategy",
      "competence",
      "self_efficacy"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any"
    ],
    "where": [
      "any"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "Pick your iron + vitamin C power couple:",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Spinach + lemon squeeze",
        "Lentils + bell peppers",
        "Fortified cereal + strawberries",
        "Black beans + orange salsa",
        "Chickpeas + tomatoes"
      ],
      "multiple": true
    },
    "difficulty": 1,
    "source": "dietitian_reviewed",
    "areas": [
      "nutrition",
      "organization"
    ]
  },
  {
    "tip_id": "e0870a76-f08e-4907-97f7-926bd3c42487",
    "micro_version": "Open a pack of seaweed snacks and take a bite.",
    "summary": "Munch roasted seaweed snacks for an iodine boost.",
    "details_md": "**The Experiment:** Keep seaweed snack packs as your salty, crunchy go‑to.\n\n**Why it Works:** Seaweed provides iodine to support thyroid and baby’s brain.\n\n**How to Try It:**\n• Rotate with other snacks to avoid overdoing iodine.\n• Check labels; avoid high‑dose kelp supplements.\n• Pair with cooked rice/avocado for a mini 'sushi' bite.",
    "area": "eating",
    "goals": [
      "iodine_intake",
      "healthy_snacking",
      "nutrient_density"
    ],
    "helps_with": [
      "iodine_gap",
      "chip_cravings"
    ],
    "contraindications": [
      "Moderate intake; avoid concentrated kelp supplements without clinician guidance. Choose nori/laver snacks; avoid kelp/kombu due to excessive iodine. If you have thyroid disease or take levothyroxine, discuss consistent iodine intake with your clinician."
    ],
    "mechanisms": [
      "snacks",
      "nutrition_upgrade",
      "craving_substitution",
      "convenience"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any"
    ],
    "where": [
      "home",
      "work"
    ],
    "involves": [
      "seaweed_snacks"
    ],
    "satisfies": [
      "salty_crunchy"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "What flavor seaweed snack are you trying?",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Classic sea salt",
        "Wasabi kick",
        "Sesame",
        "Teriyaki",
        "Olive oil"
      ],
      "multiple": false
    },
    "difficulty": 1,
    "source": "dietitian_reviewed"
  },
  {
    "tip_id": "09517674-539f-425f-9548-94ec13805eff",
    "micro_version": "Add \"Nori Sheets\" to your grocery list.",
    "summary": "Make mini nori hand‑rolls with cooked fillings at home.",
    "details_md": "**The Experiment:** Use seaweed sheets to wrap rice, avocado, and cooked tofu/shrimp/egg.\n\n**Why it Works:** A fun, fast way to combine iodine, healthy fats, and protein—sushi vibes, zero worry.\n\n**How to Try It:**\n• Prep a container of cooked rice for the week.\n• Lay out fillings and roll like a taco.\n• Serve with soy sauce and pickled ginger.",
    "area": "eating",
    "goals": [
      "iodine_intake",
      "DHA_intake",
      "protein_intake",
      "nutrient_density"
    ],
    "helps_with": [
      "cravings",
      "nutrient_gaps"
    ],
    "contraindications": [
      "Ensure fillings are fully cooked and use pasteurized mayo; avoid raw sprouts."
    ],
    "mechanisms": [
      "meal_prep",
      "food_swap",
      "fun",
      "novelty"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any"
    ],
    "where": [
      "home"
    ],
    "involves": [
      "nori",
      "rice",
      "avocado",
      "tofu",
      "shrimp_cooked",
      "egg"
    ],
    "requires": [
      "rice_cooker (optional)",
      "basic_cooking"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "Build your hand roll filling combo:",
    "personalization_type": "multi_text",
    "personalization_config": {
      "items": [
        {
          "label": "Protein:",
          "placeholder": "e.g., Cooked shrimp, scrambled egg, tofu"
        },
        {
          "label": "Veggie/extras:",
          "placeholder": "e.g., Avocado, cucumber, carrot"
        }
      ]
    },
    "difficulty": 1,
    "source": "dietitian_reviewed",
    "areas": [
      "nutrition",
      "organization"
    ]
  },
  {
    "tip_id": "ac1037c0-7e02-4b05-99a3-2b75021e6533",
    "micro_version": "Squeeze an avocado gently to check for ripeness.",
    "summary": "Smash avocado on whole‑grain toast for a folate‑rich breakfast.",
    "details_md": "**The Experiment:** Top a slice of whole‑grain bread with 1/2 avocado, lemon, and salt.\n\n**Why it Works:** Combines natural folate from avocado with fortified folic acid in bread.\n\n**How to Try It:**\n• Add a cooked egg for protein/choline.\n• Sprinkle seeds for extra fiber.\n• Squeeze lemon if tolerated.",
    "area": "eating",
    "goals": [
      "folate_intake",
      "healthy_breakfast",
      "nutrient_density"
    ],
    "helps_with": [
      "folate_gap",
      "morning_energy"
    ],
    "contraindications": [],
    "mechanisms": [
      "simple_swap",
      "nutrition_upgrade",
      "taste",
      "convenience"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any"
    ],
    "where": [
      "home",
      "work"
    ],
    "involves": [
      "avocado",
      "whole_grain_bread",
      "egg"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "Design your avocado toast masterpiece:",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Classic with lemon & salt",
        "+ Fried egg on top",
        "+ Everything bagel seasoning",
        "+ Tomato slices",
        "+ Red pepper flakes",
        "+ Feta crumbles"
      ],
      "multiple": true
    },
    "difficulty": 1,
    "source": "dietitian_reviewed"
  },
  {
    "tip_id": "693e8696-2938-430a-8de1-b80996742e0e",
    "micro_version": "Remove pits from 2-3 dates and set them aside.",
    "summary": "Blend dates into a banana‑peanut butter smoothie (third‑trimester fave).",
    "details_md": "**The Experiment:** Blend milk, banana, 2–3 pitted dates, and peanut butter until creamy.\n\n**Why it Works:** Sneaks in dates for fiber/energy in a dessert‑like smoothie.\n\n**How to Try It:**\n• Use frozen banana for a milkshake vibe.\n• Add cocoa and ice for a 'chocolate shake'.\n• Adjust dates to taste if very sweet.",
    "area": "eating",
    "goals": [
      "healthy_snacking",
      "fiber_intake",
      "labor_prep_habit",
      "nutrient_density"
    ],
    "helps_with": [
      "sweet_cravings",
      "constipation"
    ],
    "contraindications": [
      "Mind total sugars if managing blood sugar; pit dates carefully."
    ],
    "mechanisms": [
      "snacks",
      "nutrition_upgrade",
      "indulgence",
      "craving_substitution"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any"
    ],
    "where": [
      "home"
    ],
    "involves": [
      "dates",
      "banana",
      "peanut_butter",
      "milk"
    ],
    "satisfies": [
      "sweet"
    ],
    "requires": [
      "blender"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "Name your date smoothie creation:",
    "personalization_type": "text",
    "personalization_config": {
      "placeholder": "e.g., Caramel Dream, Energy Blast, Labor Prep Shake"
    },
    "difficulty": 1,
    "source": "dietitian_reviewed"
  },
  {
    "tip_id": "fb3c859b-e936-48d2-b9eb-db3751b6e5bd",
    "micro_version": "Eat one date right now.",
    "summary": "Try 6 dates/day in late pregnancy—make them fun so it sticks.",
    "details_md": "**The Experiment:** Stuff dates with nut butter/cream cheese, or chop into oats/yogurt to reach your daily count.\n\n**Why it Works:** Dates are fiber‑rich and may support a smoother labor; creative prep prevents date fatigue.\n\n**How to Try It:**\n• Freeze stuffed dates for a 'caramel' treat.\n• Add to oatmeal or blend into smoothies.\n• Spread intake across the day.",
    "area": "eating",
    "goals": [
      "fiber_intake",
      "labor_prep_habit",
      "healthy_snacking",
      "nutrient_density"
    ],
    "helps_with": [
      "constipation",
      "sweet_cravings"
    ],
    "contraindications": [
      "High natural sugars; adjust if you have GD—discuss with your clinician. Dates are high in natural sugars—adjust portions if managing gestational diabetes and pair with protein/fat."
    ],
    "mechanisms": [
      "behavioral_strategy",
      "snacks",
      "reward",
      "habit"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any"
    ],
    "where": [
      "home",
      "work"
    ],
    "involves": [
      "dates",
      "nut_butter",
      "yogurt",
      "oatmeal"
    ],
    "satisfies": [
      "sweet"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "How will you make your daily dates fun?",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Stuffed with almond butter",
        "Chopped in oatmeal",
        "Blended in smoothies",
        "Frozen like candy",
        "Mixed in energy balls",
        "Plain but fancy (on a pretty plate!)"
      ],
      "multiple": true
    },
    "difficulty": 1,
    "source": "dietitian_reviewed"
  },
  {
    "tip_id": "a01d8e4c-69ab-478b-a7bf-9af0628b0295",
    "micro_version": "Get a small bag and put 1 apple and 1 protein bar in it.",
    "summary": "Carry a mini snack kit to avoid emergency junk food.",
    "details_md": "**The Experiment:** Pack nuts, dried fruit, whole‑grain crackers, and a shelf‑stable protein (e.g., nut butter packet).\n\n**Why it Works:** Prevents hanger and stabilizes blood sugar between meals.\n\n**How to Try It:**\n• Restock your kit every Sunday.\n• Keep another kit in your car/desk.\n• Add a small water bottle to the kit.",
    "area": "eating",
    "goals": [
      "healthy_snacking",
      "stable_blood_sugar",
      "reduce_ultra_processed",
      "nutrient_density"
    ],
    "helps_with": [
      "random_cravings",
      "energy_crashes",
      "nausea_from_empty_stomach"
    ],
    "contraindications": [
      "Customize for food allergies (nuts, dairy, gluten)."
    ],
    "mechanisms": [
      "environmental_design",
      "planning",
      "convenience",
      "self_efficacy"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any"
    ],
    "where": [
      "any"
    ],
    "requires": [
      "advance_planning"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "Build your emergency snack kit lineup:",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Mixed nuts",
        "Trail mix",
        "Protein bar",
        "Dried fruit",
        "Nut butter packet",
        "Whole grain crackers",
        "Jerky",
        "Dark chocolate square"
      ],
      "multiple": true
    },
    "difficulty": 1,
    "source": "dietitian_reviewed",
    "areas": [
      "nutrition",
      "organization"
    ]
  },
  {
    "tip_id": "d9013b80-59b4-485e-a829-4f6f8437983e",
    "micro_version": "Choose one recipe (like chili) to double this weekend.",
    "summary": "Batch‑cook on good days, freeze portions for tired days.",
    "details_md": "**The Experiment:** Double a recipe (soup, chili, burritos) and freeze half in single servings.\n\n**Why it Works:** Removes decision fatigue and ensures nutritious meals when energy is low.\n\n**How to Try It:**\n• Label with date and contents.\n• Freeze flat in bags for quick thawing.\n• Keep a 'freezer menu' list on the fridge.",
    "area": "eating",
    "goals": [
      "meal_prep",
      "reduce_takeout",
      "nutrient_density",
      "freezer_meal_backup"
    ],
    "helps_with": [
      "low_energy",
      "decision_fatigue",
      "grocery_budget"
    ],
    "contraindications": [
      "Cool foods quickly, store safely, and reheat leftovers to 165°F; eat within 3–4 days once thawed."
    ],
    "mechanisms": [
      "planning",
      "environmental_design",
      "convenience",
      "future_self_reward"
    ],
    "effort": "minimal",
    "time": "5-15",
    "cost": "$",
    "when": [
      "any"
    ],
    "where": [
      "home"
    ],
    "requires": [
      "freezer",
      "labels",
      "storage_bags",
      "basic_cooking",
      "advance_planning"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "What's your freezer meal MVP to batch cook?",
    "personalization_type": "text",
    "personalization_config": {
      "placeholder": "e.g., Chili, soup, burrito bowls, lasagna"
    },
    "difficulty": 1,
    "source": "dietitian_reviewed",
    "areas": [
      "nutrition",
      "organization"
    ]
  },
  {
    "tip_id": "ffc2cd67-ff11-4a61-babc-5987b819452e",
    "micro_version": "Draw a quick checklist with 3 boxes for water on a post-it.",
    "summary": "Gamify nutrition: track daily servings (water, fruit/veg, protein).",
    "details_md": "**The Experiment:** Use a habit app or fridge chart to check off targets each day.\n\n**Why it Works:** Visible progress boosts motivation and consistency without calorie counting.\n\n**How to Try It:**\n• Pick 2–3 targets (e.g., 8 cups water, 5 servings produce).\n• Celebrate streaks; keep goals gentle.\n• Reset weekly if you miss—no guilt.",
    "area": "eating",
    "goals": [
      "increase_hydration",
      "produce_priority",
      "protein_intake",
      "nutrient_density"
    ],
    "helps_with": [
      "habit_tracking",
      "intake_awareness"
    ],
    "contraindications": [],
    "mechanisms": [
      "tracking",
      "behavioral_strategy",
      "gamification",
      "visual_cues"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any"
    ],
    "where": [
      "any"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "Pick your 3 daily nutrition goals to track:",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "8 cups water",
        "5 servings veggies",
        "3 servings protein",
        "2 servings fruit",
        "1 probiotic food",
        "No sugary drinks",
        "Vitamins taken"
      ],
      "multiple": true
    },
    "difficulty": 1,
    "source": "dietitian_reviewed",
    "areas": [
      "nutrition",
      "organization"
    ]
  },
  {
    "tip_id": "bb116996-3631-47df-a068-d23bce5b0ca9",
    "micro_version": "Add \"Frozen Broccoli\" and \"Bagged Salad\" to your list.",
    "summary": "Buy pre‑cut or frozen produce to remove prep barriers.",
    "details_md": "**The Experiment:** Stock baby carrots, bagged salad, and frozen veg/fruit for instant sides and smoothies.\n\n**Why it Works:** When the healthy choice is the easy choice, you’ll choose it more often.\n\n**How to Try It:**\n• Place ready‑to‑eat produce at eye level in the fridge.\n• Keep frozen veg for quick steam‑in‑bag sides.\n• Wash grapes/berries as soon as you get home.",
    "area": "eating",
    "goals": [
      "produce_priority",
      "fiber_intake",
      "nutrient_density"
    ],
    "helps_with": [
      "constipation",
      "snacking",
      "time_pressure"
    ],
    "contraindications": [],
    "mechanisms": [
      "environmental_design",
      "simple_swap",
      "convenience",
      "visual_cues"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any"
    ],
    "where": [
      "home"
    ],
    "requires": [
      "microwave (optional)"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "What pre-cut or frozen produce will you stock?",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Baby carrots",
        "Bagged salad",
        "Frozen berries",
        "Steam-in-bag veggies",
        "Pre-cut melon",
        "Frozen smoothie packs",
        "Sliced bell peppers"
      ],
      "multiple": true
    },
    "difficulty": 1,
    "source": "dietitian_reviewed"
  },
  {
    "tip_id": "bb59679d-d891-4526-9a09-7e5d2077b3cf",
    "micro_version": "Send a food pic to a friend with the caption \"Healthy lunch!\".",
    "summary": "Team up with a friend or group to share meal pics and wins.",
    "details_md": "**The Experiment:** Start a group chat to post dinner photos and hydration check‑ins.\n\n**Why it Works:** Social accountability and ideas from peers make habits stick.\n\n**How to Try It:**\n• Pick a theme challenge (e.g., 'eat the rainbow').\n• Cheer small wins; no food policing.\n• Share 1 recipe each week.",
    "area": "eating",
    "goals": [
      "produce_priority",
      "nutrient_density",
      "meal_tracking_accountability",
      "produce_sharing_circle"
    ],
    "helps_with": [
      "social_accountability",
      "produce_variety"
    ],
    "contraindications": [],
    "mechanisms": [
      "social_support",
      "accountability",
      "community",
      "encouragement"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any"
    ],
    "where": [
      "any"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "Name your nutrition support squad:",
    "personalization_type": "text",
    "personalization_config": {
      "placeholder": "e.g., The Veggie Victors, Hydration Heroes, Snack Squad"
    },
    "difficulty": 1,
    "source": "dietitian_reviewed"
  },
  {
    "tip_id": "2540ea74-28a4-46da-a5c4-ca690b81d649",
    "micro_version": "Open your phone notes and type \"Lunch: [Food], Feeling: [Good/Bad]\".",
    "summary": "Keep a 7‑day food + symptom journal to spot your triggers and wins.",
    "details_md": "**The Experiment:** Note what you ate and how you felt (nausea, heartburn, energy) 1–3 hours later.\n\n**Why it Works:** Patterns reveal which foods help or hinder so you can personalize your plan.\n\n**How to Try It:**\n• Keep entries short—bullets or photos are fine.\n• Mark heartburn nights and what you ate before.\n• Use findings to make simple swaps next week.",
    "area": "eating",
    "goals": [
      "heartburn_relief",
      "reduce_nausea",
      "nutrient_density",
      "symptom_journaling"
    ],
    "helps_with": [
      "reflux_triggers",
      "nausea_triggers",
      "energy_management"
    ],
    "contraindications": [],
    "mechanisms": [
      "mindfulness_practice",
      "tracking",
      "self_awareness",
      "insight"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any"
    ],
    "where": [
      "any"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "How will you track your food detective work?",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Quick notes on phone",
        "Physical journal",
        "Photo log",
        "Voice memos",
        "Tracking app"
      ],
      "multiple": false
    },
    "difficulty": 1,
    "source": "dietitian_reviewed",
    "areas": [
      "nutrition",
      "organization"
    ]
  },
  {
    "tip_id": "9f34f055-f5d0-434a-94c4-7127415321f6",
    "micro_version": "Decide on one swap: \"If I want chips, I'll have popcorn.\"",
    "summary": "Find a healthier 'doppelganger' for your strongest craving.",
    "details_md": "**The Experiment:** Match the texture/temp/flavor (e.g., 'nice cream' for ice cream, popcorn for chips).\n\n**Why it Works:** Satisfies the sensory need while upgrading nutrition.\n\n**How to Try It:**\n• Identify the craving core (cold/sweet, salty/crunchy).\n• Test 2–3 alternatives and rate satisfaction.\n• Keep your favorite swap stocked.",
    "area": "eating",
    "goals": [
      "craving_management",
      "less_processed_food",
      "nutrient_density"
    ],
    "helps_with": [
      "sweet_cravings",
      "salty_cravings",
      "binges"
    ],
    "contraindications": [
      "Limit sugar alcohols if they cause bloating/diarrhea; choose caffeine‑free swaps in the afternoon/evening."
    ],
    "mechanisms": [
      "food_swap",
      "behavioral_strategy",
      "craving_substitution",
      "autonomy"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any"
    ],
    "where": [
      "home",
      "work"
    ],
    "satisfies": [
      "varies"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "What craving will you find a healthy twin for?",
    "personalization_type": "multi_text",
    "personalization_config": {
      "items": [
        {
          "label": "Craving:",
          "placeholder": "e.g., Chocolate chip cookies"
        },
        {
          "label": "Healthy twin:",
          "placeholder": "e.g., Dark chocolate oat bites"
        }
      ]
    },
    "difficulty": 1,
    "source": "dietitian_reviewed"
  },
  {
    "tip_id": "2a5cdbed-50a5-48ab-8ffe-253cb369d7bd",
    "micro_version": "Eat 3 walnuts right now.",
    "summary": "Boost omega‑3s with ground flax/chia or omega‑3 eggs if you skip fish.",
    "details_md": "**The Experiment:** Add 1 Tbsp flax/chia to breakfast and choose omega‑3 eggs for scrambles.\n\n**Why it Works:** Covers omega‑3 needs via plants and fortified foods when fish isn’t appealing.\n\n**How to Try It:**\n• Grind flax for better absorption; store cold.\n• Use omega‑3 eggs in fully cooked dishes.\n• Ask your provider about algae‑based DHA if needed.",
    "area": "eating",
    "goals": [
      "DHA_intake",
      "omega3_intake",
      "choline_intake",
      "nutrient_density"
    ],
    "helps_with": [
      "DHA_gap",
      "choline_gap"
    ],
    "contraindications": [
      "If using supplements (e.g., algae‑based DHA), select third‑party tested brands and confirm dose with your clinician."
    ],
    "mechanisms": [
      "nutrition_upgrade",
      "simple_swap",
      "stealth",
      "convenience"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any"
    ],
    "where": [
      "home"
    ],
    "involves": [
      "flaxseed",
      "chia",
      "omega3_eggs"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "How will you get your omega-3s today?",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Ground flax in oatmeal",
        "Chia in smoothie",
        "Omega-3 eggs",
        "Walnuts snack",
        "Hemp hearts on yogurt"
      ],
      "multiple": true
    },
    "difficulty": 1,
    "source": "dietitian_reviewed",
    "areas": [
      "nutrition",
      "organization"
    ]
  },
  {
    "tip_id": "c8e87457-4618-4149-945b-204d8a9cf126",
    "micro_version": "Write down one meal you know you can always tolerate.",
    "summary": "Find a healthy 'safe meal' you love—and repeat it.",
    "details_md": "**The Experiment:** Pick one balanced meal that always sounds good and put it on repeat this week.\n\n**Why it Works:** Consistency beats perfection; repeating a well‑tolerated meal ensures steady nutrients.\n\n**How to Try It:**\n• Example: veggie omelet + toast; or turkey‑cheese melt (heated) + salad.\n• Batch prep ingredients to reduce effort.\n• Rotate sides/sauces to prevent boredom.",
    "area": "eating",
    "goals": [
      "reduce_nausea",
      "nutrient_density",
      "nutrient_density",
      "safe_meal_routine"
    ],
    "helps_with": [
      "decision_fatigue",
      "aversion",
      "low_energy"
    ],
    "contraindications": [
      "Ensure meats/eggs are fully cooked and cheeses are pasteurized; vary sides to cover micronutrients over the week."
    ],
    "mechanisms": [
      "mindset_reframe",
      "planning",
      "simplicity",
      "relief"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any"
    ],
    "where": [
      "home",
      "work"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "What's your go-to safe meal that always sounds good?",
    "personalization_type": "text",
    "personalization_config": {
      "placeholder": "e.g., Veggie omelet + toast, Turkey avocado wrap, Greek yogurt parfait"
    },
    "difficulty": 1,
    "source": "dietitian_reviewed",
    "areas": [
      "nutrition",
      "organization"
    ]
  },
  {
    "tip_id": "c3d4e5f6-f003-4003-b003-34567890abcd",
    "micro_version": "Pick up your fork with your left hand (or right, if left-handed) to practice.",
    "summary": "Eat with your non-dominant hand to slow down.",
    "details_md": "**The Experiment:** At one meal today, try eating with your non-dominant hand.[4]\n\n**Why it Works:** This simple mechanical trick disrupts the automatic, mindless process of eating. The awkwardness forces you to slow down, pay more attention to each bite, and become more aware of your body's fullness cues.[4]\n\n**How to Try It:**\n• Simply switch your fork or spoon to the hand you don't normally use.\n• This works best with meals that require utensils.\n• Notice how it changes the pace and experience of your meal.",
    "area": "eating",
    "goals": [
      "portion_control",
      "mindful_eating"
    ],
    "helps_with": [
      "mindless_eating",
      "overeating"
    ],
    "contraindications": [],
    "mechanisms": [
      "behavioral_strategy",
      "mindfulness_practice",
      "self_awareness"
    ],
    "effort": "low",
    "time": "5-15",
    "cost": "$",
    "when": [
      "any",
      "meal_time"
    ],
    "where": [
      "home",
      "work",
      "restaurant"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "Which meal will you try eating like a beginner (with your non-dominant hand)?",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Breakfast",
        "Lunch",
        "Dinner",
        "Snack time"
      ],
      "multiple": false
    },
    "difficulty": 1,
    "source": "dietitian_reviewed"
  },
  {
    "tip_id": "d4e5f6g7-f004-4004-b004-4567890abcde",
    "micro_version": "Move the fruit bowl from the fridge to the center of the kitchen table.",
    "summary": "Make healthy food visible and junk food invisible.",
    "details_md": "**The Experiment:** Take 5 minutes to rearrange your kitchen counter. Place a bowl of fresh, ready-to-eat fruit in a prominent spot. Move any cookies, chips, or candy into an opaque container and put it on a high shelf or in the back of a cupboard.\n\n**Why it Works:** This is a classic choice architecture strategy. We are wired to eat what is most visible and convenient. By making the healthy choice the easy choice, you put good habits on autopilot.\n\n**How to Try It:**\n• Use a beautiful bowl for your fruit to make it more appealing.\n• The goal is \"out of sight, out of mind\" for less healthy options.[7, 8]\n• Apply the same logic to your fridge: put healthy snacks at eye level.",
    "area": "eating",
    "goals": [
      "weight_loss",
      "healthy_snacking",
      "less_processed_food"
    ],
    "helps_with": [
      "mindless_eating",
      "sugar_addiction",
      "junk_food_addiction"
    ],
    "contraindications": [],
    "mechanisms": [
      "environmental_design",
      "convenience",
      "visual_cues"
    ],
    "effort": "low",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any",
      "at_home"
    ],
    "where": [
      "home"
    ],
    "involves": [
      "junk_food",
      "fruit"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "What healthy food will you showcase and what will you hide?",
    "personalization_type": "multi_text",
    "personalization_config": {
      "items": [
        {
          "label": "Showcase on counter:",
          "placeholder": "e.g., Fresh fruit bowl, veggie sticks"
        },
        {
          "label": "Hide away:",
          "placeholder": "e.g., Cookies, chips, candy"
        }
      ]
    },
    "difficulty": 1,
    "source": "dietitian_reviewed",
    "areas": [
      "nutrition",
      "organization"
    ]
  },
  {
    "tip_id": "e5f6g7h8-f005-4005-b005-567890abcdef",
    "micro_version": "Go to the cabinet and pull out a salad plate for your next meal.",
    "summary": "Use smaller plates to trick your brain into feeling full.",
    "details_md": "**The Experiment:** For your next meal, serve your food on a smaller plate (like a salad plate instead of a large dinner plate).[2]\n\n**Why it Works:** It's a powerful visual illusion. A standard portion of food looks more abundant on a smaller plate, which can increase feelings of psychological satisfaction and fullness.[2, 9] This can help you naturally reduce your portion sizes without feeling deprived.\n\n**How to Try It:**\n• Use a 9-inch plate instead of a 12-inch one.[2]\n• You can also try using smaller bowls and even smaller utensils, like appetizer forks, to further slow down your eating.[2, 3]",
    "area": "eating",
    "goals": [
      "weight_loss",
      "portion_control",
      "mindful_eating"
    ],
    "helps_with": [
      "overeating"
    ],
    "contraindications": [],
    "mechanisms": [
      "environmental_design",
      "behavioral_strategy",
      "visual_cues",
      "satiety"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any",
      "meal_time"
    ],
    "where": [
      "home",
      "work"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "Which meal will you try the smaller plate trick with?",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Breakfast",
        "Lunch",
        "Dinner",
        "All meals today"
      ],
      "multiple": false
    },
    "difficulty": 1,
    "source": "dietitian_reviewed"
  },
  {
    "tip_id": "f6g7h8i9-f006-4006-b006-67890abcdef0",
    "micro_version": "Unlock your phone, then immediately take a sip of water.",
    "summary": "Link drinking water to checking your phone.",
    "details_md": "**The Experiment:** For one day, make this your rule: **Every time you check your phone, take a sip of water.**[10]\n\n**Why it Works:** This is a perfect example of habit stacking. It links a new, desired behavior (drinking water) to an existing, frequent habit (checking your phone), making it almost automatic.[10, 11] It's an effortless way to stay hydrated, as thirst is often mistaken for hunger.[3]\n\n**How to Try It:**\n• Keep a water bottle on your desk, in your bag, or wherever you spend the most time with your phone.\n• The cue is the action of picking up or looking at your phone.[10]\n• Don't worry about big gulps; small, consistent sips add up.",
    "area": "eating",
    "goals": [
      "increase_hydration",
      "reduce_cravings",
      "improve_energy"
    ],
    "helps_with": [
      "mindless_eating",
      "low_energy",
      "dehydration"
    ],
    "contraindications": [],
    "mechanisms": [
      "habit_stacking",
      "convenience",
      "repetition"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any",
      "using_phone"
    ],
    "where": [
      "any"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "What kind of phone checking activity will you pair with water?",
    "personalization_type": "text",
    "personalization_config": {
      "placeholder": "e.g., Checking social media, texts, email, or notifications"
    },
    "difficulty": 1,
    "source": "dietitian_reviewed"
  },
  {
    "tip_id": "g7h8i9j0-f007-4007-b007-7890abcdef01",
    "micro_version": "Decide one item (e.g., spinach, seeds) to add to tonight's dinner.",
    "summary": "Add one healthy ingredient to every meal.",
    "details_md": "**The Experiment:** For every meal you plate today, add one extra nutrient-rich ingredient.[10]\n\n**Why it Works:** This is a low-friction way to boost the nutritional value of foods you already eat. It focuses on addition, not restriction, which feels positive and is easier to sustain. Over time, these small additions compound into significant health benefits.[10]\n\n**How to Try It:**\n• Toss a handful of spinach into scrambled eggs or pasta sauce.[10]\n• Mix a tablespoon of chia seeds or flax seeds into yogurt or oatmeal.[2, 6]\n• Add extra sliced veggies (peppers, cucumbers, tomatoes) to a sandwich or wrap.[10]",
    "area": "eating",
    "goals": [
      "increase_nutrients",
      "increase_fiber",
      "produce_priority"
    ],
    "helps_with": [
      "nutrient_deficiency"
    ],
    "contraindications": [],
    "mechanisms": [
      "habit_stacking",
      "healthy_swap",
      "gamification",
      "positive_reinforcement"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any",
      "meal_time"
    ],
    "where": [
      "home",
      "work"
    ],
    "features": [
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "Pick your 'power-up' ingredients for today's meals:",
    "personalization_type": "multi_text",
    "personalization_config": {
      "items": [
        {
          "label": "Breakfast booster:",
          "placeholder": "e.g., Chia seeds, berries, spinach"
        },
        {
          "label": "Lunch upgrade:",
          "placeholder": "e.g., Avocado, nuts, extra veggies"
        },
        {
          "label": "Dinner enhancer:",
          "placeholder": "e.g., Hemp hearts, mushrooms, herbs"
        }
      ]
    },
    "difficulty": 1,
    "source": "dietitian_reviewed"
  },
  {
    "tip_id": "h8i9j0k1-f008-4008-b008-890abcdef012",
    "micro_version": "Locate a clean spray bottle or buy one.",
    "summary": "Use a spritz of water or vinegar to season popcorn.",
    "details_md": "**The Experiment:** Make a bowl of plain, air-popped popcorn. Instead of butter, lightly spritz it with water or plain vinegar before adding seasonings.[6]\n\n**Why it Works:** This clever hack allows salt-free seasonings (like garlic powder, chili powder, or cinnamon) to stick to the popcorn without adding the fat and calories of butter or oil. The vinegar spritz creates a salt-and-vinegar flavor without the high-sodium seasoning packet.[6]\n\n**How to Try It:**\n• Use a small, clean spray bottle for the water or vinegar.[6]\n• Spritz lightly, toss the popcorn, and then sprinkle on your seasoning.\n• This makes a high-fiber, whole-grain snack that satisfies a crunchy craving.[12, 13]",
    "area": "eating",
    "goals": [
      "reduce_fat",
      "reduce_sodium",
      "healthy_snacking"
    ],
    "helps_with": [
      "mindless_eating"
    ],
    "contraindications": [],
    "mechanisms": [
      "food_hack",
      "healthy_swap",
      "sensory",
      "crave_buster"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "afternoon",
      "evening",
      "snack_time",
      "craving_event"
    ],
    "where": [
      "home"
    ],
    "involves": [
      "popcorn"
    ],
    "satisfies": [
      "crunchy",
      "salty"
    ],
    "requires": [
      "air_popper",
      "microwave"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "Create your popcorn flavor adventure:",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Garlic & Herb",
        "Spicy Chili",
        "Cinnamon Sweet",
        "Ranch Style",
        "Nutritional Yeast Cheesy",
        "Everything Bagel"
      ],
      "multiple": true
    },
    "difficulty": 1,
    "source": "dietitian_reviewed"
  },
  {
    "tip_id": "i9j0k1l2-f009-4009-b009-90abcdef0123",
    "micro_version": "Peel a banana and put it in the freezer.",
    "summary": "Make creamy 'ice cream' from frozen bananas.",
    "details_md": "**The Experiment:** When you crave ice cream, blend one or two frozen, sliced bananas in a blender or food processor until it reaches a smooth, soft-serve consistency.[12, 14]\n\n**Why it Works:** This swap delivers the cold, creamy texture of ice cream using a whole fruit base. It's naturally sweet and provides fiber and potassium without the high levels of added sugar and saturated fat found in traditional ice cream.[12, 14]\n\n**How to Try It:**\n• Peel and slice ripe bananas before freezing them on a baking sheet.[12]\n• You may need to add a splash of milk or stop and scrape down the sides of the blender a few times.\n• Get creative by adding a spoonful of cocoa powder for chocolate 'nice cream' or other frozen fruits like berries.[2]",
    "area": "eating",
    "goals": [
      "weight_loss",
      "reduce_sugar",
      "reduce_fat",
      "less_processed_food"
    ],
    "helps_with": [
      "sugar_addiction"
    ],
    "contraindications": [],
    "mechanisms": [
      "healthy_swap",
      "crave_buster",
      "sensory"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "evening",
      "late_night",
      "snack_time",
      "craving_event"
    ],
    "where": [
      "home"
    ],
    "involves": [
      "ice_cream"
    ],
    "satisfies": [
      "sweet",
      "creamy",
      "cold"
    ],
    "requires": [
      "blender",
      "freezer",
      "basic_cooking",
      "advance_planning"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "What flavor will your nice cream be today?",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Classic vanilla",
        "Chocolate (+ cocoa)",
        "Peanut butter swirl",
        "Berry blast",
        "Mint chip",
        "Caramel date"
      ],
      "multiple": false
    },
    "difficulty": 2,
    "source": "dietitian_reviewed",
    "hooks": [
      {
        "hook": "I can't believe it's not ice cream",
        "subtitle": "The frozen banana trick that actually works",
        "detail": "When you blend frozen bananas, something magical happens—they transform into a creamy, soft-serve texture that genuinely mimics ice cream. You get the cold, sweet, creamy experience with fiber and potassium instead of added sugar and fat.",
        "action": "Freeze sliced ripe bananas tonight. Tomorrow when you want ice cream, blend them until smooth. Add cocoa powder for chocolate 'nice cream.'"
      },
      {
        "hook": "One ingredient. Ice cream texture.",
        "subtitle": "How frozen fruit tricks your brain",
        "detail": "Your brain craves ice cream for the cold, creamy, sweet sensation—not specifically for dairy fat and added sugar. Frozen bananas deliver all three sensory experiences while being naturally sweet and full of fiber.",
        "action": "Peel and slice two ripe bananas, freeze on a baking sheet, then blend in a food processor until creamy. Takes 2-3 minutes."
      },
      {
        "hook": "The dessert that's actually just fruit",
        "subtitle": "Why 'nice cream' satisfies ice cream cravings",
        "detail": "Traditional ice cream combines fat and sugar to hyper-activate reward pathways. Banana 'nice cream' gives you the sensory satisfaction—cold, sweet, creamy—without the blood sugar spike or the calories. Kids love it too.",
        "action": "Try your first batch plain, then experiment: cocoa powder, peanut butter, frozen berries, or a splash of vanilla extract."
      }
    ]
  },
  {
    "tip_id": "j0k1l2m3-f010-4010-b010-0abcdef01234",
    "micro_version": "Decide your rule: \"I will only order the 4-piece nugget.\"",
    "summary": "Satisfy fast-food cravings with a smaller portion.",
    "details_md": "**The Experiment:** The next time you have an undeniable craving for a specific fast-food meal, go get it—but order a smaller version. For example, get a 6-piece nugget and small fries instead of a large combo meal.[15]\n\n**Why it Works:** This strategy of mindful indulgence acknowledges that complete restriction often backfires, leading to binges.[15, 16] By satisfying the specific craving with a sensible portion, you avoid feelings of deprivation and the \"all-or-nothing\" cycle, making it a sustainable long-term approach.[15]\n\n**How to Try It:**\n• Plan the indulgence in advance to make it a conscious choice, not an impulse.\n• Skip the sugary soda to save a significant amount of calories and sugar.[17]\n• Savor the smaller portion mindfully, without distractions, to maximize enjoyment.[18]",
    "area": "eating",
    "goals": [
      "weight_loss",
      "portion_control",
      "mindful_eating",
      "fast_food_portion_cap"
    ],
    "helps_with": [
      "binge_eating",
      "diet_trauma"
    ],
    "contraindications": [],
    "mechanisms": [
      "behavioral_strategy",
      "crave_buster",
      "psychological_relief"
    ],
    "effort": "low",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any",
      "craving_event",
      "eating_out"
    ],
    "where": [
      "restaurant"
    ],
    "involves": [
      "fast_food"
    ],
    "satisfies": [
      "salty",
      "savory",
      "fatty"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "Which fast food item and what size will you get?",
    "personalization_type": "multi_text",
    "personalization_config": {
      "items": [
        {
          "label": "Fast food item:",
          "placeholder": "e.g., Chicken nuggets"
        },
        {
          "label": "Size:",
          "placeholder": "e.g., 6-piece instead of 10-piece"
        }
      ]
    },
    "difficulty": 2,
    "source": "dietitian_reviewed",
    "areas": [
      "nutrition",
      "organization"
    ]
  },
  {
    "tip_id": "k1l2m3n4-f011-4011-b011-abcdef012345",
    "micro_version": "Put 3 slices of pepperoni and cheese in a mug.",
    "summary": "Get a quick pizza fix with this low-carb bowl.",
    "details_md": "**The Experiment:** For a super-fast, savory craving-buster, put some pepperoni, a spoonful of tomato sauce, and shredded mozzarella cheese in a microwave-safe bowl. Microwave until melty and gooey.[19]\n\n**Why it Works:** This hack isolates the most craved flavors of pizza (the savory meat, tangy sauce, and melted cheese) without the high-carb crust. It's a quick, portion-controlled way to satisfy a specific craving in under two minutes.[19]\n\n**How to Try It:**\n• Use a few slices of pepperoni, about a tablespoon of sauce, and a sprinkle of cheese.[19]\n• You can add other pizza toppings you have on hand, like sliced olives or mushrooms.\n• Eat with a fork to slow down and savor the flavors.",
    "area": "eating",
    "goals": [
      "reduce_carbs",
      "portion_control",
      "reduce_cravings"
    ],
    "helps_with": [
      "junk_food_addiction"
    ],
    "contraindications": [],
    "mechanisms": [
      "food_hack",
      "crave_buster",
      "convenience",
      "sensory"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any",
      "craving_event",
      "snack_time"
    ],
    "where": [
      "home",
      "work"
    ],
    "involves": [
      "pizza"
    ],
    "satisfies": [
      "savory",
      "cheesy",
      "salty"
    ],
    "requires": [
      "microwave"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "Build your pizza bowl toppings:",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Pepperoni",
        "Mushrooms",
        "Olives",
        "Bell peppers",
        "Italian sausage",
        "Extra cheese"
      ],
      "multiple": true
    },
    "difficulty": 1,
    "source": "dietitian_reviewed"
  },
  {
    "tip_id": "l2m3n4o5-f012-4012-b012-bcdef0123456",
    "micro_version": "Slice an apple and get the peanut butter jar.",
    "summary": "Turn a simple apple into a fun, decadent-tasting snack.",
    "details_md": "**The Experiment:** Slice an apple and arrange the slices on a plate. Drizzle with a spoonful of melted peanut or almond butter and sprinkle with a few dark chocolate chips.[20]\n\n**Why it Works:** This snack hits multiple craving points—sweet, salty, and crunchy—while being based on a whole fruit. It feels indulgent but provides fiber from the apple and healthy fats and protein from the nut butter, making it far more satisfying and nutritious than a candy bar.[20, 14]\n\n**How to Try It:**\n• Use an apple corer/slicer to make it even faster.[20, 21]\n• Warm the nut butter in the microwave for 10-15 seconds to make it easy to drizzle.\n• Get creative with toppings like a sprinkle of cinnamon, granola, or shredded coconut.[20]",
    "area": "eating",
    "goals": [
      "healthy_snacking",
      "increase_fiber",
      "reduce_sugar"
    ],
    "helps_with": [
      "sugar_addiction"
    ],
    "contraindications": [],
    "mechanisms": [
      "food_hack",
      "healthy_swap",
      "sensory",
      "gamification"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "afternoon",
      "evening",
      "snack_time",
      "craving_event"
    ],
    "where": [
      "home",
      "work"
    ],
    "involves": [
      "candy",
      "cookies"
    ],
    "satisfies": [
      "sweet",
      "crunchy",
      "salty"
    ],
    "requires": [
      "microwave"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "Design your apple snack masterpiece:",
    "personalization_type": "multi_text",
    "personalization_config": {
      "items": [
        {
          "label": "Apple type:",
          "placeholder": "e.g., Honeycrisp, Granny Smith, Gala"
        },
        {
          "label": "Toppings:",
          "placeholder": "e.g., PB drizzle + chocolate chips + cinnamon"
        }
      ]
    },
    "difficulty": 1,
    "source": "dietitian_reviewed"
  },
  {
    "tip_id": "m3n4o5p6-f013-4013-b013-cdef01234567",
    "micro_version": "Add rotisserie chicken to your grocery list.",
    "summary": "Use a rotisserie chicken for fast, healthy protein all week.",
    "details_md": "**The Experiment:** Buy one rotisserie chicken during your weekly grocery shop. As soon as you get home, pull the meat from the bones and store it in the fridge.[22]\n\n**Why it Works:** This is a meal prep game-changer for busy people. It provides a ready-to-use, versatile source of lean protein, eliminating the time and effort of cooking chicken during the week. This makes assembling healthy meals incredibly fast.[23, 22]\n\n**How to Try It:**\n• Add the shredded chicken to bagged salads for a quick lunch.[5]\n• Use it as a filling for wraps or quesadillas with whole-wheat tortillas.[19, 23]\n• Mix it with Greek yogurt or avocado for a healthier chicken salad.[24, 25]",
    "area": "eating",
    "goals": [
      "increase_protein",
      "meal_prep",
      "protein_shortcuts"
    ],
    "helps_with": [
      "decision_fatigue"
    ],
    "contraindications": [],
    "mechanisms": [
      "food_hack",
      "meal_prep_strategy",
      "convenience",
      "efficiency"
    ],
    "effort": "low",
    "time": "5-15",
    "cost": "$",
    "when": [
      "any",
      "meal_prep_time"
    ],
    "where": [
      "home"
    ],
    "requires": [
      "advance_planning"
    ],
    "features": [
      "solo_friendly",
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "diet_trauma_safe",
      "not_diety"
    ],
    "personalization_prompt": "What will you make with your rotisserie chicken this week?",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Chicken salad",
        "Wraps/sandwiches",
        "Quesadillas",
        "Soup",
        "Grain bowls",
        "Stir-fry"
      ],
      "multiple": true
    },
    "difficulty": 1,
    "source": "dietitian_reviewed",
    "areas": [
      "nutrition",
      "organization"
    ]
  },
  {
    "tip_id": "b1c2d3e4-0011-4011-a011-234567890ab1",
    "micro_version": "Put eggs or Greek yogurt on the counter for breakfast.",
    "summary": "Boost your breakfast with protein.",
    "details_md": "**The Experiment:** Start your day with a protein-rich breakfast, aiming for 15-25 grams.[1]\n\n**Why it Works:** Protein is the most satiating macronutrient. It digests slowly and suppresses hunger hormones, which helps you feel full longer and can reduce cravings later in the day.[1, 2] Your body also burns more calories digesting protein compared to carbs and fats.\n\n**How to Try It:**\n• Swap a bagel or cereal for two eggs with whole-wheat toast and avocado.[1]\n• Add a scoop of protein powder or a tablespoon of peanut butter to your oatmeal.[3]\n• Top high-protein frozen waffles with nuts and berries.[1]",
    "area": "eating",
    "goals": [
      "weight_loss",
      "increase_protein",
      "reduce_cravings",
      "improve_energy"
    ],
    "helps_with": [
      "mindless_eating",
      "sugar_addiction"
    ],
    "contraindications": [],
    "mechanisms": [
      "healthy_swap",
      "habit_stacking",
      "satiety",
      "energy_boost"
    ],
    "effort": "low",
    "time": "5-15",
    "cost": "$",
    "when": [
      "morning",
      "meal_time"
    ],
    "where": [
      "home"
    ],
    "involves": [
      "cereal",
      "bagels",
      "toast"
    ],
    "requires": [
      "stove_top",
      "microwave",
      "basic_cooking",
      "advance_planning"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "Build your protein breakfast power-up:",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Add Greek yogurt",
        "Mix in protein powder",
        "Top with nuts/seeds",
        "Add eggs",
        "Include cottage cheese",
        "Sprinkle hemp hearts"
      ],
      "multiple": true
    },
    "difficulty": 2,
    "source": "dietitian_reviewed"
  },
  {
    "tip_id": "c2d3e4f5-0012-4012-a012-34567890ab12",
    "micro_version": "Check if you have Greek yogurt or avocado in the fridge.",
    "summary": "Swap mayonnaise for Greek yogurt or mashed avocado.",
    "details_md": "**The Experiment:** For one day, replace the mayonnaise in your sandwiches, salads (like tuna or chicken), or spreads with either plain nonfat Greek yogurt or mashed avocado.\n\n**Why it Works:** This swap significantly cuts saturated fat and calories. Greek yogurt provides a major protein boost, increasing satiety.[4] Mashed avocado adds healthy monounsaturated fats and fiber, which also help you feel full.[5]\n\n**How to Try It:**\n• Use a 1:1 ratio when swapping mayo for plain Greek yogurt in a recipe like tuna salad.[4]\n• For a sandwich, simply spread a quarter of a mashed avocado instead of mayo.\n• Add a squeeze of lemon or lime to the avocado to prevent browning.",
    "area": "eating",
    "goals": [
      "weight_loss",
      "reduce_fat",
      "increase_protein",
      "less_processed_food"
    ],
    "contraindications": [],
    "mechanisms": [
      "healthy_swap",
      "sensory"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "morning",
      "afternoon",
      "meal_time",
      "snack_time"
    ],
    "where": [
      "home",
      "work"
    ],
    "involves": [
      "mayonnaise"
    ],
    "preserves": [
      "tuna_salad",
      "chicken_salad",
      "sandwiches"
    ],
    "satisfies": [
      "creamy"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "Which mayo swap will you try in what dish?",
    "personalization_type": "multi_text",
    "personalization_config": {
      "items": [
        {
          "label": "Swap choice:",
          "placeholder": "e.g., Greek yogurt or avocado"
        },
        {
          "label": "In what dish:",
          "placeholder": "e.g., tuna salad, sandwich"
        }
      ]
    },
    "difficulty": 1,
    "source": "dietitian_reviewed",
    "helps_with": []
  },
  {
    "tip_id": "d3e4f5g6-0013-4013-a013-4567890ab123",
    "micro_version": "Swap sour cream for Greek yogurt at your next meal.",
    "summary": "Swap sour cream for plain Greek yogurt or blended cottage cheese.",
    "details_md": "**The Experiment:** Instead of using sour cream as a topping (on tacos, chili, or baked potatoes) or in a dip, use plain nonfat Greek yogurt or blended low-fat cottage cheese.\n\n**Why it Works:** This is a high-impact swap that dramatically increases protein while cutting fat and calories. Blended cottage cheese has a neutral, creamy flavor, while Greek yogurt offers a similar tangy taste to sour cream. Both options increase satiety.[4, 6]\n\n**How to Try It:**\n• Use a 1:1 ratio of plain Greek yogurt for sour cream.\n• To make a creamy base, blend low-fat cottage cheese in a food processor or blender until completely smooth. You can add ranch seasoning for a high-protein veggie dip.[6]",
    "area": "eating",
    "goals": [
      "weight_loss",
      "reduce_fat",
      "increase_protein"
    ],
    "contraindications": [],
    "mechanisms": [
      "healthy_swap",
      "crave_buster",
      "sensory"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "afternoon",
      "evening",
      "meal_time"
    ],
    "where": [
      "home",
      "restaurant"
    ],
    "involves": [
      "sour_cream"
    ],
    "preserves": [
      "tacos",
      "chili",
      "baked_potato",
      "dips"
    ],
    "satisfies": [
      "creamy",
      "savory"
    ],
    "requires": [
      "blender"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "Pick your creamy topping upgrade:",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Greek yogurt on tacos",
        "Blended cottage cheese for dip",
        "Greek yogurt on baked potato",
        "Cottage cheese on chili"
      ],
      "multiple": false
    },
    "difficulty": 1,
    "source": "dietitian_reviewed",
    "helps_with": []
  },
  {
    "tip_id": "e4f5g6h7-0014-4014-a014-567890ab1234",
    "micro_version": "Pop some popcorn kernels or buy air-popped popcorn.",
    "summary": "Swap potato chips for air-popped popcorn.",
    "details_md": "**The Experiment:** When you're craving a crunchy, salty snack, reach for air-popped popcorn instead of potato chips.\n\n**Why it Works:** Popcorn is a whole grain, providing significantly more fiber and volume for far fewer calories than chips. This satisfies the desire for a crunchy snack while helping you feel full and avoiding the high fat content of fried chips.[7, 8]\n\n**How to Try It:**\n• Use an air popper or the paper bag method in the microwave to pop plain kernels.\n• Control the salt yourself and experiment with other seasonings like chili powder, garlic powder, or nutritional yeast for a cheesy flavor.[7]",
    "area": "eating",
    "goals": [
      "weight_loss",
      "reduce_fat",
      "increase_fiber",
      "less_processed_food"
    ],
    "helps_with": [
      "mindless_eating"
    ],
    "contraindications": [],
    "mechanisms": [
      "healthy_swap",
      "crave_buster",
      "sensory"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "afternoon",
      "evening",
      "late_night",
      "snack_time",
      "boredom",
      "craving_event"
    ],
    "where": [
      "home",
      "work"
    ],
    "involves": [
      "potato_chips"
    ],
    "satisfies": [
      "crunchy",
      "salty"
    ],
    "requires": [
      "microwave",
      "air_popper"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "When's your chip craving time that you'll swap for popcorn?",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Movie night",
        "3pm slump",
        "After dinner",
        "Lunch side",
        "Weekend snacking"
      ],
      "multiple": false
    },
    "difficulty": 1,
    "source": "dietitian_reviewed"
  },
  {
    "tip_id": "f5g6h7i8-0015-4015-a015-67890ab12345",
    "micro_version": "Put grapes in the freezer right now.",
    "summary": "Swap ice cream for frozen grapes or 'nice cream'.",
    "details_md": "**The Experiment:** When a craving for a sweet, frozen dessert strikes, try eating a bowl of frozen grapes or making banana 'nice cream' instead of regular ice cream.\n\n**Why it Works:** These swaps provide the sweet, cold, and creamy sensations of ice cream using whole fruits, which contain natural sugars, fiber, and micronutrients without the high levels of added sugar and saturated fat found in most ice creams.[9] Frozen grapes develop a sorbet-like texture.[10]\n\n**How to Try It:**\n• For frozen grapes, simply wash, dry, and freeze a bunch of grapes in a single layer on a baking sheet. Store in a freezer-safe bag.\n• For 'nice cream', blend one or two frozen, sliced bananas in a blender or food processor until it reaches a smooth, soft-serve consistency.[11]",
    "area": "eating",
    "goals": [
      "weight_loss",
      "reduce_sugar",
      "reduce_fat",
      "less_processed_food"
    ],
    "helps_with": [
      "sugar_addiction"
    ],
    "contraindications": [],
    "mechanisms": [
      "healthy_swap",
      "crave_buster",
      "sensory"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "evening",
      "late_night",
      "snack_time",
      "craving_event"
    ],
    "where": [
      "home"
    ],
    "involves": [
      "ice_cream"
    ],
    "satisfies": [
      "sweet",
      "creamy",
      "cold"
    ],
    "requires": [
      "freezer",
      "blender",
      "basic_cooking",
      "advance_planning"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "Create your frozen dessert strategy:",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Frozen grape poppers",
        "Banana nice cream",
        "Mixed berry nice cream",
        "Chocolate banana blend"
      ],
      "multiple": false
    },
    "difficulty": 2,
    "source": "dietitian_reviewed"
  },
  {
    "tip_id": "g6h7i8j9-0016-4016-a016-7890ab123456",
    "micro_version": "Add lean ground turkey to your grocery list.",
    "summary": "Swap ground beef for 99% lean ground turkey.",
    "details_md": "**The Experiment:** In a recipe that calls for ground beef, such as pasta sauce, tacos, or burgers, use 99% lean ground turkey or chicken instead.\n\n**Why it Works:** This simple swap dramatically reduces the total and saturated fat content of your meal without sacrificing the satisfying texture of ground meat. This is an easy way to make favorite comfort foods healthier.[4, 6]\n\n**How to Try It:**\n• Substitute it 1:1 in any recipe.\n• Since lean turkey is lower in fat, you may need to add a small amount of oil to the pan to prevent sticking.\n• Be sure to season it well, as it has a milder flavor than beef.",
    "area": "eating",
    "goals": [
      "weight_loss",
      "reduce_fat",
      "heart_health"
    ],
    "contraindications": [],
    "mechanisms": [
      "healthy_swap",
      "mastery"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "evening",
      "meal_time"
    ],
    "where": [
      "home"
    ],
    "involves": [
      "ground_beef"
    ],
    "preserves": [
      "tacos",
      "pasta_sauce",
      "burgers",
      "chili"
    ],
    "satisfies": [
      "savory"
    ],
    "requires": [
      "stove_top",
      "basic_cooking"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "Which ground beef dish will you make with turkey instead?",
    "personalization_type": "text",
    "personalization_config": {
      "placeholder": "e.g., Turkey tacos, turkey bolognese, turkey burgers"
    },
    "difficulty": 1,
    "source": "dietitian_reviewed",
    "helps_with": []
  },
  {
    "tip_id": "h7i8j9k0-0017-4017-a017-890ab1234567",
    "micro_version": "Mentally divide your plate into quarters before serving.",
    "summary": "Reorganize your plate: ½ veggies, ¼ protein, ¼ whole grains.",
    "details_md": "**The Experiment:** For one meal, visually divide your plate and fill half of it with non-starchy vegetables, one quarter with lean protein, and one quarter with whole grains.[3, 12]\n\n**Why it Works:** This simple visual guide, promoted by Harvard's Healthy Eating Plate, naturally increases your intake of fiber, vitamins, and minerals while managing portions of more calorie-dense starches and proteins. It promotes fullness and nutrient density without needing to count calories.[3, 12]\n\n**How to Try It:**\n• Start by filling the largest section (half your plate) with colorful veggies like broccoli, spinach, bell peppers, or a large salad.\n• Add a palm-sized portion of lean protein like grilled chicken, fish, or beans.[12]\n• Fill the final quarter with a fist-sized portion of whole grains like quinoa, brown rice, or whole-wheat pasta.[12]\n• Note: Starchy veggies like potatoes, corn, and peas count as grains in this model.[3]",
    "area": "eating",
    "goals": [
      "weight_loss",
      "produce_priority",
      "portion_control",
      "improve_gut_health"
    ],
    "helps_with": [
      "mindless_eating"
    ],
    "contraindications": [],
    "mechanisms": [
      "mindset_shift",
      "environment_design",
      "mastery",
      "decision_ease"
    ],
    "effort": "low",
    "time": "0-5",
    "cost": "$",
    "when": [
      "afternoon",
      "evening",
      "meal_time"
    ],
    "where": [
      "home",
      "restaurant"
    ],
    "preserves": [
      "chicken",
      "fish",
      "rice",
      "pasta"
    ],
    "features": [
      "veggie_heavy",
      "family_friendly",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "chaos_proof"
    ],
    "personalization_prompt": "Design your balanced plate for tonight:",
    "personalization_type": "multi_text",
    "personalization_config": {
      "items": [
        {
          "label": "½ plate veggies:",
          "placeholder": "e.g., roasted broccoli & salad"
        },
        {
          "label": "¼ protein:",
          "placeholder": "e.g., grilled chicken"
        },
        {
          "label": "¼ grains:",
          "placeholder": "e.g., quinoa"
        }
      ]
    },
    "difficulty": 2,
    "source": "dietitian_reviewed",
    "areas": [
      "nutrition",
      "organization"
    ]
  },
  {
    "tip_id": "i8j9k0l1-0018-4018-a018-90ab12345678",
    "micro_version": "Move your toothbrush to the kitchen counter.",
    "summary": "Brush your teeth right after dinner to prevent evening snacking.",
    "details_md": "**The Experiment:** Tonight, make it a point to brush and floss your teeth immediately after you finish eating dinner.\n\n**Why it Works:** This simple habit creates a powerful psychological cue that eating is finished for the day. The clean, minty feeling makes other foods and drinks (besides water) less appealing, which can effectively curb mindless late-night snacking or dessert cravings.[13]\n\n**How to Try It:**\n• As soon as you've cleared your plate, head to the bathroom and complete your dental routine.\n• Make it a non-negotiable part of your dinner cleanup process.\n• If a craving hits later, the thought of having to brush again can be a deterrent.",
    "area": "eating",
    "goals": [
      "weight_loss",
      "reduce_sugar",
      "reduce_cravings"
    ],
    "helps_with": [
      "mindless_eating",
      "late_night_eating"
    ],
    "contraindications": [],
    "mechanisms": [
      "habit_stacking",
      "time_ritual",
      "crave_buster",
      "decision_ease",
      "sensory"
    ],
    "effort": "low",
    "time": "0-5",
    "cost": "$",
    "when": [
      "evening",
      "meal_time",
      "boredom",
      "craving_event"
    ],
    "where": [
      "home"
    ],
    "involves": [
      "dessert",
      "snacks"
    ],
    "features": [
      "solo_friendly",
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "What time will you brush to signal 'kitchen closed'?",
    "personalization_type": "text",
    "personalization_config": {
      "placeholder": "e.g., 7:30 PM right after dishes"
    },
    "difficulty": 1,
    "source": "dietitian_reviewed"
  },
  {
    "tip_id": "a1b2c3d4-0001-4001-a001-1234567890ab",
    "summary": "Swap one sugary drink for water or sparkling water.",
    "hooks": [
      {
        "hook": "The single swap that cuts the most sugar",
        "subtitle": "Why liquid calories hit differently than food",
        "detail": "Sugary drinks are the #1 source of added sugar in the American diet. Unlike solid food, liquid calories don't trigger fullness signals—so you consume more without feeling satisfied. Swapping just one drink removes 150+ empty calories.",
        "action": "Today, when you'd normally reach for soda or juice, grab sparkling water instead. Add lemon or lime if you want flavor."
      },
      {
        "hook": "Your soda habit is costing you 15+ pounds a year",
        "subtitle": "The math on liquid sugar",
        "detail": "One daily soda adds up to 55,000+ calories annually—roughly 15 pounds of body weight. Sparkling water gives you the carbonation and ritual without the sugar or calories. It's the highest-impact single swap you can make.",
        "action": "Keep sparkling water in the fridge where your soda usually lives. Make it the default reach."
      },
      {
        "hook": "Bubbles without the sugar crash",
        "subtitle": "How to get the fizz fix without the downside",
        "detail": "If you love carbonation, you don't have to give it up—just swap what's in the bubbles. Sparkling water satisfies the sensory craving for fizz while keeping blood sugar stable and cutting empty calories.",
        "action": "At your next meal, order sparkling water with lemon instead of soda. Notice if you miss it as much as you expected."
      }
    ],
    "details_md": "**The Experiment:** For one day, whenever you would normally reach for a soda, sweetened juice, or other sugary beverage, substitute it with plain or naturally flavored sparkling water.\n\n**Why it Works:** Sugary drinks are a major source of \"empty\" calories and added sugars, which can contribute to weight gain and blood sugar fluctuations. Water or sparkling water provides hydration and can offer the satisfying carbonation of soda without the sugar or calories, helping to break the sugar habit.\n\n**How to Try It:**\n• Keep a pitcher of cold water or a few cans of sparkling water in the fridge\n• If you enjoy flavor, add a squeeze of fresh lemon, lime, or a few muddled berries yourself\n• When at a restaurant, ask for water or sparkling water with lemon instead of soda",
    "area": "eating",
    "goals": [
      "weight_loss",
      "reduce_sugar",
      "improve_hydration",
      "less_processed_food"
    ],
    "helps_with": [
      "mindless_eating",
      "sugar_addiction"
    ],
    "contraindications": [
      "phenylketonuria"
    ],
    "mechanisms": [
      "healthy_swap",
      "crave_buster",
      "sensory",
      "decision_ease"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "morning",
      "afternoon",
      "evening",
      "craving_event",
      "meal_time",
      "snack_time"
    ],
    "where": [
      "home",
      "work",
      "restaurant",
      "social_event",
      "commute"
    ],
    "involves": [
      "soda"
    ],
    "satisfies": [
      "sweet",
      "soda"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "Which sugary drink will you swap today?",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Morning juice",
        "Lunchtime soda",
        "Afternoon sweet tea",
        "Dinner soda",
        "Evening juice"
      ],
      "multiple": false
    },
    "difficulty": 1,
    "source": "dietitian_reviewed"
  },
  {
    "tip_id": "a1b2c3d4-0002-4002-a002-1234567890ac",
    "micro_version": "Put one vegetable (carrots, broccoli) on your plate right now.",
    "summary": "Add one serving of vegetables to your dinner.",
    "details_md": "**The Experiment:** Intentionally add one extra serving of vegetables to your dinner plate tonight. A serving is about the size of your fist or one cup.\n\n**Why it Works:** Most people don't eat enough vegetables. They are packed with vitamins, minerals, fiber, and antioxidants that are crucial for health and can lower the risk of chronic diseases. Adding volume with low-calorie, high-fiber vegetables can also help you feel fuller, aiding in portion control and weight management.\n\n**How to Try It:**\n• Add a handful of spinach to a pasta sauce or soup\n• Steam or roast a side of broccoli or green beans\n• Start your meal with a simple side salad\n• Frozen vegetables are a quick and easy option; just microwave and serve",
    "area": "eating",
    "goals": [
      "increase_veggies",
      "weight_loss",
      "improve_gut_health",
      "less_processed_food",
      "plant_based"
    ],
    "helps_with": [
      "mindless_eating"
    ],
    "contraindications": [],
    "mechanisms": [
      "healthy_swap",
      "habit_stacking",
      "mastery",
      "sensory"
    ],
    "effort": "low",
    "time": "0-5",
    "cost": "$",
    "when": [
      "evening",
      "meal_time"
    ],
    "where": [
      "home"
    ],
    "preserves": [
      "pasta",
      "meat",
      "cheese"
    ],
    "satisfies": [
      "soft",
      "crispy"
    ],
    "requires": [
      "microwave_only",
      "basic_cooking"
    ],
    "features": [
      "veggie_light",
      "family_friendly",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "Which veggie will join your dinner party tonight?",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Roasted broccoli",
        "Side salad",
        "Steamed green beans",
        "Sautéed spinach",
        "Roasted carrots",
        "Frozen mixed veggies"
      ],
      "multiple": false
    },
    "difficulty": 1,
    "source": "dietitian_reviewed"
  },
  {
    "tip_id": "f4a5b6c7-d8e9-0123-fabc-456789012345",
    "micro_version": "Put your phone in another room before your next meal.",
    "summary": "Eat one meal without distractions.",
    "details_md": "**The Experiment:** Choose one meal today—breakfast, lunch, or dinner—and eat it without any distractions. This means no phone, no TV, no computer, no reading. Just you and your food.\n\n**Why it Works:** Mindful eating helps you reconnect with your body's natural hunger and fullness signals. Eating without distraction allows you to slow down, savor flavors and textures, and better recognize when you are satisfied, which can prevent overeating.\n\n**How to Try It:**\n• Sit at a table\n• Put your phone in another room\n• Pay attention to the colors, smells, sounds, and textures of your food\n• Try putting your fork down between bites to pace yourself",
    "area": "eating",
    "goals": [
      "weight_loss",
      "improve_gut_health"
    ],
    "helps_with": [
      "speed_eating",
      "mindless_eating",
      "emotional_eating"
    ],
    "contraindications": [],
    "mechanisms": [
      "mindset_shift",
      "environment_design",
      "sensory",
      "comfort"
    ],
    "effort": "high",
    "time": "15-30min",
    "cost": "$",
    "when": [
      "morning",
      "afternoon",
      "evening",
      "meal_time"
    ],
    "where": [
      "home",
      "work"
    ],
    "preserves": [
      "chocolate",
      "cheese",
      "bread",
      "pasta",
      "meat"
    ],
    "features": [
      "solo_friendly",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety"
    ],
    "personalization_prompt": "Which meal will be your mindful moment?",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Peaceful breakfast",
        "Focused lunch",
        "Calm dinner"
      ],
      "multiple": false
    },
    "difficulty": 3,
    "source": "dietitian_reviewed",
    "sustainability": "daily"
  },
  {
    "tip_id": "a1b2c3d4-0006-4006-a006-1234567890b0",
    "micro_version": "Put a glass of water next to your coffee maker right now.",
    "summary": "Habit Stack: Drink a glass of water before your morning coffee.",
    "details_md": "**The Experiment:** Before you have your first cup of coffee or tea in the morning, drink a full 8 oz (240 ml) glass of water.\n\n**Why it Works:** This is a classic \"habit stacking\" technique, attaching a new desired habit (drinking water) to a strong existing habit (morning coffee). It starts your day with hydration, as you're often dehydrated after sleeping, and can help you reach your daily fluid goals.\n\n**How to Try It:**\n• The night before, place a glass and a pitcher of water right next to your coffee maker\n• Make it a non-negotiable rule: water first, then coffee",
    "area": "eating",
    "goals": [
      "improve_hydration",
      "improve_energy"
    ],
    "contraindications": [],
    "mechanisms": [
      "habit_stacking",
      "time_ritual",
      "decision_ease",
      "energy_boost"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "morning",
      "craving_event"
    ],
    "where": [
      "home",
      "work"
    ],
    "preserves": [
      "coffee"
    ],
    "requires": [
      "advance_planning"
    ],
    "features": [
      "solo_friendly",
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "Where will you place your water glass reminder?",
    "personalization_type": "text",
    "personalization_config": {
      "placeholder": "e.g., On top of coffee maker, next to mug, by the kettle"
    },
    "difficulty": 1,
    "source": "coach_curated",
    "sustainability": "daily",
    "areas": [
      "nutrition",
      "organization"
    ],
    "helps_with": []
  },
  {
    "tip_id": "a1b2c3d4-0010-4010-a010-1234567890b4",
    "micro_version": "Move one tempting snack to a hard-to-reach cabinet.",
    "summary": "The Kitchen Makeover: Hide one 'red light' food.",
    "details_md": "**The Experiment:** Identify one food or snack in your kitchen that you tend to overeat or know isn't helping your goals (a \"red light\" food). Move it out of sight.\n\n**Why it Works:** This is a small-scale version of the \"kitchen makeover\". You are changing your food environment to make the less-healthy choice harder to access. Out of sight is often out of mind. This reduces the number of times you have to exercise willpower during the day.\n\n**How to Try It:**\n• Move the cookies from the counter to a high shelf in the pantry\n• Put the bag of chips in an opaque container instead of its original crinkly bag\n• Ask a family member to store a tempting item where you won't easily find it",
    "area": "eating",
    "goals": [
      "weight_loss",
      "reduce_sugar",
      "less_processed_food"
    ],
    "helps_with": [
      "mindless_eating",
      "boredom_eating",
      "stress_eating"
    ],
    "contraindications": [],
    "mechanisms": [
      "environment_design",
      "decision_ease"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "morning",
      "afternoon",
      "evening",
      "late_night",
      "boredom",
      "stress"
    ],
    "where": [
      "home"
    ],
    "preserves": [
      "chocolate",
      "cheese",
      "coffee",
      "bread"
    ],
    "features": [
      "solo_friendly",
      "veggie_free",
      "family_friendly",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "Plan your kitchen makeover:",
    "personalization_type": "multi_text",
    "personalization_config": {
      "items": [
        {
          "label": "What 'red light' food will you hide?",
          "placeholder": "e.g., Chips, cookies, candy"
        },
        {
          "label": "Where will you hide it?",
          "placeholder": "e.g., Top shelf, basement, behind other items"
        }
      ]
    },
    "difficulty": 1,
    "source": "coach_curated",
    "sustainability": "daily",
    "areas": [
      "nutrition",
      "organization"
    ]
  },
  {
    "tip_id": "a1b2c3d4-0019-4019-a019-1234567890bd",
    "micro_version": "Stand up and walk around the block after your next meal.",
    "summary": "Go for a 10-minute walk after a meal.",
    "details_md": "**The Experiment:** After one of your main meals today, go for a brisk 10-minute walk.\n\n**Why it Works:** Walking after a meal can aid digestion and help stabilize blood sugar levels. It's a gentle way to incorporate more physical activity into your day, which contributes to overall energy expenditure and health. It also serves as a positive post-meal ritual instead of immediately sitting on the couch.\n\n**How to Try It:**\n• Right after you finish eating, put on your shoes and head out the door\n• Walk around your neighborhood, a nearby park, or even just around your office building\n• Listen to a podcast or music to make it more enjoyable",
    "area": "eating",
    "goals": [
      "weight_loss",
      "improve_gut_health",
      "reduce_sugar",
      "improve_energy"
    ],
    "helps_with": [
      "emotional_eating",
      "stress_eating"
    ],
    "contraindications": [
      "elderly_frailty"
    ],
    "mechanisms": [
      "habit_stacking",
      "time_ritual",
      "energy_boost",
      "comfort"
    ],
    "effort": "low",
    "time": "5-15",
    "cost": "$",
    "when": [
      "afternoon",
      "evening",
      "meal_time"
    ],
    "where": [
      "home",
      "work"
    ],
    "preserves": [
      "chocolate",
      "cheese",
      "coffee",
      "bread",
      "pasta"
    ],
    "features": [
      "solo_friendly",
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety"
    ],
    "personalization_prompt": "I will try a 10-minute walk after:",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Breakfast",
        "Lunch",
        "Dinner"
      ],
      "multiple": false,
      "style": "buttons"
    },
    "difficulty": 2,
    "source": "coach_curated",
    "sustainability": "daily"
  },
  {
    "tip_id": "a1b2c3d4-0030-4030-a030-1234567890c8",
    "micro_version": "Drink one full glass of water before deciding on a snack.",
    "summary": "Drink a glass of water when you feel a snack craving.",
    "details_md": "**The Experiment:** The next time you feel a craving for a snack, especially when you know you're not truly hungry (e.g., out of boredom or stress), drink a full glass of water first.\n\n**Why it Works:** Thirst is often mistaken for hunger. Pausing to drink water gives you a moment to check in with your body's cues. It may satisfy the craving, or at least give you time to make a more mindful choice about whether you really need to eat.\n\n**How to Try It:**\n• When a craving hits, go to the kitchen or water cooler and drink an 8 oz (240 ml) glass of water\n• Wait 10-15 minutes\n• Re-evaluate if you are still hungry. If you are, choose a healthy snack. If not, you've successfully hydrated and avoided unnecessary calories",
    "area": "eating",
    "goals": [
      "improve_hydration",
      "weight_loss",
      "mindset_shift",
      "reduce_sugar"
    ],
    "helps_with": [
      "mindless_eating",
      "boredom_eating",
      "stress_eating",
      "grazing"
    ],
    "contraindications": [],
    "mechanisms": [
      "crave_buster",
      "mindset_shift",
      "decision_ease",
      "mastery"
    ],
    "effort": "low",
    "time": "0-5",
    "cost": "$",
    "when": [
      "afternoon",
      "evening",
      "late_night",
      "craving_event",
      "boredom",
      "stress"
    ],
    "where": [
      "home",
      "work"
    ],
    "preserves": [
      "chocolate",
      "cheese",
      "bread"
    ],
    "features": [
      "solo_friendly",
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "What's your usual craving time that you'll try water first?",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Mid-morning munchies",
        "3pm slump",
        "After dinner",
        "Late night snack attack",
        "Watching TV"
      ],
      "multiple": false
    },
    "difficulty": 2,
    "source": "coach_curated",
    "sustainability": "daily"
  },
  {
    "tip_id": "a1b2c3d4-0004-4004-a004-1234567890ae",
    "micro_version": "Get a mason jar and place it on the counter for tomorrow.",
    "summary": "Prep a 'Salad in a Jar' for tomorrow's lunch.",
    "details_md": "**The Experiment:** Prepare a \"salad in a jar\" tonight for a ready-to-go, healthy lunch tomorrow.\n\n**Why it Works:** This is a classic meal prep technique that makes a healthy choice incredibly convenient, overcoming the common barrier of not having time to make a healthy lunch during a busy day. It's a form of environmental design for your future self.\n\n**How to Try It:**\n• Get a large mason jar\n• Layer ingredients from wettest to driest: Pour dressing in the bottom. Add hard vegetables (carrots, cucumbers, peppers). Add grains/proteins (quinoa, chickpeas, grilled chicken). Add softer items (cheese, seeds, nuts). Top with leafy greens\n• Seal and store in the fridge. To eat, just shake and pour into a bowl",
    "area": "eating",
    "goals": [
      "weight_loss",
      "increase_veggies",
      "less_processed_food",
      "plant_based",
      "improve_gut_health"
    ],
    "contraindications": [],
    "mechanisms": [
      "planning_ahead",
      "environment_design",
      "skill_building",
      "decision_ease",
      "mastery"
    ],
    "effort": "medium",
    "time": "5-15",
    "cost": "$",
    "when": [
      "evening",
      "meal_time"
    ],
    "where": [
      "home"
    ],
    "preserves": [
      "cheese"
    ],
    "requires": [
      "basic_cooking",
      "advance_planning",
      "advance_prep"
    ],
    "features": [
      "solo_friendly",
      "veggie_heavy",
      "family_friendly",
      "partner_resistant_ok",
      "diet_trauma_safe",
      "not_diety"
    ],
    "personalization_prompt": "Plan your salad jar layers. What ingredients will you use?",
    "personalization_type": "list",
    "personalization_config": {
      "items": [
        {
          "label": "Bottom Layer - Dressing",
          "placeholder": "e.g., Balsamic vinaigrette",
          "description": "Keeps other ingredients from getting soggy",
          "customizable": "description"
        },
        {
          "label": "Layer 2 - Hearty Veggies",
          "placeholder": "e.g., Cucumbers, tomatoes, peppers",
          "description": "Won't wilt from dressing",
          "customizable": "description"
        },
        {
          "label": "Layer 3 - Protein",
          "placeholder": "e.g., Grilled chicken, chickpeas, eggs",
          "description": "Your main protein source",
          "customizable": "description"
        },
        {
          "label": "Layer 4 - Soft Items",
          "placeholder": "e.g., Cheese, avocado, berries",
          "description": "Protected from dressing",
          "customizable": "description"
        },
        {
          "label": "Top Layer - Greens",
          "placeholder": "e.g., Spinach, lettuce, arugula",
          "description": "Stays crisp on top",
          "customizable": "description"
        }
      ]
    },
    "difficulty": 2,
    "source": "coach_curated",
    "sustainability": "daily",
    "areas": [
      "nutrition",
      "organization"
    ],
    "helps_with": []
  },
  {
    "tip_id": "a1b2c3d4-0005-4005-a005-1234567890af",
    "micro_version": "At your next meal, stop eating when you're satisfied, not stuffed.",
    "summary": "Eat to 80% full at one meal.",
    "details_md": "**The Experiment:** At one of your main meals today, practice eating slowly and stop when you feel about 80% full—satisfied, but not stuffed.\n\n**Why it Works:** It takes about 15-20 minutes for your stomach to signal to your brain that it's full. Eating slowly and stopping at 80% full gives your body time to register satiety, helping you naturally reduce calorie intake without feeling deprived. This is a core habit for long-term weight management.\n\n**How to Try It:**\n• Serve yourself a slightly smaller portion than usual to start\n• Eat slowly and mindfully, paying attention to how your stomach feels\n• Pause halfway through your meal for a minute and assess your hunger level\n• Aim for a feeling of \"no longer hungry\" rather than \"full\"",
    "area": "eating",
    "goals": [
      "weight_loss",
      "improve_gut_health"
    ],
    "helps_with": [
      "speed_eating",
      "mindless_eating"
    ],
    "contraindications": [],
    "mechanisms": [
      "mindset_shift",
      "self_monitoring",
      "mastery",
      "comfort"
    ],
    "effort": "high",
    "time": "15-30min",
    "cost": "$",
    "when": [
      "morning",
      "afternoon",
      "evening",
      "meal_time"
    ],
    "where": [
      "home",
      "work",
      "restaurant"
    ],
    "preserves": [
      "chocolate",
      "cheese",
      "bread",
      "pasta",
      "meat"
    ],
    "features": [
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly"
    ],
    "personalization_prompt": "What sensations in your body will you pay attention to know you are at 80% full?",
    "personalization_type": "text",
    "personalization_config": {
      "placeholder": "e.g., Slight pressure in stomach, no longer feeling hungry, clothes feeling comfortable"
    },
    "difficulty": 3,
    "source": "dietitian_reviewed",
    "sustainability": "daily"
  },
  {
    "tip_id": "a7b8c9d0-e1f2-3456-abcd-789012345678",
    "micro_version": "Add an egg or Greek yogurt to tomorrow's breakfast.",
    "summary": "Add 20-30g of protein to your breakfast.",
    "details_md": "**The Experiment:** Include a significant source of protein—like eggs, Greek yogurt, or protein powder—in your breakfast today. Aim for 20-30 grams.\n\n**Why it Works:** Protein is the most satiating macronutrient. Including it in your first meal helps you feel full longer, reducing cravings and the likelihood of snacking on less healthy options before lunch. After an overnight fast, it also helps stabilize blood sugar and provides sustained energy.\n\n**How to Try It:**\n• Scramble 2-3 eggs instead of having just toast or cereal\n• Stir a scoop of protein powder into your oatmeal or yogurt\n• A cup of Greek yogurt or cottage cheese can provide over 20g of protein",
    "area": "eating",
    "goals": [
      "weight_loss",
      "muscle_gain",
      "reduce_sugar",
      "improve_energy"
    ],
    "helps_with": [
      "grazing"
    ],
    "contraindications": [
      "egg_allergy",
      "lactose_intolerance",
      "phenylketonuria",
      "kidney_disease"
    ],
    "mechanisms": [
      "healthy_swap",
      "planning_ahead",
      "energy_boost",
      "mastery"
    ],
    "effort": "low",
    "time": "5-15",
    "cost": "$",
    "when": [
      "morning",
      "meal_time"
    ],
    "where": [
      "home",
      "work"
    ],
    "involves": [
      "dairy",
      "meat"
    ],
    "preserves": [
      "coffee",
      "bread"
    ],
    "satisfies": [
      "creamy",
      "soft"
    ],
    "requires": [
      "basic_stove",
      "microwave_only",
      "basic_cooking"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "diet_trauma_safe",
      "not_diety"
    ],
    "personalization_prompt": "Design your protein-packed breakfast:",
    "personalization_type": "text",
    "personalization_config": {
      "placeholder": "e.g., 2 eggs + Greek yogurt + berries = 25g protein"
    },
    "difficulty": 2,
    "source": "dietitian_reviewed",
    "sustainability": "daily"
  },
  {
    "tip_id": "a1b2c3d4-0008-4008-a008-1234567890b2",
    "micro_version": "Grab an apple or banana to have instead of your usual snack.",
    "summary": "Swap a processed snack for a piece of whole fruit.",
    "details_md": "**The Experiment:** Today, when you feel like having a snack, replace your usual choice (like chips, cookies, or a granola bar) with a piece of whole fruit.\n\n**Why it Works:** Whole fruit provides fiber, vitamins, and water, making it a more nutrient-dense and filling choice than most processed snacks. This simple swap reduces your intake of added sugars, refined grains, and unhealthy fats while increasing your intake of beneficial nutrients.\n\n**How to Try It:**\n• Keep a bowl of fruit like apples, bananas, or oranges visible on your counter\n• Pack a portable fruit with you for work or errands\n• To make it more satisfying, pair it with a small handful of nuts or a spoonful of peanut butter",
    "area": "eating",
    "goals": [
      "increase_veggies",
      "weight_loss",
      "reduce_sugar",
      "less_processed_food",
      "improve_gut_health"
    ],
    "helps_with": [
      "mindless_eating",
      "boredom_eating"
    ],
    "contraindications": [],
    "mechanisms": [
      "healthy_swap",
      "crave_buster",
      "sensory",
      "decision_ease"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "afternoon",
      "morning",
      "snack_time",
      "craving_event"
    ],
    "where": [
      "home",
      "work",
      "commute"
    ],
    "satisfies": [
      "sweet",
      "crunchy",
      "crispy",
      "soft"
    ],
    "features": [
      "solo_friendly",
      "veggie_light",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "What's your fruit swap mission today?",
    "personalization_type": "multi_text",
    "personalization_config": {
      "items": [
        {
          "label": "Swap OUT:",
          "placeholder": "e.g., Afternoon cookies, chips"
        },
        {
          "label": "Swap IN:",
          "placeholder": "e.g., Crispy apple, juicy orange"
        }
      ]
    },
    "difficulty": 1,
    "source": "dietitian_reviewed",
    "sustainability": "daily"
  },
  {
    "tip_id": "b4c5d6e7-f8a9-0123-bcde-456789012345",
    "micro_version": "Try one new vegetable at your next meal.",
    "summary": "Try the \"Colorful Plate\" Challenge.",
    "details_md": "**The Experiment:** For one meal today, try to include foods of at least three different colors on your plate.\n\n**Why it Works:** Different colors in fruits and vegetables signify the presence of different vitamins, minerals, and antioxidant compounds (phytonutrients). \"Eating the rainbow\" is a simple heuristic to ensure you're getting a wide variety of nutrients that support overall health, from your immune system to your cardiovascular system.\n\n**How to Try It:**\n• Make a salad with green lettuce, red tomatoes, and yellow bell peppers\n• Create a stir-fry with purple cabbage, orange carrots, and green broccoli\n• Roast a mix of red onions, green zucchini, and white cauliflower",
    "area": "eating",
    "goals": [
      "increase_veggies",
      "improve_gut_health",
      "less_processed_food",
      "plant_based"
    ],
    "contraindications": [],
    "mechanisms": [
      "skill_building",
      "mindset_shift",
      "sensory",
      "novelty",
      "mastery"
    ],
    "effort": "low",
    "time": "5-15",
    "cost": "$",
    "when": [
      "evening",
      "afternoon",
      "meal_time"
    ],
    "where": [
      "home"
    ],
    "satisfies": [
      "crunchy",
      "soft"
    ],
    "requires": [
      "basic_stove",
      "full_kitchen",
      "basic_cooking",
      "advance_planning"
    ],
    "features": [
      "veggie_heavy",
      "family_friendly",
      "kid_approved",
      "diet_trauma_safe",
      "not_diety"
    ],
    "personalization_prompt": "Build your rainbow! Pick 3+ colors for your plate:",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Red (tomatoes, peppers)",
        "Orange (carrots, squash)",
        "Yellow (corn, bell peppers)",
        "Green (broccoli, spinach)",
        "Blue/Purple (eggplant, cabbage)",
        "White (cauliflower, onions)"
      ],
      "multiple": true
    },
    "difficulty": 2,
    "source": "dietitian_reviewed",
    "sustainability": "occasional",
    "helps_with": []
  },
  {
    "tip_id": "a1b2c3d4-e5f6-7890-abcd-123456789012",
    "summary": "Choose whole grains over refined for one meal.",
    "hooks": [
      {
        "hook": "Same meal, better fuel",
        "subtitle": "Why the type of carb matters as much as the amount",
        "detail": "Whole grains keep all three parts of the grain—bran, germ, and endosperm—giving you fiber, B vitamins, and minerals that refined grains strip away. The fiber slows digestion, keeping you full longer and your blood sugar steadier.",
        "action": "At one meal today, swap white for whole: brown rice instead of white, whole wheat bread instead of white, or quinoa instead of regular pasta."
      },
      {
        "hook": "White carbs are missing the good parts",
        "subtitle": "What refining actually removes",
        "detail": "When grains are refined, the fiber-rich bran and nutrient-dense germ are stripped away, leaving only the starchy endosperm. You lose fiber, iron, B vitamins, and the compounds that help you feel full. Whole grains keep everything.",
        "action": "Check your pantry: pick one refined grain product to swap for whole grain on your next grocery run."
      },
      {
        "hook": "The swap that keeps you full longer",
        "subtitle": "How fiber changes what happens after you eat",
        "detail": "Whole grains have 3-4x more fiber than refined grains. That fiber slows digestion, prevents blood sugar spikes, and keeps you satisfied for hours instead of hunting for snacks. Same carbs, different outcome.",
        "action": "Try this swap at lunch or dinner today: whole wheat pasta, brown rice, or quinoa instead of the white version."
      }
    ],
    "details_md": "**The Experiment:** For one meal today, deliberately choose a whole-grain carbohydrate source instead of a refined one.\n\n**Why it Works:** Whole grains like brown rice, quinoa, and whole-wheat bread contain more fiber, vitamins, and minerals than their refined counterparts (white rice, white bread). The higher fiber content helps you feel full for longer, aids in digestion, and supports more stable blood sugar levels.\n\n**How to Try It:**\n• Swap white pasta for whole-wheat pasta\n• Choose brown rice instead of white rice with your stir-fry or curry\n• Make a sandwich using 100% whole-wheat bread. Look for \"100% whole grain\" as the first ingredient",
    "area": "eating",
    "goals": [
      "weight_loss",
      "improve_gut_health",
      "less_processed_food",
      "better_lipids",
      "reduce_sugar",
      "improve_energy"
    ],
    "contraindications": [
      "celiac"
    ],
    "mechanisms": [
      "healthy_swap",
      "skill_building",
      "mastery",
      "sensory"
    ],
    "effort": "low",
    "time": "0-5",
    "cost": "$",
    "when": [
      "morning",
      "afternoon",
      "evening",
      "meal_time",
      "grocery_shopping"
    ],
    "where": [
      "home",
      "restaurant"
    ],
    "involves": [
      "bread",
      "pasta",
      "carbs"
    ],
    "preserves": [
      "meat",
      "cheese"
    ],
    "satisfies": [
      "chewy"
    ],
    "requires": [
      "basic_stove",
      "basic_cooking"
    ],
    "features": [
      "veggie_free",
      "partner_resistant_ok",
      "no_planning",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "Which grain swap will you make?",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Brown rice instead of white",
        "Whole wheat pasta",
        "Quinoa bowl",
        "Whole grain bread",
        "Overnight oats"
      ],
      "multiple": false
    },
    "difficulty": 2,
    "source": "dietitian_reviewed",
    "sustainability": "daily",
    "helps_with": []
  },
  {
    "tip_id": "a1b2c3d4-0012-4012-a012-1234567890b6",
    "micro_version": "Grab a small handful of nuts to eat right now.",
    "summary": "Have a healthy fat snack like nuts or avocado.",
    "details_md": "**The Experiment:** For one of your snacks today, choose a source of healthy fats, such as a small handful of nuts (about 1/4 cup) or half an avocado.\n\n**Why it Works:** Healthy fats are essential for hormone production and overall health. They are also very satiating, helping to keep you full between meals. Nuts and avocados also provide fiber and other important nutrients. Contrary to old beliefs, dietary fat is not to be feared and is a key part of a balanced diet.\n\n**How to Try It:**\n• A small handful of almonds, walnuts, or mixed nuts\n• Half an avocado sprinkled with salt and pepper\n• Apple slices with a tablespoon of natural almond or peanut butter",
    "area": "eating",
    "goals": [
      "weight_loss",
      "better_lipids",
      "improve_energy",
      "reduce_sugar"
    ],
    "helps_with": [
      "grazing"
    ],
    "contraindications": [
      "nut_allergy"
    ],
    "mechanisms": [
      "healthy_swap",
      "crave_buster",
      "sensory",
      "energy_boost"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "afternoon",
      "snack_time"
    ],
    "where": [
      "home",
      "work",
      "commute"
    ],
    "satisfies": [
      "salty",
      "creamy",
      "crunchy"
    ],
    "features": [
      "solo_friendly",
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "Pick your healthy fat superhero snack for today:",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "The Nutty Professor (mixed nuts)",
        "Avocado Assassin (half avocado + salt)",
        "Apple & Almond Butter Alliance",
        "Walnut Warrior Mix",
        "Pistchio Power Pack"
      ],
      "multiple": false
    },
    "difficulty": 1,
    "source": "dietitian_reviewed",
    "sustainability": "daily"
  },
  {
    "tip_id": "e9f0a1b2-c3d4-5678-efab-901234567890",
    "micro_version": "Grab some small bags and portion out one snack now.",
    "summary": "Pre-portion snacks into daily servings.",
    "details_md": "**The Experiment:** If you plan to have snacks today, take five minutes in the morning to portion them out into small bags or containers.\n\n**Why it Works:** This simple act of planning prevents mindless overeating directly from a large bag or box. It's a form of portion control that requires a decision only once, making it easier to stick to your goals when you're hungry or distracted later in the day. It's a key strategy for managing calorie intake without obsessive counting.\n\n**How to Try It:**\n• Get out a box of crackers or a bag of nuts\n• Use a measuring cup or just count out a reasonable serving size (e.g., 1 ounce of nuts)\n• Put each portion into a separate small container or zip-top bag",
    "area": "eating",
    "goals": [
      "weight_loss",
      "reduce_sugar",
      "less_processed_food",
      "better_lipids"
    ],
    "helps_with": [
      "mindless_eating",
      "grazing",
      "boredom_eating"
    ],
    "contraindications": [
      "nut_allergy"
    ],
    "mechanisms": [
      "planning_ahead",
      "environment_design",
      "self_monitoring",
      "decision_ease",
      "mastery"
    ],
    "effort": "low",
    "time": "5-15",
    "cost": "$",
    "when": [
      "morning",
      "snack_time",
      "grocery_shopping"
    ],
    "where": [
      "home",
      "work"
    ],
    "preserves": [
      "chocolate",
      "cheese"
    ],
    "requires": [
      "advance_planning",
      "advance_prep"
    ],
    "features": [
      "solo_friendly",
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok"
    ],
    "personalization_prompt": "What snacks will you portion out and how many 'snack packs' will you make?",
    "personalization_type": "multi_text",
    "personalization_config": {
      "items": [
        {
          "label": "Snack to portion:",
          "placeholder": "e.g., Trail mix, crackers, nuts"
        },
        {
          "label": "Number of packs:",
          "placeholder": "e.g., 5 for the week"
        }
      ]
    },
    "difficulty": 2,
    "source": "coach_curated",
    "sustainability": "daily",
    "areas": [
      "nutrition",
      "organization"
    ]
  },
  {
    "tip_id": "a1b2c3d4-0014-4014-a014-1234567890b8",
    "micro_version": "Mix olive oil, lemon juice, and salt in a jar.",
    "summary": "Make your own simple salad dressing.",
    "details_md": "**The Experiment:** Instead of using a store-bought bottled dressing, make your own simple vinaigrette for your salad today.\n\n**Why it Works:** Many commercial salad dressings are high in added sugar, unhealthy fats, and sodium. Making your own is surprisingly easy and gives you complete control over the ingredients. A simple olive oil and vinegar dressing provides heart-healthy monounsaturated fats.\n\n**How to Try It:**\n• The classic ratio is 3 parts oil to 1 part vinegar\n• In a small jar, combine 3 tablespoons of extra virgin olive oil with 1 tablespoon of balsamic or red wine vinegar\n• Add a pinch of salt, pepper, and maybe a little Dijon mustard. Shake well and serve",
    "area": "eating",
    "goals": [
      "less_processed_food",
      "better_lipids",
      "reduce_sugar",
      "increase_veggies"
    ],
    "contraindications": [],
    "mechanisms": [
      "skill_building",
      "healthy_swap",
      "mastery",
      "sensory"
    ],
    "effort": "low",
    "time": "0-5",
    "cost": "$",
    "when": [
      "afternoon",
      "evening",
      "meal_time"
    ],
    "where": [
      "home"
    ],
    "preserves": [
      "cheese",
      "meat"
    ],
    "satisfies": [
      "smooth"
    ],
    "requires": [
      "basic_cooking"
    ],
    "features": [
      "solo_friendly",
      "veggie_moderate",
      "family_friendly",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "Create your signature dressing blend:",
    "personalization_type": "multi_text",
    "personalization_config": {
      "items": [
        {
          "label": "Oil base:",
          "placeholder": "e.g., Olive oil, avocado oil"
        },
        {
          "label": "Acid:",
          "placeholder": "e.g., Lemon juice, balsamic vinegar"
        },
        {
          "label": "Special touch:",
          "placeholder": "e.g., Dijon mustard, herbs, garlic"
        }
      ]
    },
    "difficulty": 2,
    "source": "dietitian_reviewed",
    "sustainability": "daily",
    "helps_with": []
  },
  {
    "tip_id": "f8a9b0c1-d2e3-4567-fabc-890123456789",
    "micro_version": "Set a timer for 20 minutes when you start eating.",
    "summary": "Eat Slowly: Take 20+ Minutes for Main Meals.",
    "details_md": "**The Experiment:** For one meal, set a timer for 20 minutes and aim to make the meal last the entire time.\n\n**Why it Works:** This is a structured way to practice eating slowly. It forces you to pace yourself, which gives your brain the time it needs to register fullness cues from your stomach, helping to prevent overeating. This practice builds body awareness and is a powerful tool for mindful eating.\n\n**How to Try It:**\n• Set a timer on your phone or kitchen timer for 20 minutes when you sit down to eat\n• Take smaller bites and chew each bite thoroughly\n• Put your utensils down between bites\n• Try to be the last person at the table to finish",
    "area": "eating",
    "goals": [
      "weight_loss",
      "improve_gut_health"
    ],
    "helps_with": [
      "speed_eating",
      "mindless_eating"
    ],
    "contraindications": [],
    "mechanisms": [
      "time_ritual",
      "self_monitoring",
      "skill_building",
      "mastery",
      "comfort"
    ],
    "effort": "high",
    "time": "15-30min",
    "cost": "$",
    "when": [
      "evening",
      "afternoon",
      "meal_time"
    ],
    "where": [
      "home",
      "restaurant"
    ],
    "preserves": [
      "chocolate",
      "cheese",
      "bread",
      "pasta",
      "meat",
      "coffee"
    ],
    "features": [
      "no_planning",
      "impulse_friendly"
    ],
    "personalization_prompt": "What's your strategy for timing 20 minutes?",
    "personalization_type": "text",
    "personalization_config": {
      "placeholders": [
        "e.g., Set a timer, put on a 20-minute playlist, watch a sitcom episode"
      ]
    },
    "difficulty": 3,
    "source": "coach_curated",
    "sustainability": "occasional"
  },
  {
    "tip_id": "a1b2c3d4-0016-4016-a016-1234567890ba",
    "micro_version": "Slice a cucumber or grab some watermelon from the fridge.",
    "summary": "Eat your water: snack on cucumber or watermelon.",
    "details_md": "**The Experiment:** Have a snack that consists of a high-water-content food, like cucumber slices, celery sticks, or watermelon chunks.\n\n**Why it Works:** Hydration doesn't only come from beverages. Many fruits and vegetables have very high water content, contributing to your daily fluid needs. These snacks are also typically low in calories and high in nutrients, making them a refreshing and healthy choice.\n\n**How to Try It:**\n• Slice up a cucumber and enjoy it plain or with a little hummus\n• Cut a watermelon into cubes for a sweet, hydrating treat\n• Pack some celery sticks for a crunchy snack on the go",
    "area": "eating",
    "goals": [
      "improve_hydration",
      "increase_veggies",
      "weight_loss"
    ],
    "helps_with": [
      "mindless_eating"
    ],
    "contraindications": [],
    "mechanisms": [
      "healthy_swap",
      "sensory",
      "energy_boost"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "afternoon",
      "snack_time"
    ],
    "where": [
      "home",
      "work"
    ],
    "satisfies": [
      "crunchy",
      "sweet",
      "crispy"
    ],
    "features": [
      "solo_friendly",
      "veggie_light",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "Choose your 'water you can eat' adventure:",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Cucumber Spa Day",
        "Watermelon Wave",
        "Celery Crunch Time",
        "Melon Medley",
        "Tomato Tango"
      ],
      "multiple": false
    },
    "difficulty": 1,
    "source": "dietitian_reviewed",
    "sustainability": "daily"
  },
  {
    "tip_id": "a1b2c3d4-0017-4017-a017-1234567890bb",
    "micro_version": "Add one leafy green to your next meal.",
    "summary": "Add a \"flavor boost\" to water instead of juice.",
    "details_md": "**The Experiment:** Instead of reaching for a fruit juice or other sweet drink, flavor a glass of water with fresh ingredients.\n\n**Why it Works:** This provides flavor and novelty without the high sugar content of juice. Even 100% fruit juice is a concentrated source of sugar without the fiber of whole fruit. Infusing water is a great way to make hydration more appealing and reduce overall sugar intake.\n\n**How to Try It:**\n• Add slices of citrus (lemon, orange) and some fresh mint leaves to a pitcher of water\n• Muddle a few raspberries or strawberries at the bottom of your glass before adding water\n• Try cucumber slices and a sprig of rosemary for a more savory, spa-like flavor",
    "area": "eating",
    "goals": [
      "improve_hydration",
      "reduce_sugar",
      "less_processed_food"
    ],
    "helps_with": [
      "sugar_addiction"
    ],
    "contraindications": [],
    "mechanisms": [
      "healthy_swap",
      "crave_buster",
      "sensory",
      "novelty"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "afternoon",
      "morning",
      "craving_event",
      "snack_time"
    ],
    "where": [
      "home",
      "work"
    ],
    "satisfies": [
      "sweet",
      "smooth"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "Create your signature water infusion recipe:",
    "personalization_type": "text",
    "personalization_config": {
      "placeholder": "e.g., Lemon + cucumber + mint = Spa Water"
    },
    "difficulty": 1,
    "source": "dietitian_reviewed",
    "sustainability": "daily"
  },
  {
    "tip_id": "a1b2c3d4-0018-4018-a018-1234567890bc",
    "micro_version": "Check if you have Greek yogurt in the fridge.",
    "summary": "Try Greek yogurt instead of sour cream or mayo.",
    "details_md": "**The Experiment:** If you have a meal or snack that typically uses sour cream or mayonnaise, try substituting plain Greek yogurt instead.\n\n**Why it Works:** Plain Greek yogurt has a similar tangy flavor and creamy texture to sour cream and mayo but is significantly higher in protein and lower in fat and calories. This swap boosts the nutritional value of your meal, helping you feel fuller and supporting muscle maintenance or growth.\n\n**How to Try It:**\n• Top a baked potato or chili with a dollop of Greek yogurt\n• Make a creamy dip for vegetables using Greek yogurt as the base\n• Use it in place of mayonnaise when making tuna or chicken salad",
    "area": "eating",
    "goals": [
      "weight_loss",
      "muscle_gain",
      "less_processed_food",
      "better_lipids"
    ],
    "contraindications": [
      "lactose_intolerance"
    ],
    "mechanisms": [
      "healthy_swap",
      "mastery",
      "sensory"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "afternoon",
      "evening",
      "meal_time",
      "snack_time"
    ],
    "where": [
      "home"
    ],
    "involves": [
      "dairy"
    ],
    "preserves": [
      "bread",
      "meat"
    ],
    "satisfies": [
      "creamy"
    ],
    "features": [
      "solo_friendly",
      "veggie_free",
      "family_friendly",
      "partner_resistant_ok",
      "no_planning",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "Where will you deploy your Greek yogurt swap?",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Baked potato topping",
        "Taco night",
        "Dip base",
        "Tuna/chicken salad",
        "Sandwich spread"
      ],
      "multiple": false
    },
    "difficulty": 2,
    "source": "coach_curated",
    "sustainability": "daily",
    "helps_with": []
  },
  {
    "tip_id": "a1b2c3d4-0020-4020-a020-1234567890be",
    "micro_version": "Put oats, milk, and a jar on the counter for tonight.",
    "summary": "Create 'overnight oats' for tomorrow's breakfast.",
    "details_md": "**The Experiment:** Tonight, take 5 minutes to prepare overnight oats for a grab-and-go breakfast tomorrow morning.\n\n**Why it Works:** This is a powerful meal prep strategy that eliminates morning decision-making and ensures you start the day with a high-fiber, protein-rich meal. It's convenient, customizable, and helps prevent skipping breakfast or grabbing a less healthy option on the way out the door.\n\n**How to Try It:**\n• In a jar or container, combine equal parts rolled oats and milk (or a milk alternative). A common starting point is 1/2 cup of each\n• Add a scoop of protein powder, a tablespoon of chia seeds, and some fruit like berries\n• Stir, cover, and refrigerate overnight. Eat it cold in the morning",
    "area": "eating",
    "goals": [
      "weight_loss",
      "muscle_gain",
      "improve_hydration",
      "improve_gut_health",
      "reduce_sugar"
    ],
    "helps_with": [
      "grazing"
    ],
    "contraindications": [
      "celiac"
    ],
    "mechanisms": [
      "planning_ahead",
      "environment_design",
      "decision_ease",
      "mastery"
    ],
    "effort": "low",
    "time": "0-5",
    "cost": "$",
    "when": [
      "evening",
      "meal_time"
    ],
    "where": [
      "home"
    ],
    "involves": [
      "dairy",
      "carbs"
    ],
    "preserves": [
      "coffee"
    ],
    "satisfies": [
      "creamy",
      "soft"
    ],
    "requires": [
      "advance_planning",
      "advance_prep"
    ],
    "features": [
      "solo_friendly",
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "diet_trauma_safe",
      "not_diety"
    ],
    "personalization_prompt": "Design your overnight oats masterpiece:",
    "personalization_type": "multi_text",
    "personalization_config": {
      "items": [
        {
          "label": "Base:",
          "placeholder": "e.g., Oats + almond milk"
        },
        {
          "label": "Mix-ins:",
          "placeholder": "e.g., Chia seeds, protein powder"
        },
        {
          "label": "Toppings:",
          "placeholder": "e.g., Berries, nuts, cinnamon"
        }
      ]
    },
    "difficulty": 2,
    "source": "coach_curated",
    "sustainability": "daily",
    "areas": [
      "nutrition",
      "organization"
    ]
  },
  {
    "tip_id": "a1b2c3d4-0021-4021-a021-1234567890bf",
    "micro_version": "Boil 2-3 eggs right now so they're ready for snacking.",
    "summary": "Have a hard-boiled egg for a snack.",
    "details_md": "**The Experiment:** For a quick and easy snack, have one or two hard-boiled eggs.\n\n**Why it Works:** Eggs are a powerhouse of nutrition, providing high-quality protein and essential vitamins and minerals in a convenient, low-calorie package. The protein helps keep you full and satisfied, making it an excellent choice for curbing hunger between meals and preventing overeating later.\n\n**How to Try It:**\n• Boil a batch of eggs at the beginning of the week so they are ready to grab\n• A perfectly hard-boiled egg with a just-cooked yolk takes about 7-8 minutes in boiling water\n• Peel and eat with a sprinkle of salt and pepper",
    "area": "eating",
    "goals": [
      "weight_loss",
      "muscle_gain",
      "reduce_sugar"
    ],
    "helps_with": [
      "grazing"
    ],
    "contraindications": [
      "egg_allergy"
    ],
    "mechanisms": [
      "healthy_swap",
      "planning_ahead",
      "decision_ease",
      "energy_boost"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "morning",
      "afternoon",
      "snack_time"
    ],
    "where": [
      "home",
      "work"
    ],
    "involves": [
      "meat"
    ],
    "satisfies": [
      "soft"
    ],
    "requires": [
      "basic_stove",
      "basic_cooking",
      "advance_planning",
      "advance_prep"
    ],
    "features": [
      "solo_friendly",
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "When will your egg snack power you up?",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Mid-morning boost",
        "Pre-lunch fuel",
        "Afternoon pick-me-up",
        "Pre-workout power",
        "Evening protein"
      ],
      "multiple": false
    },
    "difficulty": 1,
    "source": "dietitian_reviewed",
    "sustainability": "daily",
    "areas": [
      "nutrition",
      "organization"
    ]
  },
  {
    "tip_id": "d0e1f2a3-b4c5-6789-defa-012345678901",
    "micro_version": "Look at your palm - that's your protein portion size.",
    "summary": "Use Palm-Size Protein Portions.",
    "details_md": "**The Experiment:** Include one palm-sized portion of protein (e.g., chicken, fish, tofu) at every meal today.\n\n**Why it Works:** Consistent protein intake helps maintain muscle mass and promotes satiety. Using your palm as a guide is a simple, portable way to estimate a 20-30g protein portion without measuring or weighing, making it easy to apply anywhere.\n\n**How to Try It:**\n• At each meal, look at the protein source on your plate\n• A serving that is roughly the size and thickness of your palm (excluding fingers) is one portion\n• Adjust based on your hand size and activity level",
    "area": "eating",
    "goals": [
      "muscle_gain",
      "weight_loss",
      "improve_energy"
    ],
    "contraindications": [],
    "mechanisms": [
      "skill_building",
      "self_monitoring",
      "decision_ease",
      "mastery"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "morning",
      "afternoon",
      "evening",
      "meal_time"
    ],
    "where": [
      "home",
      "work",
      "restaurant"
    ],
    "involves": [
      "meat"
    ],
    "preserves": [
      "bread",
      "pasta",
      "cheese"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "When will you try using palm-sized protein portions?",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Breakfast",
        "Lunch",
        "Dinner",
        "Afternoon snack",
        "Evening snack"
      ],
      "multiple": true,
      "style": "buttons"
    },
    "difficulty": 1,
    "source": "coach_curated",
    "sustainability": "daily",
    "helps_with": []
  },
  {
    "tip_id": "e1f2a3b4-c5d6-7890-efab-123456789012",
    "micro_version": "Put vegetables on half your plate at your next meal.",
    "summary": "Fill Half Your Plate with Vegetables First.",
    "details_md": "**The Experiment:** At one meal today, physically fill half of your plate with vegetables before adding any protein or carbohydrates.\n\n**Why it Works:** This \"higher-order habit\" is highly effective. This sequential plating strategy naturally controls portions of more calorie-dense foods while ensuring you meet your fiber and micronutrient needs. It makes a healthy choice the default.\n\n**How to Try It:**\n• Grab your plate. Before adding anything else, load up half of it with a large salad, steamed broccoli, or roasted mixed vegetables\n• Then, use the remaining half of the plate for your protein and carb sources",
    "area": "eating",
    "goals": [
      "increase_veggies",
      "weight_loss",
      "improve_gut_health",
      "plant_based"
    ],
    "helps_with": [
      "mindless_eating"
    ],
    "contraindications": [],
    "mechanisms": [
      "habit_stacking",
      "skill_building",
      "decision_ease",
      "mastery"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "afternoon",
      "evening",
      "meal_time"
    ],
    "where": [
      "home",
      "restaurant"
    ],
    "preserves": [
      "meat",
      "cheese",
      "bread",
      "pasta"
    ],
    "features": [
      "veggie_heavy",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety"
    ],
    "personalization_prompt": "Which veggie all-star will fill your plate's starting lineup?",
    "personalization_type": "text",
    "personalization_config": {
      "placeholder": "e.g., Roasted rainbow carrots, Caesar salad, steamed broccoli brigade"
    },
    "difficulty": 1,
    "source": "dietitian_reviewed",
    "sustainability": "daily"
  },
  {
    "tip_id": "a1b2c3d4-0024-4024-a024-1234567890c2",
    "micro_version": "Write down what you'll eat for breakfast and lunch tomorrow.",
    "summary": "Plan tomorrow's meals before you go to bed.",
    "details_md": "**The Experiment:** Tonight, take 5-10 minutes to think about and write down what you plan to eat for your main meals tomorrow.\n\n**Why it Works:** Planning ahead significantly reduces in-the-moment decision making when you're hungry, stressed, or rushed—times when you're most likely to make less-healthy choices. This simple act of forethought makes you more likely to stick to your goals.\n\n**How to Try It:**\n• Use a notebook or a notes app on your phone\n• Jot down a simple plan: Breakfast (e.g., oatmeal), Lunch (e.g., leftovers from dinner), Dinner (e.g., chicken and veggies)\n• This can also help you identify if you need to do any small prep tasks, like taking something out of the freezer",
    "area": "eating",
    "goals": [
      "weight_loss",
      "reduce_sugar",
      "less_processed_food"
    ],
    "helps_with": [
      "stress_eating",
      "mindless_eating"
    ],
    "contraindications": [],
    "mechanisms": [
      "planning_ahead",
      "mindset_shift",
      "decision_ease",
      "mastery"
    ],
    "effort": "medium",
    "time": "5-15",
    "cost": "$",
    "when": [
      "evening",
      "boredom",
      "stress"
    ],
    "where": [
      "home"
    ],
    "preserves": [
      "chocolate",
      "cheese",
      "coffee",
      "bread",
      "pasta",
      "meat"
    ],
    "features": [
      "solo_friendly",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety"
    ],
    "personalization_prompt": "Create tomorrow's meal lineup (like planning your dream team!):",
    "personalization_type": "multi_text",
    "personalization_config": {
      "items": [
        {
          "label": "Breakfast MVP:",
          "placeholder": "e.g., Overnight oats with berries"
        },
        {
          "label": "Lunch Champion:",
          "placeholder": "e.g., Leftover soup + salad"
        },
        {
          "label": "Dinner Finale:",
          "placeholder": "e.g., Grilled chicken + veggies"
        }
      ]
    },
    "difficulty": 2,
    "source": "coach_curated",
    "sustainability": "daily",
    "areas": [
      "nutrition",
      "organization"
    ]
  },
  {
    "tip_id": "a1b2c3d4-0025-4025-a025-1234567890c3",
    "micro_version": "Decide on one meat-free meal you'll have this week.",
    "summary": "Try a 'Meat-Free Monday' (or any day) meal.",
    "details_md": "**The Experiment:** For one day this week, or just for one meal, replace the meat with a plant-based protein source.\n\n**Why it Works:** Incorporating more plant-based meals can increase your intake of fiber and beneficial plant compounds, while potentially reducing intake of saturated fat. Beans, peas, and lentils are excellent sources of fiber and protein and are very budget-friendly.\n\n**How to Try It:**\n• Make tacos or chili using black beans or lentils instead of ground beef\n• Try a veggie burger instead of a beef burger\n• Add chickpeas or tofu to a salad or stir-fry for your protein source",
    "area": "eating",
    "goals": [
      "plant_based",
      "increase_veggies",
      "improve_gut_health",
      "better_lipids",
      "less_processed_food"
    ],
    "contraindications": [],
    "mechanisms": [
      "healthy_swap",
      "novelty",
      "sensory"
    ],
    "effort": "low",
    "time": "15-30min",
    "cost": "$",
    "when": [
      "evening",
      "meal_time"
    ],
    "where": [
      "home",
      "restaurant"
    ],
    "preserves": [
      "cheese",
      "bread",
      "pasta"
    ],
    "satisfies": [
      "soft",
      "chewy"
    ],
    "requires": [
      "basic_stove",
      "basic_cooking",
      "advance_planning"
    ],
    "features": [
      "veggie_moderate",
      "diet_trauma_safe",
      "not_diety"
    ],
    "personalization_prompt": "What plant-powered meal will you try?",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Black bean tacos",
        "Lentil bolognese",
        "Chickpea curry",
        "Veggie burger",
        "Tofu stir-fry",
        "Bean chili"
      ],
      "multiple": false
    },
    "difficulty": 2,
    "source": "dietitian_reviewed",
    "sustainability": "occasional",
    "areas": [
      "nutrition",
      "organization"
    ],
    "helps_with": []
  },
  {
    "tip_id": "b2c3d4e5-f6a7-8901-bcde-234567890123",
    "micro_version": "Add salmon or sardines to your grocery list.",
    "summary": "Eat one serving of oily fish this week.",
    "details_md": "**The Experiment:** Plan to have at least one meal this week that includes a serving of oily fish like salmon, mackerel, or sardines.\n\n**Why it Works:** Oily fish are rich in omega-3 fatty acids (EPA/DHA), which are crucial for heart and brain health and have anti-inflammatory properties. Most people don't get enough of these beneficial fats. Aim for 2-3 servings weekly.\n\n**How to Try It:**\n• Grill or bake a salmon fillet for dinner\n• Add canned sardines or mackerel to a salad for a quick and easy lunch\n• Choose from fresh, frozen, or canned options to fit your budget and convenience needs",
    "area": "eating",
    "goals": [
      "better_lipids",
      "nutrient_density",
      "improve_energy"
    ],
    "contraindications": [
      "fish_allergy",
      "shellfish_allergy"
    ],
    "mechanisms": [
      "planning_ahead",
      "healthy_swap",
      "mastery"
    ],
    "effort": "low",
    "time": "5-15",
    "cost": "$",
    "when": [
      "evening",
      "afternoon",
      "meal_time"
    ],
    "where": [
      "home",
      "restaurant"
    ],
    "involves": [
      "meat"
    ],
    "preserves": [
      "bread",
      "pasta",
      "cheese"
    ],
    "satisfies": [
      "soft"
    ],
    "requires": [
      "basic_stove",
      "full_kitchen",
      "intermediate_cooking",
      "advance_planning"
    ],
    "features": [
      "veggie_free",
      "partner_resistant_ok",
      "diet_trauma_safe",
      "not_diety"
    ],
    "personalization_prompt": "Which omega-3 superstar will you try?",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Grilled salmon",
        "Sardines on salad",
        "Mackerel sandwich",
        "Tuna steaks",
        "Smoked salmon bagel"
      ],
      "multiple": false
    },
    "difficulty": 3,
    "source": "dietitian_reviewed",
    "sustainability": "occasional",
    "areas": [
      "nutrition",
      "organization"
    ],
    "helps_with": []
  },
  {
    "tip_id": "a1b2c3d4-0031-4031-a031-1234567890c9",
    "micro_version": "Put rice, quinoa, or farro on your grocery list.",
    "summary": "Batch cook a versatile grain for the week.",
    "details_md": "**The Experiment:** Cook a large batch of a whole grain like quinoa, brown rice, or farro to use in meals throughout the week.\n\n**Why it Works:** Having a pre-cooked, healthy carbohydrate source ready in the fridge makes assembling quick meals effortless. It's a foundational meal prep task that saves time and makes it easy to build balanced bowls, salads, and side dishes without starting from scratch each time.\n\n**How to Try It:**\n• Cook 2-3 cups of your chosen grain according to package directions\n• Let it cool completely, then store it in an airtight container in the refrigerator for up to 4 days\n• Add a scoop to salads, use as a base for a \"rice bowl,\" or serve as a side with protein and veggies",
    "area": "eating",
    "goals": [
      "weight_loss",
      "less_processed_food",
      "improve_gut_health",
      "plant_based"
    ],
    "contraindications": [
      "celiac"
    ],
    "mechanisms": [
      "planning_ahead",
      "environment_design",
      "decision_ease",
      "mastery"
    ],
    "effort": "low",
    "time": "15-30min",
    "cost": "$",
    "when": [
      "evening",
      "meal_time"
    ],
    "where": [
      "home"
    ],
    "involves": [
      "carbs"
    ],
    "preserves": [
      "meat",
      "cheese"
    ],
    "satisfies": [
      "chewy"
    ],
    "requires": [
      "basic_stove",
      "basic_cooking",
      "advance_planning",
      "advance_prep"
    ],
    "features": [
      "solo_friendly",
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "diet_trauma_safe",
      "not_diety"
    ],
    "personalization_prompt": "What grain will be your meal prep MVP?",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Quinoa Queen",
        "Brown Rice Boss",
        "Farro Fantastic",
        "Wild Rice Warrior",
        "Barley Buddy"
      ],
      "multiple": false
    },
    "difficulty": 3,
    "source": "coach_curated",
    "sustainability": "daily",
    "areas": [
      "nutrition",
      "organization"
    ],
    "helps_with": []
  },
  {
    "tip_id": "a5b6c7d8-e9f0-1234-abcd-567890123456",
    "micro_version": "Rate your hunger from 1-10 before your next meal.",
    "summary": "Use the 'Hunger Scale' before you eat.",
    "details_md": "**The Experiment:** Before you eat a meal or snack, take a moment to rate your physical hunger on a scale of 1 to 10.\n\n**Why it Works:** This mindful eating exercise helps you distinguish between true, physical hunger and emotional or environmental triggers for eating (like stress or seeing food). Practicing this helps you become more attuned to your body's signals, leading to more intentional eating choices.\n\n**How to Try It:**\n• Use a simple scale: 1 = ravenous, 5 = neutral, 10 = uncomfortably stuffed\n• Pause before eating and ask, \"What number am I right now?\"\n• Ideally, you want to start eating around a 3 or 4 (\"getting hungry\") and stop around a 6 or 7 (\"pleasantly satisfied\")",
    "area": "eating",
    "goals": [
      "weight_loss",
      "improve_energy"
    ],
    "helps_with": [
      "emotional_eating",
      "stress_eating",
      "boredom_eating",
      "mindless_eating"
    ],
    "contraindications": [],
    "mechanisms": [
      "self_monitoring",
      "skill_building",
      "mindset_shift",
      "mastery"
    ],
    "effort": "medium",
    "time": "0-5",
    "cost": "$",
    "when": [
      "morning",
      "afternoon",
      "evening",
      "late_night",
      "meal_time",
      "snack_time",
      "craving_event"
    ],
    "where": [
      "home",
      "work",
      "restaurant"
    ],
    "preserves": [
      "chocolate",
      "cheese",
      "coffee",
      "bread",
      "pasta",
      "meat"
    ],
    "features": [
      "family_friendly",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly"
    ],
    "personalization_prompt": "Give your hunger levels fun, memorable names! Like Lion (1), Kitty (5), and Sloth (10). What would you call yours?",
    "personalization_type": "scale",
    "personalization_config": {
      "scale_customization": "names",
      "scale_labels": [
        "Extremely hungry - stomach growling",
        "Satisfied - comfortable and content",
        "Overly full - uncomfortably stuffed"
      ],
      "placeholders": [
        "e.g., Lion",
        "e.g., Kitty",
        "e.g., Sloth"
      ]
    },
    "difficulty": 2,
    "source": "dietitian_reviewed",
    "sustainability": "daily"
  },
  {
    "tip_id": "a1b2c3d4-0033-4033-a033-1234567890cb",
    "micro_version": "Open a can of beans and add them to your next meal.",
    "summary": "Add beans or lentils to a soup or salad.",
    "details_md": "**The Experiment:** Boost the fiber and protein content of a soup or salad by adding a half-cup of canned beans or lentils.\n\n**Why it Works:** Beans and lentils are nutritional powerhouses—high in plant-based protein, dietary fiber, and various micronutrients. Adding them to a meal makes it more filling and satisfying, helping with appetite control. They are also an inexpensive way to add bulk and nutrition.\n\n**How to Try It:**\n• Drain and rinse a can of chickpeas, black beans, or kidney beans\n• Toss them into your favorite green salad\n• Stir them into a canned or homemade soup to make it heartier",
    "area": "eating",
    "goals": [
      "plant_based",
      "increase_veggies",
      "improve_gut_health",
      "weight_loss",
      "muscle_gain"
    ],
    "contraindications": [],
    "mechanisms": [
      "healthy_swap",
      "decision_ease",
      "sensory"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "afternoon",
      "evening",
      "meal_time"
    ],
    "where": [
      "home"
    ],
    "preserves": [
      "cheese",
      "bread",
      "meat"
    ],
    "satisfies": [
      "soft",
      "creamy"
    ],
    "features": [
      "solo_friendly",
      "veggie_light",
      "family_friendly",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "Which legume will boost your meal?",
    "personalization_type": "multi_text",
    "personalization_config": {
      "items": [
        {
          "label": "Bean choice:",
          "placeholder": "e.g., Chickpeas, black beans, lentils"
        },
        {
          "label": "Add to what:",
          "placeholder": "e.g., Garden salad, minestrone soup"
        }
      ]
    },
    "difficulty": 1,
    "source": "dietitian_reviewed",
    "sustainability": "daily",
    "areas": [
      "nutrition",
      "organization"
    ],
    "helps_with": []
  },
  {
    "tip_id": "a1b2c3d4-0034-4034-a034-1234567890cc",
    "micro_version": "Put a protein bar or bag of nuts in your bag right now.",
    "summary": "Pack an 'emergency' healthy snack in your bag.",
    "details_md": "**The Experiment:** Place a non-perishable, healthy snack in your work bag, car, or purse to have on hand for unexpected hunger.\n\n**Why it Works:** This is an environmental design strategy to combat \"hanger\" and prevent you from grabbing a convenient but unhealthy option when you're out and about. Having a planned, healthy option available removes the decision-making process when you're low on energy and willpower.\n\n**How to Try It:**\n• Good options include a small bag of almonds, a protein bar, or a packet of nut butter\n• Choose something you genuinely like so you'll be happy to eat it\n• Replace it as soon as you use it so you're always prepared",
    "area": "eating",
    "goals": [
      "weight_loss",
      "reduce_sugar",
      "less_processed_food"
    ],
    "helps_with": [
      "mindless_eating",
      "stress_eating"
    ],
    "contraindications": [
      "nut_allergy"
    ],
    "mechanisms": [
      "planning_ahead",
      "environment_design",
      "decision_ease",
      "comfort"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "morning",
      "afternoon",
      "craving_event",
      "stress"
    ],
    "where": [
      "work",
      "commute",
      "travel"
    ],
    "requires": [
      "advance_planning"
    ],
    "features": [
      "solo_friendly",
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "What healthy snack will you pack?",
    "personalization_type": "text",
    "personalization_config": {
      "placeholders": [
        "e.g., almonds, protein bar, apple slices"
      ]
    },
    "difficulty": 1,
    "source": "coach_curated",
    "sustainability": "daily",
    "areas": [
      "nutrition",
      "organization"
    ]
  },
  {
    "tip_id": "a1b2c3d4-0035-4035-a035-1234567890cd",
    "micro_version": "Put frozen veggies in your cart next grocery trip.",
    "summary": "Replace cooking oil with a spray for one meal.",
    "details_md": "**The Experiment:** When pan-frying or sautéing a meal, use an oil spray instead of pouring oil from a bottle.\n\n**Why it Works:** Cooking oils are very calorie-dense (a tablespoon of olive oil has about 120 calories). While healthy fats are important, it's easy to pour far more than you need, adding hundreds of hidden calories to a meal. Using a spray provides a much lighter coating, significantly reducing the overall calorie count without sacrificing the non-stick benefit.\n\n**How to Try It:**\n• Use a commercial oil spray (like avocado or olive oil spray)\n• Lightly coat the surface of your pan just until it shimmers\n• This is a great trick for sautéing vegetables or cooking eggs or lean proteins",
    "area": "eating",
    "goals": [
      "weight_loss",
      "better_lipids"
    ],
    "contraindications": [],
    "mechanisms": [
      "healthy_swap",
      "skill_building",
      "decision_ease"
    ],
    "effort": "low",
    "time": "0-5",
    "cost": "$",
    "when": [
      "morning",
      "afternoon",
      "evening",
      "meal_time"
    ],
    "where": [
      "home"
    ],
    "preserves": [
      "meat",
      "cheese",
      "bread",
      "pasta"
    ],
    "requires": [
      "basic_stove",
      "basic_cooking"
    ],
    "features": [
      "solo_friendly",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "chaos_proof"
    ],
    "personalization_prompt": "What dish will get the spray treatment?",
    "personalization_type": "text",
    "personalization_config": {
      "placeholder": "e.g., Morning scrambled eggs, stir-fry veggies"
    },
    "difficulty": 2,
    "source": "coach_curated",
    "sustainability": "daily",
    "helps_with": []
  },
  {
    "tip_id": "c5d6e7f8-a9b0-1234-cdef-567890123456",
    "micro_version": "Add zucchini or spaghetti squash to your grocery list.",
    "summary": "Try a vegetable-based 'noodle' instead of pasta.",
    "details_md": "**The Experiment:** For a pasta-style dish, try substituting traditional pasta with a vegetable alternative like spaghetti squash or zucchini noodles (\"zoodles\").\n\n**Why it Works:** This swap dramatically increases the vegetable and fiber content of your meal while significantly reducing the carbohydrate and calorie load. It's a great way to enjoy the comfort of a pasta dish while working towards goals like weight loss or increased vegetable intake.\n\n**How to Try It:**\n• You can buy pre-spiralized zucchini noodles or make your own with a spiralizer. Sauté them for a few minutes until tender\n• For spaghetti squash, cut it in half, roast it, and then scrape out the noodle-like strands with a fork\n• Top with your favorite sauce and protein",
    "area": "eating",
    "goals": [
      "increase_veggies",
      "weight_loss",
      "reduce_sugar",
      "plant_based",
      "less_processed_food"
    ],
    "contraindications": [],
    "mechanisms": [
      "healthy_swap",
      "novelty",
      "skill_building",
      "sensory",
      "mastery"
    ],
    "effort": "medium",
    "time": "15-30min",
    "cost": "$",
    "when": [
      "afternoon",
      "evening",
      "meal_time"
    ],
    "where": [
      "home"
    ],
    "preserves": [
      "cheese",
      "meat"
    ],
    "satisfies": [
      "carbs",
      "soft"
    ],
    "requires": [
      "full_kitchen",
      "intermediate_cooking",
      "advance_planning"
    ],
    "features": [
      "veggie_heavy"
    ],
    "personalization_prompt": "Pick your veggie noodle adventure:",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Zucchini zoodles",
        "Spaghetti squash strands",
        "Carrot ribbons",
        "Sweet potato spirals",
        "Cucumber noodles"
      ],
      "multiple": false
    },
    "difficulty": 3,
    "source": "community_submitted",
    "sustainability": "occasional",
    "helps_with": []
  },
  {
    "tip_id": "a1b2c3d4-0037-4037-a037-1234567890cf",
    "micro_version": "Add one fiber-rich food to your next meal.",
    "summary": "Deconstruct your craving: What do you really want?",
    "details_md": "**The Experiment:** When a strong craving hits, pause for a moment and try to identify the sensory properties you're seeking. Is it crunchy, creamy, sweet, salty, or warm?\n\n**Why it Works:** Cravings are often for a specific texture or sensation, not necessarily a specific unhealthy food. By deconstructing the craving, you can find a healthier alternative that satisfies the sensory need. This is a mindful approach to managing cravings rather than just suppressing them.\n\n**How to Try It:**\n• Craving crunchy and salty chips? Try roasted chickpeas or celery sticks with salt\n• Craving creamy ice cream? Try a bowl of rich Greek yogurt with berries\n• Craving something sweet? Try a piece of fruit or a date",
    "area": "eating",
    "goals": [
      "reduce_sugar",
      "weight_loss"
    ],
    "helps_with": [
      "emotional_eating",
      "stress_eating",
      "boredom_eating"
    ],
    "contraindications": [],
    "mechanisms": [
      "crave_buster",
      "mindset_shift",
      "mastery"
    ],
    "effort": "medium",
    "time": "0-5",
    "cost": "$",
    "when": [
      "afternoon",
      "evening",
      "late_night",
      "craving_event",
      "boredom",
      "stress"
    ],
    "where": [
      "home",
      "work"
    ],
    "features": [
      "solo_friendly",
      "family_friendly",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "Predict what you think you'll crave (compare later to what it actually is):",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Salty",
        "Sweet",
        "Savory",
        "Crunchy",
        "Chewy",
        "Creamy"
      ],
      "multiple": true,
      "style": "buttons"
    },
    "difficulty": 2,
    "source": "coach_curated",
    "sustainability": "daily"
  },
  {
    "tip_id": "f6a7b8c9-d0e1-2345-fabc-678901234567",
    "micro_version": "Eat the protein on your plate before the carbs.",
    "summary": "Eat your protein and veggies first.",
    "details_md": "**The Experiment:** At one meal today, eat the protein and vegetables on your plate before you eat the carbohydrates.\n\n**Why it Works:** Starting with high-fiber vegetables and satiating protein can help you feel fuller sooner. By the time you get to the more calorie-dense carbs, you may be satisfied with a smaller portion. This meal sequencing can trigger satiety hormones and improve post-meal energy levels.\n\n**How to Try It:**\n• If you have a side salad, eat it first\n• If you have roasted broccoli and chicken, finish all the broccoli and chicken before starting the rice or potatoes\n• This works especially well at restaurants when bread is served—ask for a side salad to eat while you wait",
    "area": "eating",
    "goals": [
      "weight_loss",
      "increase_veggies",
      "improve_gut_health",
      "improve_energy",
      "better_lipids"
    ],
    "helps_with": [
      "speed_eating",
      "mindless_eating"
    ],
    "contraindications": [],
    "mechanisms": [
      "mindset_shift",
      "habit_stacking",
      "skill_building",
      "decision_ease",
      "mastery"
    ],
    "effort": "low",
    "time": "0-5",
    "cost": "$",
    "when": [
      "afternoon",
      "evening",
      "meal_time"
    ],
    "where": [
      "home",
      "restaurant"
    ],
    "preserves": [
      "bread",
      "pasta",
      "cheese"
    ],
    "features": [
      "veggie_moderate",
      "family_friendly",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety"
    ],
    "personalization_prompt": "At which meal will you try the protein-veggie-first strategy?",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Breakfast",
        "Lunch",
        "Dinner",
        "Restaurant meal"
      ],
      "multiple": false
    },
    "difficulty": 2,
    "source": "community_submitted",
    "sustainability": "daily"
  },
  {
    "tip_id": "a1b2c3d4-0039-4039-a039-1234567890d1",
    "micro_version": "Fill your water bottle and place it directly in front of your keyboard.",
    "summary": "Keep a water bottle on your desk or in sight all day.",
    "details_md": "**The Experiment:** For the entire day, keep a filled water bottle within your arm's reach and line of sight.\n\n**Why it Works:** This is a classic and highly effective environmental design tactic. The visual cue of the water bottle constantly reminds you to drink, and its accessibility makes it effortless to do so. This simple change can dramatically increase your daily water intake.\n\n**How to Try It:**\n• Choose a reusable water bottle you enjoy using\n• Fill it up first thing in the morning and place it on your desk, in your car's cup holder, or on the counter where you spend the most time\n• Refill it as soon as it's empty",
    "area": "eating",
    "goals": [
      "improve_hydration"
    ],
    "contraindications": [],
    "mechanisms": [
      "environment_design",
      "decision_ease"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "morning",
      "afternoon",
      "evening"
    ],
    "where": [
      "home",
      "work",
      "commute",
      "travel"
    ],
    "preserves": [
      "chocolate",
      "cheese",
      "coffee",
      "bread",
      "pasta",
      "meat",
      "soda"
    ],
    "features": [
      "solo_friendly",
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "Where will your water bottle live today?",
    "personalization_type": "text",
    "personalization_config": {
      "placeholder": "e.g., Right side of desk, kitchen counter, car cup holder"
    },
    "difficulty": 1,
    "source": "dietitian_reviewed",
    "sustainability": "daily",
    "helps_with": []
  },
  {
    "tip_id": "a1b2c3d4-0040-4040-a040-1234567890d2",
    "micro_version": "Put one scoop of protein powder in a bowl.",
    "summary": "Create a \"protein pudding\" for a sweet treat.",
    "details_md": "**The Experiment:** Make a healthy, high-protein dessert by creating a \"pudding\" from protein powder.\n\n**Why it Works:** This satisfies a craving for a sweet, creamy dessert while providing a significant dose of protein, which is much more satiating and beneficial for body composition than a typical sugar-laden pudding. It's a smart swap that aligns with fitness goals.\n\n**How to Try It:**\n• In a small bowl, mix one scoop of casein or a whey/casein blend protein powder (vanilla or chocolate work well) with a very small amount of milk or water\n• Stir vigorously, adding liquid a tiny bit at a time, until it reaches a thick, pudding-like consistency\n• For best results, chill in the freezer for 10-15 minutes",
    "area": "eating",
    "goals": [
      "reduce_sugar",
      "muscle_gain",
      "weight_loss"
    ],
    "helps_with": [
      "sugar_addiction",
      "night_eating"
    ],
    "contraindications": [
      "lactose_intolerance",
      "soy_allergy"
    ],
    "mechanisms": [
      "healthy_swap",
      "crave_buster",
      "sensory",
      "mastery"
    ],
    "effort": "low",
    "time": "0-5",
    "cost": "$",
    "when": [
      "evening",
      "late_night",
      "craving_event"
    ],
    "where": [
      "home"
    ],
    "involves": [
      "dairy"
    ],
    "preserves": [
      "chocolate"
    ],
    "satisfies": [
      "sweet",
      "chocolate",
      "creamy"
    ],
    "features": [
      "solo_friendly",
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "What flavor protein pudding will you make?",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Chocolate Dream",
        "Vanilla Delight",
        "Strawberry Swirl",
        "Peanut Butter Paradise",
        "Cookies & Cream"
      ],
      "multiple": false
    },
    "difficulty": 2,
    "source": "coach_curated",
    "sustainability": "daily"
  },
  {
    "tip_id": "a1b2c3d4-0041-4041-a041-1234567890d3",
    "micro_version": "Open a can of chickpeas and drain them.",
    "summary": "Make a 'no-tuna' chickpea salad sandwich.",
    "details_md": "**The Experiment:** For a plant-based lunch, try making a \"tuna-style\" salad using mashed chickpeas instead of tuna.\n\n**Why it Works:** This is a creative, plant-based swap that mimics a familiar comfort food. Chickpeas provide plant-based protein and a high amount of fiber, making it a filling and gut-healthy option. It's a great way to reduce meat consumption and increase plant food variety.\n\n**How to Try It:**\n• Drain a can of chickpeas and mash them with a fork\n• Mix with a small amount of Greek yogurt or vegan mayo, chopped celery, red onion, and seasonings like dill, salt, and pepper\n• Serve on whole-wheat bread or with crackers",
    "area": "eating",
    "goals": [
      "plant_based",
      "increase_veggies",
      "improve_gut_health",
      "better_lipids"
    ],
    "contraindications": [
      "lactose_intolerance"
    ],
    "mechanisms": [
      "healthy_swap",
      "novelty",
      "sensory"
    ],
    "effort": "low",
    "time": "5-15",
    "cost": "$",
    "when": [
      "afternoon",
      "meal_time"
    ],
    "where": [
      "home"
    ],
    "preserves": [
      "bread",
      "cheese"
    ],
    "satisfies": [
      "soft",
      "creamy"
    ],
    "requires": [
      "basic_cooking"
    ],
    "features": [
      "solo_friendly",
      "veggie_light",
      "family_friendly",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety"
    ],
    "personalization_prompt": "What will you add to your chickpea salad?",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Classic (celery + onion)",
        "Mediterranean (olives + sun-dried tomatoes)",
        "Curry spiced",
        "Dill pickle relish",
        "Everything bagel seasoning"
      ],
      "multiple": true
    },
    "difficulty": 2,
    "source": "coach_curated",
    "sustainability": "occasional",
    "areas": [
      "nutrition",
      "organization"
    ],
    "helps_with": []
  },
  {
    "tip_id": "e7f8a9b0-c1d2-3456-efab-789012345678",
    "micro_version": "Calculate your bedtime minus 3 hours (e.g., 11pm - 3 = 8pm) and set an alarm.",
    "summary": "Stop eating 2-3 hours before bedtime.",
    "details_md": "**The Experiment:** Tonight, try to finish your last meal or snack at least 2-3 hours before you go to sleep.\n\n**Why it Works:** This creates a mini-fasting window overnight. For some people, this can improve sleep quality and digestion. It also helps to prevent late-night, often mindless, snacking which can contribute significant calories. Chronobiology research shows late-night eating can disrupt circadian rhythms.\n\n**How to Try It:**\n• If you normally go to bed at 11 PM, aim to have your dinner or last snack finished by 8 PM\n• If you feel hungry after this time, try having a glass of water or a cup of herbal tea",
    "area": "eating",
    "goals": [
      "weight_loss",
      "improve_gut_health",
      "improve_energy"
    ],
    "helps_with": [
      "night_eating",
      "mindless_eating"
    ],
    "contraindications": [
      "t1_diabetes",
      "t2_diabetes",
      "pregnancy",
      "breastfeeding"
    ],
    "mechanisms": [
      "time_ritual",
      "crave_buster",
      "mastery",
      "comfort"
    ],
    "effort": "medium",
    "time": "0-5",
    "cost": "$",
    "when": [
      "evening",
      "late_night",
      "meal_time",
      "snack_time"
    ],
    "where": [
      "home"
    ],
    "preserves": [
      "chocolate",
      "cheese",
      "coffee",
      "bread",
      "pasta",
      "meat"
    ],
    "requires": [
      "advance_planning"
    ],
    "features": [
      "solo_friendly"
    ],
    "personalization_prompt": "What time will be your 'kitchen closed' cutoff?",
    "personalization_type": "text",
    "personalization_config": {
      "placeholder": "e.g., 8:00 PM, 7:30 PM"
    },
    "difficulty": 3,
    "source": "dietitian_reviewed",
    "sustainability": "occasional",
    "areas": [
      "nutrition",
      "organization"
    ]
  },
  {
    "tip_id": "a1b2c3d4-0043-4043-a043-1234567890d5",
    "micro_version": "Chop one head of broccoli or cauliflower.",
    "summary": "Roast a big batch of mixed vegetables.",
    "details_md": "**The Experiment:** Roast a large sheet pan of mixed vegetables to have on hand for the next few days.\n\n**Why it Works:** Roasting brings out the natural sweetness of vegetables, making them delicious and appealing. Having a batch ready in the fridge makes it incredibly easy to add a serving of vegetables to any meal—breakfast, lunch, or dinner—with zero prep time.\n\n**How to Try It:**\n• Chop up hearty vegetables like broccoli, cauliflower, bell peppers, onions, and sweet potatoes\n• Toss with a little olive oil, salt, and pepper\n• Roast on a sheet pan at 400°F (200°C) for 20-30 minutes, until tender and slightly browned\n• Add them to eggs, salads, grain bowls, or eat as a side",
    "area": "eating",
    "goals": [
      "increase_veggies",
      "less_processed_food",
      "plant_based",
      "improve_gut_health"
    ],
    "contraindications": [],
    "mechanisms": [
      "planning_ahead",
      "environment_design",
      "decision_ease",
      "sensory"
    ],
    "effort": "low",
    "time": "15-30min",
    "cost": "$",
    "when": [
      "evening",
      "meal_time"
    ],
    "where": [
      "home"
    ],
    "preserves": [
      "meat",
      "cheese",
      "bread",
      "pasta"
    ],
    "satisfies": [
      "soft",
      "crispy"
    ],
    "requires": [
      "full_kitchen",
      "basic_cooking",
      "advance_planning",
      "advance_prep"
    ],
    "features": [
      "solo_friendly",
      "veggie_heavy",
      "family_friendly",
      "partner_resistant_ok",
      "diet_trauma_safe",
      "not_diety"
    ],
    "personalization_prompt": "Design your veggie roasting squad (pick 3-5 players!):",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Broccoli Boss",
        "Cauliflower Captain",
        "Sweet Potato Star",
        "Bell Pepper Hero",
        "Zucchini Warrior",
        "Brussels Sprout Brigade",
        "Carrot Champion",
        "Red Onion Ranger"
      ],
      "multiple": true
    },
    "difficulty": 3,
    "source": "coach_curated",
    "sustainability": "daily",
    "areas": [
      "nutrition",
      "organization"
    ],
    "helps_with": []
  },
  {
    "tip_id": "d4e5f6a7-b8c9-0123-defa-456789012345",
    "micro_version": "Write down exactly what you just ate in your phone.",
    "summary": "Track your food for one day without judgment.",
    "details_md": "**The Experiment:** For just one day, keep a log of everything you eat and drink. Approach it with curiosity, not judgment.\n\n**Why it Works:** People often underestimate their total food intake by a significant margin. A single day of tracking can provide powerful awareness about \"mindless\" nibbles, portion sizes, and hidden calories from drinks and sauces. It's a data-gathering exercise to inform future choices, not a tool for restriction.\n\n**How to Try It:**\n• Use a simple notebook or a free app\n• Be honest and record everything, including the cream in your coffee and the handful of fries you stole from a friend\n• At the end of the day, simply review the log and notice any patterns or surprises",
    "area": "eating",
    "goals": [
      "weight_loss",
      "reduce_sugar",
      "improve_energy"
    ],
    "helps_with": [
      "mindless_eating",
      "grazing",
      "emotional_eating"
    ],
    "contraindications": [],
    "mechanisms": [
      "self_monitoring",
      "skill_building",
      "mastery"
    ],
    "effort": "high",
    "time": "5-15",
    "cost": "$",
    "when": [
      "morning",
      "afternoon",
      "evening",
      "late_night"
    ],
    "where": [
      "home",
      "work",
      "restaurant"
    ],
    "preserves": [
      "chocolate",
      "cheese",
      "coffee",
      "bread",
      "pasta",
      "meat",
      "soda"
    ],
    "features": [
      "solo_friendly",
      "family_friendly",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly"
    ],
    "personalization_prompt": "How will you track your food detective work?",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Pen & paper journal",
        "Phone notes app",
        "Photo diary",
        "Voice memos",
        "Tracking app"
      ],
      "multiple": false
    },
    "difficulty": 4,
    "source": "community_submitted",
    "sustainability": "temporary"
  },
  {
    "tip_id": "a1b2c3d4-0045-4045-a045-1234567890d7",
    "micro_version": "Boil 2 eggs right now so they are ready for breakfast.",
    "summary": "Have a savory, protein-rich breakfast.",
    "details_md": "**The Experiment:** Instead of a sweet breakfast (like sweetened cereal, pastries, or sugary yogurt), opt for a savory one centered around protein.\n\n**Why it Works:** Starting the day with a savory, high-protein meal can help regulate appetite and reduce sugar cravings throughout the day. It sets a different metabolic and psychological tone for the day compared to a breakfast that resembles a dessert.\n\n**How to Try It:**\n• A simple egg scramble with spinach and a side of avocado\n• A whole-wheat tortilla with black beans, salsa, and a fried egg\n• Leftover chicken and roasted vegetables from last night's dinner",
    "area": "eating",
    "goals": [
      "reduce_sugar",
      "weight_loss",
      "muscle_gain",
      "improve_energy"
    ],
    "helps_with": [
      "sugar_addiction",
      "grazing"
    ],
    "contraindications": [
      "egg_allergy"
    ],
    "mechanisms": [
      "healthy_swap",
      "mindset_shift",
      "energy_boost",
      "sensory"
    ],
    "effort": "low",
    "time": "5-15",
    "cost": "$",
    "when": [
      "morning",
      "meal_time"
    ],
    "where": [
      "home"
    ],
    "involves": [
      "meat",
      "dairy"
    ],
    "preserves": [
      "coffee"
    ],
    "satisfies": [
      "soft",
      "creamy"
    ],
    "requires": [
      "basic_stove",
      "basic_cooking"
    ],
    "features": [
      "solo_friendly",
      "veggie_light",
      "family_friendly",
      "partner_resistant_ok",
      "no_planning",
      "diet_trauma_safe",
      "not_diety"
    ],
    "personalization_prompt": "What savory breakfast will power your morning?",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Veggie scramble",
        "Avocado toast + egg",
        "Breakfast burrito",
        "Leftover dinner",
        "Cottage cheese bowl"
      ],
      "multiple": false
    },
    "difficulty": 2,
    "source": "dietitian_reviewed",
    "sustainability": "daily"
  },
  {
    "tip_id": "a1b2c3d4-0046-4046-a046-1234567890d8",
    "micro_version": "Text a friend: \"Hey, want to go for a walk this weekend?\"",
    "summary": "Social Swap: Suggest a walk instead of a coffee/drink meeting.",
    "details_md": "**The Experiment:** For your next casual social catch-up, suggest going for a walk together instead of meeting for coffee, drinks, or a meal.\n\n**Why it Works:** This reframes social time to include physical activity, helping you reach step goals and reduce sedentary time. It decouples socializing from consuming calories, which can be a major source of unplanned intake. It's a great way to connect with friends while supporting your health goals.\n\n**How to Try It:**\n• \"Hey, instead of grabbing coffee, would you be up for a walk around the park on Saturday?\"\n• Take phone calls while walking around your neighborhood\n• This works well for one-on-one meetings or casual friend dates",
    "area": "eating",
    "goals": [
      "weight_loss",
      "improve_energy"
    ],
    "helps_with": [
      "social_eating"
    ],
    "contraindications": [
      "elderly_frailty"
    ],
    "mechanisms": [
      "healthy_swap",
      "environment_design",
      "social",
      "energy_boost"
    ],
    "effort": "low",
    "time": "15-30min",
    "cost": "$",
    "when": [
      "morning",
      "afternoon",
      "evening",
      "social_event"
    ],
    "where": [
      "work",
      "social_event"
    ],
    "preserves": [
      "chocolate",
      "cheese",
      "coffee",
      "bread",
      "pasta",
      "meat",
      "soda"
    ],
    "requires": [
      "advance_planning"
    ],
    "features": [
      "group_activity",
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "diet_trauma_safe",
      "not_diety"
    ],
    "personalization_prompt": "Where will you suggest walking?",
    "personalization_type": "text",
    "personalization_config": {
      "placeholders": [
        "e.g., The park, downtown, nature trail, neighborhood loop"
      ]
    },
    "difficulty": 2,
    "source": "coach_curated",
    "sustainability": "occasional",
    "areas": [
      "nutrition",
      "organization"
    ]
  },
  {
    "tip_id": "a1b2c3d4-0047-4047-a047-1234567890d9",
    "micro_version": "Pour a handful of frozen berries into a bowl.",
    "summary": "Try a \"dessert\" of frozen fruit.",
    "details_md": "**The Experiment:** For a healthy dessert, try eating a bowl of frozen fruit.\n\n**Why it Works:** Frozen fruit like mango chunks, berries, or banana slices have a surprisingly creamy, sorbet-like texture when eaten partially thawed. This satisfies a craving for a cold, sweet treat with all the benefits of whole fruit (fiber, vitamins) and none of the added sugar of ice cream or sorbet.\n\n**How to Try It:**\n• Frozen mango or pineapple chunks are particularly creamy\n• Frozen grapes taste like little bites of sorbet\n• For a \"nice cream,\" blend frozen bananas with a splash of milk until smooth",
    "area": "eating",
    "goals": [
      "reduce_sugar",
      "weight_loss",
      "increase_veggies"
    ],
    "helps_with": [
      "sugar_addiction",
      "night_eating"
    ],
    "contraindications": [],
    "mechanisms": [
      "healthy_swap",
      "crave_buster",
      "sensory",
      "comfort"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "evening",
      "late_night",
      "craving_event"
    ],
    "where": [
      "home"
    ],
    "satisfies": [
      "sweet",
      "creamy",
      "soft"
    ],
    "features": [
      "solo_friendly",
      "veggie_light",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "What frozen fruit will you try as dessert tonight?",
    "personalization_type": "text",
    "personalization_config": {
      "placeholders": [
        "e.g., Frozen grapes, mango chunks, or mixed berries"
      ]
    },
    "difficulty": 1,
    "source": "dietitian_reviewed",
    "sustainability": "daily"
  },
  {
    "tip_id": "a1b2c3d4-0048-4048-a048-1234567890da",
    "micro_version": "Practice putting your fork down completely after your next bite.",
    "summary": "Put your fork down between every few bites.",
    "details_md": "**The Experiment:** During one meal today, consciously place your fork or spoon down on the table after every two or three bites.\n\n**Why it Works:** This is a simple, powerful mechanical trick to slow down your eating pace. It forces a pause, allowing you to breathe, check in with your fullness level, and engage more with your meal and dining companions. It's a key practice for developing mindful eating habits.\n\n**How to Try It:**\n• Take a bite. Chew it thoroughly. Swallow. Then, place your utensil completely down on the plate or table\n• Take a sip of water or a deep breath before picking it up again\n• It will feel awkward at first, but it's highly effective",
    "area": "eating",
    "goals": [
      "weight_loss",
      "improve_gut_health"
    ],
    "helps_with": [
      "speed_eating",
      "mindless_eating"
    ],
    "contraindications": [],
    "mechanisms": [
      "mindset_shift",
      "time_ritual",
      "mastery"
    ],
    "effort": "medium",
    "time": "15-30min",
    "cost": "$",
    "when": [
      "afternoon",
      "evening",
      "meal_time"
    ],
    "where": [
      "home",
      "restaurant"
    ],
    "preserves": [
      "chocolate",
      "cheese",
      "coffee",
      "bread",
      "pasta",
      "meat"
    ],
    "features": [
      "no_planning",
      "impulse_friendly"
    ],
    "personalization_prompt": "At which meal will you practice the fork-down technique?",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Breakfast",
        "Lunch",
        "Dinner",
        "My biggest meal"
      ],
      "multiple": false
    },
    "difficulty": 2,
    "source": "coach_curated",
    "sustainability": "occasional"
  },
  {
    "tip_id": "a1b2c3d4-0049-4049-a049-1234567890db",
    "micro_version": "Put the jar of hemp seeds/walnuts on the dining table.",
    "summary": "Add a healthy \"topper\" to your meal.",
    "details_md": "**The Experiment:** Enhance the nutritional value and flavor of one meal by adding a healthy \"topper.\"\n\n**Why it Works:** Toppers like seeds, nuts, or nutritional yeast are a simple way to add a boost of healthy fats, protein, fiber, or vitamins without fundamentally changing the meal. It's an easy \"add-in\" strategy that improves nutrient density.\n\n**How to Try It:**\n• Sprinkle a tablespoon of hemp seeds or chopped walnuts on a salad or yogurt bowl\n• Add a spoonful of nutritional yeast to scrambled eggs or popcorn for a cheesy, B-vitamin-rich flavor\n• Top a soup or chili with fresh chopped herbs like cilantro or parsley",
    "area": "eating",
    "goals": [
      "increase_veggies",
      "better_lipids",
      "improve_gut_health",
      "muscle_gain"
    ],
    "contraindications": [
      "nut_allergy"
    ],
    "mechanisms": [
      "healthy_swap",
      "skill_building",
      "sensory",
      "mastery"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "morning",
      "afternoon",
      "evening",
      "meal_time"
    ],
    "where": [
      "home"
    ],
    "preserves": [
      "chocolate",
      "cheese",
      "coffee",
      "bread",
      "pasta",
      "meat"
    ],
    "satisfies": [
      "crunchy"
    ],
    "features": [
      "solo_friendly",
      "veggie_light",
      "family_friendly",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "Choose your power-up topper team:",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Hemp hearts",
        "Chia seeds",
        "Nutritional yeast",
        "Chopped nuts",
        "Fresh herbs",
        "Pumpkin seeds"
      ],
      "multiple": true
    },
    "difficulty": 1,
    "source": "coach_curated",
    "sustainability": "daily",
    "helps_with": []
  },
  {
    "tip_id": "a1b2c3d4-0050-4050-a050-1234567890dc",
    "micro_version": "Write down one thing you ate today that made you feel good.",
    "summary": "Review your day and plan one small improvement for tomorrow.",
    "details_md": "**The Experiment:** Before bed, take two minutes to reflect on your eating today. Identify one small, positive change you could make tomorrow.\n\n**Why it Works:** This practice of reflection and gentle course-correction fosters a growth mindset and prevents the all-or-nothing trap. It's about continuous, incremental improvement rather than perfection. Planning one small change makes the goal feel achievable and builds momentum over time.\n\n**How to Try It:**\n• Ask yourself: \"What went well today? What was one challenge?\"\n• Based on that, decide on one tiny improvement. Examples: \"Tomorrow, I'll pack an apple for my afternoon snack,\" or \"I'll try to have a glass of water with lunch.\"\n• Write it down to increase your commitment",
    "area": "eating",
    "goals": [
      "weight_loss"
    ],
    "helps_with": [
      "mindless_eating",
      "emotional_eating"
    ],
    "contraindications": [],
    "mechanisms": [
      "self_monitoring",
      "mindset_shift",
      "planning_ahead",
      "mastery"
    ],
    "effort": "medium",
    "time": "0-5",
    "cost": "$",
    "when": [
      "late_night",
      "stress"
    ],
    "where": [
      "home"
    ],
    "preserves": [
      "chocolate",
      "cheese",
      "coffee",
      "bread",
      "pasta",
      "meat",
      "soda"
    ],
    "features": [
      "solo_friendly",
      "family_friendly",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety"
    ],
    "personalization_prompt": "What one small win will you aim for tomorrow?",
    "personalization_type": "text",
    "personalization_config": {
      "placeholder": "e.g., Pack an apple, drink water with lunch, add veggies to dinner"
    },
    "difficulty": 4,
    "source": "coach_curated",
    "sustainability": "daily",
    "areas": [
      "nutrition",
      "organization"
    ]
  },
  {
    "tip_id": "b2c3d4e5-f6a7-8901-bcde-f23456789012",
    "micro_version": "Drink one large glass of water now if you are exercising later.",
    "summary": "Drink 16-20oz Water 2-3 Hours Pre-Workout.",
    "details_md": "**The Experiment:** Consume 16-20oz (about 500ml) of water 2-3 hours before training, plus another 8oz (240ml) 30 minutes prior.\n\n**Why it Works:** Pre-hydration ensures optimal blood volume for nutrient delivery and thermoregulation. This timing allows for absorption while preventing GI discomfort during exercise. Monitor urine color - pale yellow is ideal.\n\n**How to Try It:**\n• Set a reminder on your phone for 2-3 hours before your planned workout\n• Drink a large glass of water\n• Have another glass as you're getting ready to go",
    "area": "eating",
    "goals": [
      "improve_hydration",
      "endurance_performance",
      "strength_performance",
      "improve_energy"
    ],
    "contraindications": [],
    "mechanisms": [
      "planning_ahead",
      "time_ritual",
      "energy_boost",
      "mastery"
    ],
    "effort": "low",
    "time": "0-5",
    "cost": "$",
    "when": [
      "morning",
      "afternoon",
      "evening",
      "meal_time"
    ],
    "where": [
      "home",
      "work"
    ],
    "preserves": [
      "chocolate",
      "cheese",
      "coffee",
      "bread",
      "pasta",
      "meat"
    ],
    "requires": [
      "advance_planning"
    ],
    "features": [
      "solo_friendly",
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "diet_trauma_safe",
      "not_diety"
    ],
    "personalization_prompt": "When is your workout and when will you pre-hydrate?",
    "personalization_type": "multi_text",
    "personalization_config": {
      "items": [
        {
          "label": "Workout time:",
          "placeholder": "e.g., 6 PM"
        },
        {
          "label": "Hydration time:",
          "placeholder": "e.g., 3 PM"
        }
      ]
    },
    "difficulty": 2,
    "source": "coach_curated",
    "sustainability": "daily",
    "areas": [
      "fitness",
      "nutrition",
      "organization"
    ],
    "helps_with": []
  },
  {
    "tip_id": "c3d4e5f6-a7b8-9012-cdef-345678901234",
    "micro_version": "Set a timer for 20 minutes before your usual dinner time.",
    "summary": "Practice Pre-Meal Water: 8-16oz Before Eating.",
    "details_md": "**The Experiment:** Consume 8-16 oz (1-2 glasses) of water 20-30 minutes before each main meal.\n\n**Why it Works:** Research shows pre-meal water consumption aids digestion, helps with portion control by increasing satiety signals, and contributes to daily hydration needs.\n\n**How to Try It:**\n• Set a timer for 30 minutes before you typically eat lunch and dinner\n• Drink a glass of water when the timer goes off\n• This simple habit can naturally reduce caloric intake",
    "area": "eating",
    "goals": [
      "improve_hydration",
      "weight_loss",
      "improve_gut_health"
    ],
    "helps_with": [
      "speed_eating",
      "mindless_eating"
    ],
    "contraindications": [],
    "mechanisms": [
      "habit_stacking",
      "time_ritual",
      "decision_ease",
      "comfort"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "morning",
      "afternoon",
      "evening",
      "meal_time"
    ],
    "where": [
      "home",
      "work",
      "restaurant"
    ],
    "preserves": [
      "chocolate",
      "cheese",
      "coffee",
      "bread",
      "pasta",
      "meat"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "Before which meal will you drink water today?",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Breakfast",
        "Lunch",
        "Dinner",
        "All three!"
      ],
      "multiple": false
    },
    "difficulty": 1,
    "source": "dietitian_reviewed",
    "sustainability": "daily"
  },
  {
    "tip_id": "e5f6a7b8-c9d0-1234-efab-567890123456",
    "micro_version": "Add a pinch of salt to your water bottle now.",
    "summary": "Add Pinch of Salt to Water During 60+ Minute Workouts.",
    "details_md": "**The Experiment:** Add 1/4 tsp of salt to every 16-20oz (500ml) of water you drink during extended, sweaty exercise sessions (60+ minutes).\n\n**Why it Works:** Sodium enhances fluid absorption and replaces key electrolytes lost through sweat. This improves hydration efficiency compared to plain water during long workouts, especially in hot weather or for heavy sweaters.\n\n**How to Try It:**\n• Pre-mix your workout water bottle with a small pinch of salt\n• This is not necessary for casual, short-duration exercise",
    "area": "eating",
    "goals": [
      "improve_hydration",
      "endurance_performance"
    ],
    "contraindications": [
      "hypertension",
      "kidney_disease"
    ],
    "mechanisms": [
      "healthy_swap",
      "skill_building",
      "energy_boost",
      "mastery"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "morning",
      "afternoon",
      "evening",
      "meal_time"
    ],
    "where": [
      "home",
      "work"
    ],
    "preserves": [
      "chocolate",
      "cheese",
      "coffee",
      "bread",
      "pasta",
      "meat"
    ],
    "requires": [
      "advance_planning"
    ],
    "features": [
      "solo_friendly",
      "veggie_free",
      "family_friendly",
      "partner_resistant_ok",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "What long workout will you try the salt water hack with?",
    "personalization_type": "text",
    "personalization_config": {
      "placeholder": "e.g., Saturday long run, bike ride, tennis match"
    },
    "difficulty": 2,
    "source": "coach_curated",
    "sustainability": "occasional",
    "areas": [
      "fitness",
      "nutrition"
    ],
    "helps_with": []
  },
  {
    "tip_id": "b8c9d0e1-f2a3-4567-bcde-890123456789",
    "micro_version": "Check the label of your next meal's protein source to see the grams.",
    "summary": "Practice Protein Distribution: 25-30g Per Meal.",
    "details_md": "**The Experiment:** Aim for 25-30g of protein at breakfast, lunch, and dinner today.\n\n**Why it Works:** Sports nutrition research shows distributed protein intake increases muscle protein synthesis by 25% compared to concentrating it in one meal. This supports muscle maintenance, satiety, and metabolic health throughout the day.\n\n**How to Try It:**\n• Use the \"palm-size\" portion guide for a rough estimate\n• Include a protein source at each meal (e.g., eggs for breakfast, chicken for lunch, fish for dinner)",
    "area": "eating",
    "goals": [
      "muscle_gain",
      "weight_loss",
      "improve_energy",
      "strength_performance"
    ],
    "helps_with": [
      "grazing"
    ],
    "contraindications": [
      "kidney_disease"
    ],
    "mechanisms": [
      "planning_ahead",
      "skill_building",
      "mastery",
      "energy_boost"
    ],
    "effort": "medium",
    "time": "5-15",
    "cost": "$",
    "when": [
      "morning",
      "afternoon",
      "evening",
      "meal_time"
    ],
    "where": [
      "home",
      "work"
    ],
    "involves": [
      "meat",
      "dairy"
    ],
    "preserves": [
      "chocolate",
      "coffee",
      "bread",
      "pasta"
    ],
    "requires": [
      "basic_stove",
      "basic_cooking",
      "advance_planning"
    ],
    "features": [
      "solo_friendly",
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "diet_trauma_safe",
      "not_diety"
    ],
    "personalization_prompt": "Map out your protein game plan for the day (aim for 25-30g each meal):",
    "personalization_type": "multi_text",
    "personalization_config": {
      "items": [
        {
          "label": "Breakfast protein:",
          "placeholder": "e.g., 3 eggs + Greek yogurt = 27g"
        },
        {
          "label": "Lunch protein:",
          "placeholder": "e.g., Grilled chicken salad = 30g"
        },
        {
          "label": "Dinner protein:",
          "placeholder": "e.g., Salmon fillet = 28g"
        }
      ]
    },
    "difficulty": 3,
    "source": "dietitian_reviewed",
    "sustainability": "daily"
  },
  {
    "tip_id": "c9d0e1f2-a3b4-5678-cdef-901234567890",
    "micro_version": "Buy a tub of cottage cheese.",
    "summary": "Try Pre-Sleep Protein: 20-40g Before Bed.",
    "details_md": "**The Experiment:** Consume a source of slow-digesting protein 30-90 minutes before you go to sleep.\n\n**Why it Works:** Overnight muscle protein synthesis occurs during sleep. Casein protein, found in dairy, provides a sustained release of amino acids for 7+ hours, supporting muscle recovery and growth.\n\n**How to Try It:**\n• Have a bowl of Greek yogurt or cottage cheese\n• Mix a scoop of casein protein powder with a small amount of water or milk to make a pudding",
    "area": "eating",
    "goals": [
      "muscle_gain",
      "strength_performance",
      "improve_energy"
    ],
    "helps_with": [
      "night_eating"
    ],
    "contraindications": [
      "lactose_intolerance",
      "ibs"
    ],
    "mechanisms": [
      "time_ritual",
      "skill_building",
      "comfort",
      "mastery"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "late_night",
      "snack_time"
    ],
    "where": [
      "home"
    ],
    "involves": [
      "dairy"
    ],
    "preserves": [
      "chocolate"
    ],
    "satisfies": [
      "creamy"
    ],
    "features": [
      "solo_friendly",
      "veggie_free",
      "family_friendly",
      "partner_resistant_ok",
      "no_planning",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "What will be your bedtime protein choice?",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Greek yogurt",
        "Cottage cheese",
        "Casein shake",
        "Protein pudding",
        "String cheese + nuts"
      ],
      "multiple": false
    },
    "difficulty": 2,
    "source": "coach_curated",
    "sustainability": "daily",
    "areas": [
      "fitness",
      "nutrition",
      "organization"
    ]
  },
  {
    "tip_id": "f2a3b4c5-d6e7-8901-fabc-234567890123",
    "micro_version": "Pack a bag of baby carrots with your lunch.",
    "summary": "Add One Extra Serving of Vegetables to Lunch.",
    "details_md": "**The Experiment:** Include one additional cup of vegetables in your lunch today.\n\n**Why it Works:** Increasing vegetable intake is linked to reduced risk of chronic diseases. Adding raw veggies, a side salad, or steamed vegetables boosts fiber and micronutrients, helping you feel fuller and more energized.\n\n**How to Try It:**\n• Add a handful of baby carrots or cherry tomatoes to your lunch bag\n• Mix leftover roasted vegetables into your salad or grain bowl\n• Add a layer of spinach or sprouts to your sandwich",
    "area": "eating",
    "goals": [
      "increase_veggies",
      "improve_gut_health",
      "better_lipids",
      "weight_loss"
    ],
    "contraindications": [],
    "mechanisms": [
      "healthy_swap",
      "planning_ahead",
      "sensory",
      "comfort"
    ],
    "effort": "low",
    "time": "5-15",
    "cost": "$",
    "when": [
      "afternoon",
      "meal_time"
    ],
    "where": [
      "home",
      "work"
    ],
    "preserves": [
      "bread",
      "meat",
      "cheese"
    ],
    "satisfies": [
      "crunchy",
      "soft"
    ],
    "requires": [
      "advance_planning"
    ],
    "features": [
      "solo_friendly",
      "veggie_moderate",
      "family_friendly",
      "partner_resistant_ok",
      "diet_trauma_safe",
      "not_diety"
    ],
    "personalization_prompt": "Which veggie sidekick joins your lunch today?",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Baby carrots",
        "Cherry tomatoes",
        "Cucumber slices",
        "Bell pepper strips",
        "Snap peas",
        "Side salad"
      ],
      "multiple": false
    },
    "difficulty": 2,
    "source": "dietitian_reviewed",
    "sustainability": "daily",
    "helps_with": []
  },
  {
    "tip_id": "a3b4c5d6-e7f8-9012-abcd-345678901234",
    "micro_version": "Wash a handful of lettuce and put it in a bowl.",
    "summary": "Start Dinner with a Salad.",
    "details_md": "**The Experiment:** Begin your evening meal with a side salad or raw vegetables before eating the main course.\n\n**Why it Works:** Meal sequencing research shows starting with low-calorie, high-fiber foods increases overall vegetable consumption and enhances satiety. This can naturally reduce intake of higher-calorie foods later in the meal.\n\n**How to Try It:**\n• Prepare a simple salad with greens and a light vinaigrette\n• Eat the entire salad before your main dish is served\n• At a restaurant, order a side salad as an appetizer",
    "area": "eating",
    "goals": [
      "increase_veggies",
      "weight_loss",
      "improve_gut_health"
    ],
    "helps_with": [
      "speed_eating",
      "mindless_eating"
    ],
    "contraindications": [],
    "mechanisms": [
      "time_ritual",
      "habit_stacking",
      "sensory",
      "comfort"
    ],
    "effort": "minimal",
    "time": "5-15",
    "cost": "$",
    "when": [
      "evening",
      "meal_time"
    ],
    "where": [
      "home",
      "restaurant"
    ],
    "preserves": [
      "meat",
      "cheese",
      "bread",
      "pasta"
    ],
    "satisfies": [
      "crunchy",
      "soft"
    ],
    "features": [
      "veggie_heavy",
      "family_friendly",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety"
    ],
    "personalization_prompt": "Build your dinner starter salad:",
    "personalization_type": "text",
    "personalization_config": {
      "placeholder": "e.g., Mixed greens + tomatoes + balsamic"
    },
    "difficulty": 2,
    "source": "dietitian_reviewed",
    "sustainability": "daily"
  },
  {
    "tip_id": "d6e7f8a9-b0c1-2345-defa-678901234567",
    "micro_version": "Set your alarm 10 minutes earlier to allow for breakfast.",
    "summary": "Eat Breakfast Within 1 Hour of Waking.",
    "details_md": "**The Experiment:** Consume a balanced breakfast within 60 minutes of waking up.\n\n**Why it Works:** Circadian rhythm research suggests eating within an hour of waking aligns with natural metabolic patterns, which can improve glucose control and optimize insulin sensitivity for the day ahead.\n\n**How to Try It:**\n• Have a simple, pre-planned breakfast ready to go (like overnight oats)\n• Even a protein shake or a piece of fruit with nut butter counts",
    "area": "eating",
    "goals": [
      "improve_energy",
      "weight_loss",
      "improve_gut_health"
    ],
    "helps_with": [
      "grazing"
    ],
    "contraindications": [],
    "mechanisms": [
      "time_ritual",
      "planning_ahead",
      "energy_boost",
      "comfort"
    ],
    "effort": "low",
    "time": "5-15",
    "cost": "$",
    "when": [
      "morning",
      "meal_time"
    ],
    "where": [
      "home"
    ],
    "preserves": [
      "coffee",
      "chocolate"
    ],
    "requires": [
      "advance_planning"
    ],
    "features": [
      "solo_friendly",
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "diet_trauma_safe",
      "not_diety"
    ],
    "personalization_prompt": "What time do you wake up and when will you eat?",
    "personalization_type": "multi_text",
    "personalization_config": {
      "items": [
        {
          "label": "Wake time:",
          "placeholder": "e.g., 7 AM"
        },
        {
          "label": "Breakfast by:",
          "placeholder": "e.g., 7:45 AM"
        }
      ]
    },
    "difficulty": 2,
    "source": "dietitian_reviewed",
    "sustainability": "daily",
    "areas": [
      "nutrition",
      "organization"
    ]
  },
  {
    "tip_id": "a9b0c1d2-e3f4-5678-abcd-901234567890",
    "micro_version": "Plan a carb-heavy lunch (e.g., pasta/rice) for tomorrow.",
    "summary": "Front-Load Carbs: Consume Most Before Evening.",
    "details_md": "**The Experiment:** Try to consume the majority (e.g., 60-70%) of your daily carbohydrates before 6 PM.\n\n**Why it Works:** This strategy aligns carb intake with the time of day your body is typically more active and insulin-sensitive. It can support training energy while potentially improving sleep quality by reducing late-night blood sugar fluctuations.\n\n**How to Try It:**\n• Focus on including healthy carbs with your breakfast and lunch\n• Have a lighter-carb dinner, focusing more on protein and vegetables",
    "area": "eating",
    "goals": [
      "improve_energy",
      "weight_loss",
      "endurance_performance"
    ],
    "contraindications": [
      "t1_diabetes",
      "t2_diabetes"
    ],
    "mechanisms": [
      "time_ritual",
      "planning_ahead",
      "energy_boost",
      "mastery"
    ],
    "effort": "medium",
    "time": "0-5",
    "cost": "$",
    "when": [
      "morning",
      "afternoon",
      "meal_time"
    ],
    "where": [
      "home",
      "work"
    ],
    "involves": [
      "bread",
      "pasta",
      "carbs"
    ],
    "preserves": [
      "meat",
      "cheese",
      "chocolate"
    ],
    "requires": [
      "advance_planning"
    ],
    "features": [
      "solo_friendly",
      "veggie_moderate",
      "partner_resistant_ok"
    ],
    "personalization_prompt": "Which carbs will you enjoy earlier in the day?",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Morning oatmeal",
        "Lunch sandwich",
        "Afternoon fruit",
        "Pre-workout banana",
        "Rice with lunch"
      ],
      "multiple": true
    },
    "difficulty": 3,
    "source": "coach_curated",
    "sustainability": "occasional",
    "areas": [
      "nutrition",
      "organization"
    ],
    "helps_with": []
  },
  {
    "tip_id": "c1d2e3f4-a5b6-7890-cdef-123456789012",
    "micro_version": "Write down: \"If [Trigger], then I will [Action]\" on a card.",
    "summary": "Create an \"If-Then\" Food Plan.",
    "details_md": "**The Experiment:** Create a specific plan for a potential food challenge, such as: \"If I feel hungry at 3pm, then I'll eat the apple I have at my desk.\"\n\n**Why it Works:** These \"implementation intentions\" are scientifically proven to be effective. They create an automatic response to environmental cues, reducing decision fatigue and impulsive choices when you're tired or stressed.\n\n**How to Try It:**\n• Identify a common trigger (e.g., mid-afternoon slump)\n• Decide on a specific, healthy response\n• Write it down: \"If [situation], then I will [action]\"",
    "area": "eating",
    "goals": [
      "weight_loss",
      "reduce_sugar"
    ],
    "helps_with": [
      "stress_eating",
      "boredom_eating",
      "emotional_eating"
    ],
    "contraindications": [],
    "mechanisms": [
      "planning_ahead",
      "mindset_shift",
      "decision_ease",
      "mastery"
    ],
    "effort": "low",
    "time": "0-5",
    "cost": "$",
    "when": [
      "morning",
      "stress",
      "boredom",
      "craving_event"
    ],
    "where": [
      "home",
      "work"
    ],
    "preserves": [
      "chocolate",
      "cheese",
      "coffee",
      "bread",
      "pasta",
      "meat"
    ],
    "requires": [
      "advance_planning"
    ],
    "features": [
      "solo_friendly",
      "family_friendly",
      "partner_resistant_ok",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "What's your if-then backup plan?",
    "personalization_type": "multi_text",
    "personalization_config": {
      "items": [
        {
          "label": "If this happens:",
          "placeholder": "e.g., If I'm offered cake"
        },
        {
          "label": "Then I will:",
          "placeholder": "e.g., Say I'll have some later"
        }
      ]
    },
    "difficulty": 2,
    "source": "coach_curated",
    "sustainability": "daily",
    "areas": [
      "nutrition",
      "organization"
    ]
  },
  {
    "tip_id": "9f8a7b6c-1234-4abc-9def-000000000001",
    "micro_version": "Say out loud: \"Hello, Mr. Cookie Monster.\"",
    "summary": "Give your craving a goofy name ('Sir Crunch‑a‑Lot').",
    "details_md": "**The Experiment:** When an urge hits, name it out loud ('Hi, Sir Crunch‑a‑Lot—what's up?').\n\n**Why it Works:** Emotion labeling reduces amygdala reactivity and restores choice, loosening the stress→eat loop.\n\n**How to Try It:**\n• Say the name and rate the urge 0–10\n• Take one slow breath\n• Decide: cope now or set a 10‑min check‑in timer",
    "area": "eating",
    "goals": [
      "stress_eating",
      "emotional_eating",
      "mindful_eating"
    ],
    "helps_with": [
      "stress_eating",
      "mindless_eating",
      "rumination"
    ],
    "contraindications": [],
    "mechanisms": [
      "mindset_shift",
      "fun",
      "novelty",
      "mastery"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any",
      "stress",
      "craving_event",
      "boredom"
    ],
    "where": [
      "any"
    ],
    "features": [
      "solo_friendly",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "What silly name will you give your craving?",
    "personalization_type": "text",
    "personalization_config": {
      "placeholder": "e.g., Captain Cookie Monster, The Snack Attack Dragon, Lord Chips-a-Lot"
    },
    "difficulty": 1,
    "source": "dietitian_reviewed",
    "sustainability": "daily"
  },
  {
    "tip_id": "9f8a7b6c-1234-4abc-9def-000000000002",
    "micro_version": "Repeat 3 times: \"This is hard, and I can handle it.\"",
    "summary": "Threat→Challenge: 7‑word stress script.",
    "details_md": "**The Experiment:** Before a stressor, say: “This is hard *and* I can handle it.”\n\n**Why it Works:** Reframing stress shifts physiology toward challenge (less cortisol‑driven appetite).\n\n**How to Try It:**\n• Write it on a sticky note\n• Say it before meetings or emails\n• Pair with one deep exhale",
    "area": "eating",
    "goals": [
      "stress_eating",
      "emotional_eating",
      "improve_mood"
    ],
    "helps_with": [
      "stress_eating",
      "anxiety"
    ],
    "contraindications": [],
    "mechanisms": [
      "mindset_shift",
      "habit_stacking",
      "mastery"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any",
      "stress",
      "work_break",
      "conflict"
    ],
    "where": [
      "work",
      "home",
      "any"
    ],
    "features": [
      "solo_friendly",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "Where will you post your stress script reminder?",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Computer monitor",
        "Phone wallpaper",
        "Bathroom mirror",
        "Car dashboard",
        "Desk drawer"
      ],
      "multiple": true
    },
    "difficulty": 1,
    "source": "dietitian_reviewed",
    "sustainability": "daily"
  },
  {
    "tip_id": "9f8a7b6c-1234-4abc-9def-000000000003",
    "micro_version": "Write H-A-L-T on a sticky note and put it on the fridge door.",
    "summary": "HALT check on your fridge (Hungry, Angry, Lonely, Tired).",
    "details_md": "**The Experiment:** Put a small “HALT?” sticker on the fridge and touch it before snacking.\n\n**Why it Works:** Snaps you into interoception so emotions aren’t misread as hunger.\n\n**How to Try It:**\n• Say which letter fits\n• If not true hunger, pick a matching non‑food action\n• Re‑rate urge after 5 minutes",
    "area": "eating",
    "goals": [
      "mindful_eating",
      "stress_eating"
    ],
    "helps_with": [
      "mindless_eating",
      "stress_eating"
    ],
    "contraindications": [],
    "mechanisms": [
      "mindset_shift",
      "habit_stacking",
      "decision_ease"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any",
      "craving_event",
      "evening_grazing"
    ],
    "where": [
      "home",
      "work"
    ],
    "features": [
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "Where will you place your HALT reminder?",
    "personalization_type": "text",
    "personalization_config": {
      "placeholder": "e.g., Fridge handle, pantry door, snack drawer"
    },
    "difficulty": 1,
    "source": "dietitian_reviewed",
    "sustainability": "daily"
  },
  {
    "tip_id": "9f8a7b6c-1234-4abc-9def-000000000004",
    "micro_version": "Ask yourself: \"Would I eat a plate of plain broccoli right now?\"",
    "summary": "'Broccoli Gatekeeper' test for true hunger.",
    "details_md": "**The Experiment:** Ask: 'Would plain broccoli or chickpeas satisfy me right now?'\n\n**Why it Works:** Physical hunger is flexible; emotional hunger craves something specific.\n\n**How to Try It:**\n• If no, pick a non‑food coping action\n• If yes, choose a balanced snack\n• Recheck hunger after eating",
    "area": "eating",
    "goals": [
      "mindful_eating",
      "stress_eating"
    ],
    "helps_with": [
      "mindless_eating",
      "portion_control"
    ],
    "contraindications": [],
    "mechanisms": [
      "mindset_shift",
      "decision_ease",
      "mastery"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any",
      "craving_event",
      "boredom"
    ],
    "where": [
      "any"
    ],
    "features": [
      "solo_friendly",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "What's your 'gatekeeper' food to test true hunger?",
    "personalization_type": "text",
    "personalization_config": {
      "placeholder": "e.g., Plain carrots, apple slices, rice cakes"
    },
    "difficulty": 1,
    "source": "dietitian_reviewed",
    "sustainability": "daily"
  },
  {
    "tip_id": "9f8a7b6c-1234-4abc-9def-000000000005",
    "micro_version": "Close your eyes and scan your body for tension for 10 seconds.",
    "summary": "20‑second stoplight body scan (Green/Yellow/Red).",
    "details_md": "**The Experiment:** Close eyes and check body tension: green (calm), yellow (tight), red (very tight).\n\n**Why it Works:** Links sensations to emotions so food isn’t used to numb.\n\n**How to Try It:**\n• Scan jaw/chest/stomach\n• Name the color, then pick a fitting tool (breath, text a friend)\n• Recheck color after",
    "area": "eating",
    "goals": [
      "mindful_eating",
      "stress_eating"
    ],
    "helps_with": [
      "stress_eating",
      "anxiety",
      "mindless_eating"
    ],
    "contraindications": [],
    "mechanisms": [
      "mindset_shift",
      "relief",
      "mastery"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any",
      "stress",
      "anxiety",
      "craving_event"
    ],
    "where": [
      "any"
    ],
    "features": [
      "solo_friendly",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "difficulty": 1,
    "source": "dietitian_reviewed",
    "sustainability": "daily"
  },
  {
    "tip_id": "9f8a7b6c-1234-4abc-9def-000000000006",
    "micro_version": "Rate your current hunger level from 1-10.",
    "summary": "Hunger thermometer (1–10) before/during/after meals.",
    "details_md": "**The Experiment:** Rate hunger pre‑meal, halfway, and after; aim to stop around 6–7.\n\n**Why it Works:** Trains satiety detection and reduces overshooting fullness.\n\n**How to Try It:**\n• Set a halfway alert\n• Pause utensils, rate hunger\n• Decide to continue or stop",
    "area": "eating",
    "goals": [
      "mindful_eating",
      "portion_control"
    ],
    "helps_with": [
      "portion_control",
      "mindless_eating"
    ],
    "contraindications": [],
    "mechanisms": [
      "mindset_shift",
      "habit_stacking",
      "mastery"
    ],
    "effort": "low",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any",
      "meal_time"
    ],
    "where": [
      "home",
      "restaurant",
      "work"
    ],
    "requires": [
      "timer"
    ],
    "features": [
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "Create your hunger scale metaphor (1=starving, 10=stuffed). What's your 5-6?",
    "personalization_type": "text",
    "personalization_config": {
      "placeholder": "e.g., 1=Empty tank, 5=Cruising speed, 10=Overflow"
    },
    "difficulty": 2,
    "source": "dietitian_reviewed",
    "sustainability": "daily"
  },
  {
    "tip_id": "9f8a7b6c-1234-4abc-9def-000000000007",
    "micro_version": "Set a timer for 90 seconds and just breathe.",
    "summary": "90‑second urge surf (watch the wave rise/fall).",
    "details_md": "**The Experiment:** When a craving spikes, set a 90‑sec timer and breathe slowly.\n\n**Why it Works:** Urges crest and fall; brief delay re‑engages the prefrontal cortex.\n\n**How to Try It:**\n• Inhale 4, exhale 6\n• Narrate “rising…peaking…falling”\n• Decide after the chime",
    "area": "eating",
    "goals": [
      "reduce_cravings",
      "stress_eating"
    ],
    "helps_with": [
      "stress_eating",
      "boredom_eating",
      "mindless_eating"
    ],
    "contraindications": [],
    "mechanisms": [
      "behavioral_strategy",
      "mindset_shift",
      "relief",
      "mastery"
    ],
    "effort": "low",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any",
      "craving_event",
      "evening_grazing"
    ],
    "where": [
      "any"
    ],
    "requires": [
      "timer"
    ],
    "features": [
      "solo_friendly",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "What will you visualize during your 90-second craving surf?",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Ocean wave rising and falling",
        "Storm cloud passing by",
        "Hot air balloon floating away",
        "Train passing through a station",
        "Campfire slowly dying down"
      ],
      "multiple": false
    },
    "difficulty": 2,
    "source": "dietitian_reviewed",
    "sustainability": "daily"
  },
  {
    "tip_id": "9f8a7b6c-1234-4abc-9def-000000000008",
    "micro_version": "Ask yourself: \"What do I actually need right now besides food?\"",
    "summary": "Swap self‑criticism for curiosity (“What do I need?”).",
    "details_md": "**The Experiment:** Replace “no willpower” with “What do I actually need right now?”\n\n**Why it Works:** Self‑compassion lowers cortisol and opens problem‑solving.\n\n**How to Try It:**\n• Ask need: soothe, connect, rest, fuel?\n• Pick 1 matching action\n• Note outcome 0–10",
    "area": "eating",
    "goals": [
      "stress_eating",
      "emotional_eating"
    ],
    "helps_with": [
      "stress_eating",
      "rumination"
    ],
    "contraindications": [],
    "mechanisms": [
      "mindset_shift",
      "relief",
      "mastery"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any",
      "stress",
      "anxiety",
      "craving_event"
    ],
    "where": [
      "any"
    ],
    "features": [
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "difficulty": 1,
    "source": "dietitian_reviewed",
    "sustainability": "daily"
  },
  {
    "tip_id": "9f8a7b6c-1234-4abc-9def-000000000009",
    "micro_version": "Draw a quick sun, cloud, or storm icon to represent your mood.",
    "summary": "Weather‑map your mood before meals.",
    "details_md": "**The Experiment:** Draw a sun/clouds/storm next to your meal note.\n\n**Why it Works:** Fast emotion ID prevents mislabeling feelings as hunger.\n\n**How to Try It:**\n• Mark the icon\n• Pick one matching coping tool if needed\n• Proceed to eat mindfully",
    "area": "eating",
    "goals": [
      "mindful_eating",
      "stress_eating"
    ],
    "helps_with": [
      "mindless_eating",
      "stress_eating"
    ],
    "contraindications": [],
    "mechanisms": [
      "mindset_shift",
      "habit_stacking",
      "fun",
      "decision_ease"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any",
      "meal_time"
    ],
    "where": [
      "home",
      "work",
      "restaurant"
    ],
    "features": [
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "difficulty": 1,
    "source": "dietitian_reviewed",
    "sustainability": "daily"
  },
  {
    "tip_id": "9f8a7b6c-1234-4abc-9def-000000000010",
    "micro_version": "Write one coping strategy on a small card for your wallet.",
    "summary": "Carry 3 If‑Then pocket plans.",
    "details_md": "**The Experiment:** Write: “If I feel __, then I’ll __ (non‑food).”\n\n**Why it Works:** Implementation intentions automate wise choices under stress.\n\n**How to Try It:**\n• Make 3 common If‑Then cards\n• Keep in wallet/phone\n• Use at first trigger",
    "area": "eating",
    "goals": [
      "stress_eating",
      "stress_coping_plan"
    ],
    "helps_with": [
      "stress_eating",
      "boredom_eating",
      "emotional_eating"
    ],
    "contraindications": [],
    "mechanisms": [
      "planning_ahead",
      "mindset_shift",
      "decision_ease",
      "mastery"
    ],
    "effort": "low",
    "time": "0-5",
    "cost": "$",
    "when": [
      "morning",
      "any",
      "stress",
      "boredom",
      "craving_event"
    ],
    "where": [
      "any"
    ],
    "requires": [
      "advance_planning"
    ],
    "features": [
      "solo_friendly",
      "family_friendly",
      "partner_resistant_ok",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "difficulty": 2,
    "source": "dietitian_reviewed",
    "sustainability": "daily",
    "areas": [
      "nutrition",
      "organization"
    ]
  },
  {
    "tip_id": "9f8a7b6c-1234-4abc-9def-000000000011",
    "micro_version": "Put on your favorite upbeat song and dance for the duration.",
    "summary": "One‑song dance break.",
    "details_md": "**The Experiment:** Play one upbeat song and move however you like.\n\n**Why it Works:** Brief activity metabolizes stress hormones and boosts endorphins.\n\n**How to Try It:**\n• Hit play when a craving hits\n• Move until song ends\n• Re‑rate the urge",
    "area": "eating",
    "goals": [
      "improve_mood",
      "stress_eating"
    ],
    "helps_with": [
      "stress_eating",
      "anxiety"
    ],
    "contraindications": [
      "Modify if you have pain or mobility limitations; choose gentle movements."
    ],
    "mechanisms": [
      "behavioral_strategy",
      "energy_boost",
      "fun"
    ],
    "effort": "low",
    "time": "5-15",
    "cost": "$",
    "when": [
      "any",
      "stress",
      "boredom",
      "craving_event"
    ],
    "where": [
      "home",
      "work"
    ],
    "features": [
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "difficulty": 1,
    "source": "dietitian_reviewed",
    "sustainability": "daily"
  },
  {
    "tip_id": "9f8a7b6c-1234-4abc-9def-000000000012",
    "micro_version": "Stand up and walk out the front door for 5 minutes.",
    "summary": "5‑minute reset walk (spot 5 colors/5 sounds).",
    "details_md": "**The Experiment:** Walk outside for 5–10 minutes, noticing colors and sounds.\n\n**Why it Works:** Movement + sensory focus interrupts the craving loop.\n\n**How to Try It:**\n• Step outside or hallway\n• Name 5 colors, 5 sounds\n• Re‑assess the urge",
    "area": "eating",
    "goals": [
      "improve_mood",
      "stress_eating"
    ],
    "helps_with": [
      "stress_eating",
      "rumination"
    ],
    "contraindications": [
      "Adjust for weather and mobility; indoor laps work."
    ],
    "mechanisms": [
      "behavioral_strategy",
      "relief",
      "energy_boost"
    ],
    "effort": "low",
    "time": "5-15",
    "cost": "$",
    "when": [
      "any",
      "stress",
      "work_break",
      "boredom"
    ],
    "where": [
      "outdoors",
      "work",
      "home"
    ],
    "features": [
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "difficulty": 1,
    "source": "dietitian_reviewed",
    "sustainability": "daily"
  },
  {
    "tip_id": "9f8a7b6c-1234-4abc-9def-000000000013",
    "micro_version": "Draft a text to a friend: \"Struggling with a craving, distract me?\"",
    "summary": "Text‑a‑Buddy SOS (prewritten message).",
    "details_md": "**The Experiment:** Send: “Craving + stressed—2‑min chat?” to a support person.\n\n**Why it Works:** Connection activates reward without calories.\n\n**How to Try It:**\n• Save the template in Notes\n• Ask one friend to be on call\n• Keep it to 2–3 minutes",
    "area": "eating",
    "goals": [
      "emotional_eating",
      "stress_eating"
    ],
    "helps_with": [
      "stress_eating",
      "boredom_eating",
      "loneliness"
    ],
    "contraindications": [],
    "mechanisms": [
      "behavioral_strategy",
      "planning_ahead",
      "accountability",
      "comfort"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any",
      "loneliness",
      "stress",
      "boredom"
    ],
    "where": [
      "any"
    ],
    "requires": [
      "phone",
      "advance_planning"
    ],
    "features": [
      "family_friendly",
      "partner_resistant_ok",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "difficulty": 1,
    "source": "dietitian_reviewed",
    "sustainability": "daily"
  },
  {
    "tip_id": "9f8a7b6c-1234-4abc-9def-000000000014",
    "micro_version": "Place a fidget spinner or stress ball on your desk.",
    "summary": "Hands‑Busy kit (stress ball, fidget, mini‑puzzle).",
    "details_md": "**The Experiment:** Keep 2–3 tactile items near snack zones.\n\n**Why it Works:** Redirects motor habits and buys time for urges to fade.\n\n**How to Try It:**\n• Place the kit by TV/desk\n• Use for 3–5 minutes when triggered\n• Then decide on food",
    "area": "eating",
    "goals": [
      "stress_eating",
      "stress_distraction_tool"
    ],
    "helps_with": [
      "mindless_eating",
      "evening_grazing"
    ],
    "contraindications": [],
    "mechanisms": [
      "behavioral_strategy",
      "planning_ahead",
      "convenience",
      "relief"
    ],
    "effort": "minimal",
    "time": "5-15",
    "cost": "$",
    "when": [
      "evening",
      "any",
      "screen_time",
      "work_break",
      "evening_grazing"
    ],
    "where": [
      "home",
      "work"
    ],
    "requires": [
      "advance_planning"
    ],
    "features": [
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "difficulty": 1,
    "source": "dietitian_reviewed",
    "sustainability": "daily"
  },
  {
    "tip_id": "9f8a7b6c-1234-4abc-9def-000000000015",
    "micro_version": "Unwrap a piece of mint gum and chew it.",
    "summary": "Flavor‑bomb sugar‑free gum.",
    "details_md": "**The Experiment:** Chew a strong mint for 5 minutes when cravings pop.\n\n**Why it Works:** Oral stimulation + palate reset blunts hedonic pull.\n\n**How to Try It:**\n• Keep a pack in bag/desk\n• Chew during TV ads or post‑meal urges\n• Re‑assess after 5 min",
    "area": "eating",
    "goals": [
      "reduce_cravings",
      "stress_eating"
    ],
    "helps_with": [
      "stress_eating",
      "mindless_eating"
    ],
    "contraindications": [
      "Sugar alcohols may cause GI upset; avoid if jaw/TMJ issues."
    ],
    "mechanisms": [
      "behavioral_strategy",
      "sensory",
      "convenience"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any",
      "craving_event",
      "evening_grazing",
      "screen_time"
    ],
    "where": [
      "any",
      "commute"
    ],
    "involves": [
      "sugar_free_gum"
    ],
    "satisfies": [
      "minty"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "difficulty": 1,
    "source": "dietitian_reviewed",
    "sustainability": "daily"
  },
  {
    "tip_id": "9f8a7b6c-1234-4abc-9def-000000000016",
    "micro_version": "Inhale for 4 seconds, hold for 4, exhale for 6.",
    "summary": "Box breathing 4‑4‑6.",
    "details_md": "**The Experiment:** Inhale 4, hold 4, exhale 6 for 10 breaths.\n\n**Why it Works:** Longer exhale activates the vagus nerve (calms HPA axis).\n\n**How to Try It:**\n• Sit tall, one hand on belly\n• Count silently\n• Re‑rate stress after",
    "area": "eating",
    "goals": [
      "stress_eating",
      "improve_mood"
    ],
    "helps_with": [
      "anxiety",
      "stress_eating"
    ],
    "contraindications": [
      "Stop if dizzy; breathe comfortably if pregnant or with respiratory issues."
    ],
    "mechanisms": [
      "behavioral_strategy",
      "relief"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any",
      "anxiety",
      "stress",
      "craving_event"
    ],
    "where": [
      "any"
    ],
    "features": [
      "solo_friendly",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "difficulty": 1,
    "source": "dietitian_reviewed",
    "sustainability": "daily"
  },
  {
    "tip_id": "9f8a7b6c-1234-4abc-9def-000000000017",
    "micro_version": "Go to the freezer and hold one ice cube in your hand.",
    "summary": "Ice‑cube grounding.",
    "details_md": "**The Experiment:** Hold an ice cube for ~30–45 seconds and notice sensations.\n\n**Why it Works:** Strong but safe sensory cue interrupts rumination/cravings.\n\n**How to Try It:**\n• Grab an ice cube\n• Track the sensation changing\n• Re‑assess your urge",
    "area": "eating",
    "goals": [
      "reduce_cravings",
      "stress_eating"
    ],
    "helps_with": [
      "stress_eating",
      "rumination"
    ],
    "contraindications": [
      "Skip if you have Raynaud’s or cold sensitivity."
    ],
    "mechanisms": [
      "behavioral_strategy",
      "sensory",
      "novelty"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any",
      "craving_event",
      "anxiety"
    ],
    "where": [
      "home",
      "work"
    ],
    "requires": [
      "freezer"
    ],
    "features": [
      "solo_friendly",
      "family_friendly",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "difficulty": 1,
    "source": "dietitian_reviewed",
    "sustainability": "occasional"
  },
  {
    "tip_id": "9f8a7b6c-1234-4abc-9def-000000000018",
    "micro_version": "Run your hands under warm water for 30 seconds.",
    "summary": "Warm‑hands reset (2 minutes).",
    "details_md": "**The Experiment:** Rinse hands in warm water for ~2 minutes.\n\n**Why it Works:** Warmth cues parasympathetic calm and comfort.\n\n**How to Try It:**\n• Breathe slowly while warming hands\n• Feel shoulders drop\n• Re‑check urge",
    "area": "eating",
    "goals": [
      "improve_mood",
      "stress_eating"
    ],
    "helps_with": [
      "stress_eating",
      "evening_grazing"
    ],
    "contraindications": [],
    "mechanisms": [
      "behavioral_strategy",
      "comfort",
      "relief"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "evening",
      "any",
      "evening_grazing",
      "stress"
    ],
    "where": [
      "home",
      "work"
    ],
    "requires": [
      "sink"
    ],
    "features": [
      "solo_friendly",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "difficulty": 1,
    "source": "dietitian_reviewed",
    "sustainability": "daily"
  },
  {
    "tip_id": "9f8a7b6c-1234-4abc-9def-000000000019",
    "micro_version": "Find your pet and pet them for 2 minutes.",
    "summary": "Two minutes with a pet.",
    "details_md": "**The Experiment:** Pet, play, or cuddle with an animal for ~2 minutes.\n\n**Why it Works:** Lowers cortisol and raises oxytocin—natural comfort.\n\n**How to Try It:**\n• Put phone down\n• Focus on touch/connection\n• Re‑assess craving",
    "area": "eating",
    "goals": [
      "emotional_eating",
      "improve_mood"
    ],
    "helps_with": [
      "stress_eating",
      "loneliness"
    ],
    "contraindications": [
      "Skip if allergic or animal is not receptive."
    ],
    "mechanisms": [
      "behavioral_strategy",
      "comfort"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any",
      "loneliness",
      "stress",
      "boredom"
    ],
    "where": [
      "home",
      "outdoors"
    ],
    "features": [
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "difficulty": 1,
    "source": "dietitian_reviewed",
    "sustainability": "daily"
  },
  {
    "tip_id": "9f8a7b6c-1234-4abc-9def-000000000020",
    "micro_version": "Set a timer for 90 seconds and tidy your immediate area.",
    "summary": "90‑second tidy sprint.",
    "details_md": "**The Experiment:** Set a timer and clear a small surface or drawer.\n\n**Why it Works:** Creating order restores control that stress eating mimics.\n\n**How to Try It:**\n• 90 seconds only\n• Count items to donate/trash\n• Re‑assess the urge",
    "area": "eating",
    "goals": [
      "stress_eating",
      "stress_reset_routine"
    ],
    "helps_with": [
      "stress_eating",
      "rumination"
    ],
    "contraindications": [],
    "mechanisms": [
      "behavioral_strategy",
      "mastery",
      "relief"
    ],
    "effort": "low",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any",
      "stress",
      "boredom",
      "screen_time"
    ],
    "where": [
      "home",
      "work"
    ],
    "requires": [
      "timer"
    ],
    "features": [
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "difficulty": 1,
    "source": "dietitian_reviewed",
    "sustainability": "daily"
  },
  {
    "tip_id": "9f8a7b6c-1234-4abc-9def-000000000021",
    "micro_version": "Put a rubber band around the jar of your favorite snack.",
    "summary": "Snack speed‑bump (rubber band + note on lid).",
    "details_md": "**The Experiment:** Place a rubber band and a tiny “HALT?” note on tempting containers.\n\n**Why it Works:** Adds friction and a mindful pause.\n\n**How to Try It:**\n• Band the lid\n• Read note aloud\n• Choose: portion or pause",
    "area": "eating",
    "goals": [
      "portion_control",
      "stress_eating"
    ],
    "helps_with": [
      "mindless_eating",
      "portion_control"
    ],
    "contraindications": [],
    "mechanisms": [
      "planning_ahead",
      "habit_stacking",
      "decision_ease"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "evening",
      "any",
      "evening_grazing",
      "screen_time"
    ],
    "where": [
      "home",
      "work"
    ],
    "requires": [
      "advance_planning"
    ],
    "features": [
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "What message will you write on your snack container to make you pause?",
    "personalization_type": "text",
    "personalization_config": {
      "placeholder": "e.g., 'Are you hungry or just bored?', 'HALT: Hungry? Angry? Lonely? Tired?'"
    },
    "difficulty": 1,
    "source": "dietitian_reviewed",
    "sustainability": "daily",
    "areas": [
      "nutrition",
      "organization"
    ]
  },
  {
    "tip_id": "9f8a7b6c-1234-4abc-9def-000000000022",
    "micro_version": "Take a plate out of the cabinet before opening a snack bag.",
    "summary": "Snack‑tray rule (plate it, sit at a table).",
    "details_md": "**The Experiment:** Put all snacks on a small plate and eat at a table.\n\n**Why it Works:** Visibility + context reduce mindless grazing.\n\n**How to Try It:**\n• Portion once\n• Sit down without screens\n• Pause halfway to check fullness",
    "area": "eating",
    "goals": [
      "portion_control",
      "mindful_eating"
    ],
    "helps_with": [
      "mindless_eating",
      "portion_control"
    ],
    "contraindications": [],
    "mechanisms": [
      "time_ritual",
      "habit_stacking",
      "decision_ease"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any",
      "snack_time",
      "evening_grazing"
    ],
    "where": [
      "home",
      "work"
    ],
    "requires": [
      "plate"
    ],
    "features": [
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "Where's your official 'snack zone' going to be?",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Kitchen table",
        "Dining room",
        "Breakfast nook",
        "Outdoor patio",
        "Office desk (but no work!)"
      ],
      "multiple": false
    },
    "difficulty": 1,
    "source": "dietitian_reviewed",
    "sustainability": "daily"
  },
  {
    "tip_id": "9f8a7b6c-1234-4abc-9def-000000000023",
    "micro_version": "Remove all food items from your desk surface.",
    "summary": "Hydration‑only desk rule.",
    "details_md": "**The Experiment:** Only water/tea at your desk; all eating happens elsewhere.\n\n**Why it Works:** Breaks the work‑stress→snack association.\n\n**How to Try It:**\n• Put a water bottle at desk\n• Move snacks to kitchen\n• Add a desk “No food” note",
    "area": "eating",
    "goals": [
      "stress_eating",
      "beverage_boundaries"
    ],
    "helps_with": [
      "work_snacking",
      "mindless_eating"
    ],
    "contraindications": [],
    "mechanisms": [
      "time_ritual",
      "planning_ahead",
      "mastery"
    ],
    "effort": "low",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any",
      "work_break",
      "stress"
    ],
    "where": [
      "work",
      "home"
    ],
    "involves": [
      "water",
      "tea"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "difficulty": 2,
    "source": "dietitian_reviewed",
    "sustainability": "daily"
  },
  {
    "tip_id": "9f8a7b6c-1234-4abc-9def-000000000024",
    "micro_version": "Move the healthy snacks to the shelf that is at your eye level.",
    "summary": "Pantry shuffle (produce front & center).",
    "details_md": "**The Experiment:** Move fruit/veg to eye level; stash treats high/opaque.\n\n**Why it Works:** What’s visible is what’s eaten; cue management beats willpower.\n\n**How to Try It:**\n• Clear bowls for fruit\n• Opaque bins for treats\n• Label swaps: “See me, eat me”",
    "area": "eating",
    "goals": [
      "produce_priority",
      "produce_visibility"
    ],
    "helps_with": [
      "mindless_eating",
      "reduce_cravings"
    ],
    "contraindications": [],
    "mechanisms": [
      "planning_ahead",
      "convenience"
    ],
    "effort": "minimal",
    "time": "5-15",
    "cost": "$",
    "when": [
      "any",
      "snack_time",
      "evening_grazing"
    ],
    "where": [
      "home"
    ],
    "requires": [
      "advance_planning"
    ],
    "features": [
      "veggie_moderate",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "difficulty": 1,
    "source": "dietitian_reviewed",
    "sustainability": "daily"
  },
  {
    "tip_id": "9f8a7b6c-1234-4abc-9def-000000000025",
    "micro_version": "Turn off the kitchen light to signal the kitchen is closed.",
    "summary": "“Kitchen Closed” evening ritual + brush teeth.",
    "details_md": "**The Experiment:** After dinner, lights off, counters wiped, tent card: “Kitchen Closed.” Brush teeth.\n\n**Why it Works:** Ritual creates a clear off‑switch and minty palate reset.\n\n**How to Try It:**\n• Set a nightly alarm\n• Do the 3‑step close\n• Keep mouthwash/toothbrush handy",
    "area": "eating",
    "goals": [
      "evening_grazing",
      "stress_eating"
    ],
    "helps_with": [
      "evening_grazing",
      "mindless_eating"
    ],
    "contraindications": [
      "Alcohol mouthwash can irritate; choose alcohol‑free if sensitive."
    ],
    "mechanisms": [
      "time_ritual",
      "habit_stacking",
      "closure",
      "comfort"
    ],
    "effort": "minimal",
    "time": "5-15",
    "cost": "$",
    "when": [
      "evening",
      "night",
      "post_dinner",
      "screen_time"
    ],
    "where": [
      "home"
    ],
    "satisfies": [
      "minty"
    ],
    "requires": [
      "toothbrush"
    ],
    "features": [
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "difficulty": 1,
    "source": "dietitian_reviewed",
    "sustainability": "daily"
  },
  {
    "tip_id": "9f8a7b6c-1234-4abc-9def-000000000026",
    "micro_version": "Write \"PERIMETER\" in big letters on your shopping list.",
    "summary": "Produce‑first cart (fill ⅓ before anything else).",
    "details_md": "**The Experiment:** At the store, fill the first third of your cart with produce.\n\n**Why it Works:** Anchoring bias nudges later choices healthier.\n\n**How to Try It:**\n• Shop perimeter first\n• Add grab‑ready veg/fruit\n• Then choose proteins/grains",
    "area": "eating",
    "goals": [
      "produce_priority",
      "produce_priority"
    ],
    "helps_with": [
      "reduce_cravings"
    ],
    "contraindications": [],
    "mechanisms": [
      "planning_ahead",
      "decision_ease"
    ],
    "effort": "minimal",
    "time": "5-15",
    "cost": "$",
    "when": [
      "any",
      "shopping"
    ],
    "where": [
      "home",
      "grocery"
    ],
    "requires": [
      "advance_planning"
    ],
    "features": [
      "veggie_heavy",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "diet_trauma_safe",
      "not_diety"
    ],
    "difficulty": 1,
    "source": "dietitian_reviewed",
    "sustainability": "weekly"
  },
  {
    "tip_id": "9f8a7b6c-1234-4abc-9def-000000000027",
    "micro_version": "Put cookies in a non-see-through container.",
    "summary": "Opaque decanting for treats; clear bowls for fruit.",
    "details_md": "**The Experiment:** Move treats to opaque bins; put fruit in clear bowls.\n\n**Why it Works:** Sight drives desire; manage cues to manage choices.\n\n**How to Try It:**\n• Label bins\n• Keep fruit washed/visible\n• Rotate weekly",
    "area": "eating",
    "goals": [
      "reduce_cravings",
      "treat_environment_design"
    ],
    "helps_with": [
      "mindless_eating",
      "portion_control"
    ],
    "contraindications": [],
    "mechanisms": [
      "planning_ahead",
      "convenience"
    ],
    "effort": "minimal",
    "time": "5-15",
    "cost": "$",
    "when": [
      "any",
      "snack_time"
    ],
    "where": [
      "home"
    ],
    "requires": [
      "advance_planning"
    ],
    "features": [
      "veggie_moderate",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "difficulty": 1,
    "source": "dietitian_reviewed",
    "sustainability": "weekly",
    "areas": [
      "nutrition",
      "organization"
    ]
  },
  {
    "tip_id": "9f8a7b6c-1234-4abc-9def-000000000028",
    "micro_version": "Fill 3 small containers with snacks for the week.",
    "summary": "Sunday bento‑style snack prep (single portions).",
    "details_md": "**The Experiment:** Pre‑portion snacks into small containers for the week.\n\n**Why it Works:** Built‑in stop signals fight mindless seconds.\n\n**How to Try It:**\n• Prep 5–10 portions\n• Include protein/fiber options\n• Stack at eye level",
    "area": "eating",
    "goals": [
      "portion_control",
      "mindful_eating"
    ],
    "helps_with": [
      "portion_control",
      "stress_eating"
    ],
    "contraindications": [],
    "mechanisms": [
      "meal_prep",
      "planning_ahead",
      "convenience",
      "mastery"
    ],
    "effort": "low",
    "time": "15-30min",
    "cost": "$",
    "when": [
      "morning",
      "afternoon",
      "meal_prep"
    ],
    "where": [
      "home"
    ],
    "involves": [
      "fruit",
      "yogurt",
      "nuts",
      "crackers"
    ],
    "requires": [
      "containers",
      "advance_planning"
    ],
    "features": [
      "veggie_moderate",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "diet_trauma_safe",
      "not_diety"
    ],
    "difficulty": 2,
    "source": "dietitian_reviewed",
    "sustainability": "weekly",
    "areas": [
      "nutrition",
      "organization"
    ]
  },
  {
    "tip_id": "9f8a7b6c-1234-4abc-9def-000000000029",
    "micro_version": "Move your food delivery app into a hidden folder or delete it.",
    "summary": "App hibernation week (offload food delivery apps).",
    "details_md": "**The Experiment:** Offload/hide delivery apps for 7 days.\n\n**Why it Works:** Extra clicks add just enough friction to interrupt impulses.\n\n**How to Try It:**\n• Move apps into a folder or offload\n• Keep a simple meal plan handy\n• Re‑install after the week if desired",
    "area": "eating",
    "goals": [
      "stress_eating",
      "food_delivery_boundary"
    ],
    "helps_with": [
      "impulse_orders",
      "evening_grazing"
    ],
    "contraindications": [],
    "mechanisms": [
      "planning_ahead",
      "decision_ease",
      "mastery"
    ],
    "effort": "low",
    "time": "0-5",
    "cost": "$",
    "when": [
      "evening",
      "any",
      "evening_grazing",
      "boredom",
      "craving_event"
    ],
    "where": [
      "any"
    ],
    "requires": [
      "advance_planning"
    ],
    "features": [
      "solo_friendly",
      "partner_resistant_ok",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "difficulty": 2,
    "source": "dietitian_reviewed",
    "sustainability": "occasional",
    "areas": [
      "nutrition",
      "organization"
    ]
  },
  {
    "tip_id": "9f8a7b6c-1234-4abc-9def-000000000030",
    "micro_version": "Eat a piece of fruit before you go to the grocery store.",
    "summary": "Post‑meal shopping rule.",
    "details_md": "**The Experiment:** Only grocery shop after eating.\n\n**Why it Works:** Lowers hunger‑driven hedonic purchases.\n\n**How to Try It:**\n• Add to calendar notes\n• Keep a standing list\n• Eat a small snack before unexpected runs",
    "area": "eating",
    "goals": [
      "reduce_cravings",
      "impulse_control_shopping"
    ],
    "helps_with": [
      "reduce_cravings"
    ],
    "contraindications": [],
    "mechanisms": [
      "time_ritual",
      "planning_ahead",
      "decision_ease"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "afternoon",
      "any",
      "shopping"
    ],
    "where": [
      "grocery"
    ],
    "requires": [
      "advance_planning"
    ],
    "features": [
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "difficulty": 1,
    "source": "dietitian_reviewed",
    "sustainability": "weekly"
  },
  {
    "tip_id": "9f8a7b6c-1234-4abc-9def-000000000031",
    "micro_version": "Put chopsticks on the table for your next meal.",
    "summary": "Chopsticks challenge (or tiny fork).",
    "details_md": "**The Experiment:** Eat a meal/snack with chopsticks or a tiny fork.\n\n**Why it Works:** Disrupts autopilot and slows bites so satiety can register.\n\n**How to Try It:**\n• Use for the whole portion\n• Put down between bites\n• Notice flavor changes",
    "area": "eating",
    "goals": [
      "mindful_eating",
      "portion_control"
    ],
    "helps_with": [
      "mindless_eating",
      "portion_control"
    ],
    "contraindications": [],
    "mechanisms": [
      "time_ritual",
      "novelty",
      "fun"
    ],
    "effort": "low",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any",
      "meal_time"
    ],
    "where": [
      "home",
      "work",
      "restaurant"
    ],
    "requires": [
      "chopsticks"
    ],
    "features": [
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety"
    ],
    "difficulty": 2,
    "source": "dietitian_reviewed",
    "sustainability": "occasional"
  },
  {
    "tip_id": "9f8a7b6c-1234-4abc-9def-000000000032",
    "micro_version": "Set a timer for 1 minute when you sit down to eat.",
    "summary": "Silent first minute of every meal.",
    "details_md": "**The Experiment:** Eat the first 60 seconds without talking or screens.\n\n**Why it Works:** Heightens sensory cues and early satisfaction.\n\n**How to Try It:**\n• Start in silence\n• Notice aroma/texture\n• Decide pace after 1 minute",
    "area": "eating",
    "goals": [
      "mindful_eating"
    ],
    "helps_with": [
      "mindless_eating"
    ],
    "contraindications": [],
    "mechanisms": [
      "time_ritual",
      "habit_stacking",
      "mastery",
      "sensory"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any",
      "meal_time"
    ],
    "where": [
      "home",
      "work",
      "restaurant"
    ],
    "features": [
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety"
    ],
    "difficulty": 1,
    "source": "dietitian_reviewed",
    "sustainability": "daily"
  },
  {
    "tip_id": "9f8a7b6c-1234-4abc-9def-000000000033",
    "micro_version": "Take a picture of your food before you start eating.",
    "summary": "Photo‑pause halfway through the meal.",
    "details_md": "**The Experiment:** Snap a quick plate photo at the halfway point.\n\n**Why it Works:** Visual pause interrupts momentum eating and rechecks hunger.\n\n**How to Try It:**\n• Set a halfway timer\n• Take one photo\n• Ask: do I need more?",
    "area": "eating",
    "goals": [
      "portion_control",
      "mindful_eating"
    ],
    "helps_with": [
      "portion_control",
      "mindless_eating"
    ],
    "contraindications": [],
    "mechanisms": [
      "habit_stacking",
      "accountability",
      "mastery"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any",
      "meal_time"
    ],
    "where": [
      "home",
      "work",
      "restaurant"
    ],
    "requires": [
      "phone_camera",
      "timer"
    ],
    "features": [
      "family_friendly",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety"
    ],
    "difficulty": 1,
    "source": "dietitian_reviewed",
    "sustainability": "daily"
  },
  {
    "tip_id": "9f8a7b6c-1234-4abc-9def-000000000034",
    "micro_version": "Switch your fork to your other hand for 3 bites.",
    "summary": "Eat with your non‑dominant hand.",
    "details_md": "**The Experiment:** Use your non‑dominant hand for a full snack/meal.\n\n**Why it Works:** Disrupts conditioned motor patterns, slowing intake.\n\n**How to Try It:**\n• Commit for the entire portion\n• Put utensil down between bites\n• Notice speed shift",
    "area": "eating",
    "goals": [
      "mindful_eating"
    ],
    "helps_with": [
      "mindless_eating"
    ],
    "contraindications": [],
    "mechanisms": [
      "time_ritual",
      "novelty",
      "fun"
    ],
    "effort": "low",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any",
      "meal_time"
    ],
    "where": [
      "any"
    ],
    "features": [
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety"
    ],
    "difficulty": 2,
    "source": "dietitian_reviewed",
    "sustainability": "occasional"
  },
  {
    "tip_id": "9f8a7b6c-1234-4abc-9def-000000000035",
    "micro_version": "Close your eyes and chew the first bite slowly.",
    "summary": "First‑bite spotlight.",
    "details_md": "**The Experiment:** Give the first bite total attention and rate it 1–10.\n\n**Why it Works:** Reward is front‑loaded; savoring increases satisfaction with less.\n\n**How to Try It:**\n• Close eyes for first bite\n• Name 3 flavor notes\n• Decide slow pace",
    "area": "eating",
    "goals": [
      "mindful_eating"
    ],
    "helps_with": [
      "mindless_eating",
      "portion_control"
    ],
    "contraindications": [],
    "mechanisms": [
      "time_ritual",
      "sensory",
      "mastery"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any",
      "meal_time"
    ],
    "where": [
      "any"
    ],
    "features": [
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety"
    ],
    "difficulty": 1,
    "source": "dietitian_reviewed",
    "sustainability": "daily"
  },
  {
    "tip_id": "9f8a7b6c-1234-4abc-9def-000000000036",
    "micro_version": "Write down 3 crunchy vegetables you like.",
    "summary": "Crunch‑swap board (post 6 crunchy alternatives).",
    "details_md": "**The Experiment:** List crunchy swaps (carrots, cukes, jicama, popcorn) on a visible card.\n\n**Why it Works:** Meets sensory need with lower caloric load.\n\n**How to Try It:**\n• Post near snack zone\n• Pick one when ‘crunch’ hits\n• Plate and sit to eat",
    "area": "eating",
    "goals": [
      "reduce_cravings",
      "produce_priority"
    ],
    "helps_with": [
      "stress_eating",
      "mindless_eating"
    ],
    "contraindications": [],
    "mechanisms": [
      "planning_ahead",
      "behavioral_strategy",
      "sensory",
      "convenience"
    ],
    "effort": "minimal",
    "time": "5-15",
    "cost": "$",
    "when": [
      "any",
      "craving_event",
      "snack_time"
    ],
    "where": [
      "home",
      "work"
    ],
    "involves": [
      "carrots",
      "cucumbers",
      "popcorn"
    ],
    "satisfies": [
      "crunchy"
    ],
    "requires": [
      "advance_planning"
    ],
    "features": [
      "veggie_moderate",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "personalization_prompt": "Build your 'Crunch Squad' - pick your top 6 crunchy alternatives:",
    "personalization_type": "choice",
    "personalization_config": {
      "choices": [
        "Baby carrots",
        "Cucumber slices",
        "Jicama sticks",
        "Air-popped popcorn",
        "Snap peas",
        "Bell pepper strips",
        "Apple slices",
        "Celery sticks",
        "Radishes",
        "Rice cakes"
      ],
      "multiple": true
    },
    "difficulty": 1,
    "source": "dietitian_reviewed",
    "sustainability": "daily"
  },
  {
    "tip_id": "9f8a7b6c-1234-4abc-9def-000000000037",
    "micro_version": "Take a deep breath in through your nose over your plate.",
    "summary": "Aroma anchor (3 slow breaths over food).",
    "details_md": "**The Experiment:** Smell your food for 3 breaths before first bite.\n\n**Why it Works:** Olfactory priming enhances satisfaction and slows pace.\n\n**How to Try It:**\n• Lift plate or bowl\n• Inhale, pause, exhale slowly\n• Take a small first bite",
    "area": "eating",
    "goals": [
      "mindful_eating"
    ],
    "helps_with": [
      "mindless_eating"
    ],
    "contraindications": [],
    "mechanisms": [
      "time_ritual",
      "sensory"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any",
      "meal_time"
    ],
    "where": [
      "any"
    ],
    "features": [
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety"
    ],
    "personalization_prompt": "What mantra will you think during your 3 pre-meal breaths?",
    "personalization_type": "text",
    "personalization_config": {
      "placeholder": "e.g., 'I am grateful for this food', 'This nourishes my body', 'I eat with intention'"
    },
    "difficulty": 1,
    "source": "dietitian_reviewed",
    "sustainability": "daily"
  },
  {
    "tip_id": "9f8a7b6c-1234-4abc-9def-000000000038",
    "micro_version": "Set a timer for 2 minutes in the middle of your meal.",
    "summary": "Two‑minute fork‑down timer.",
    "details_md": "**The Experiment:** Every 2 minutes, utensils down until the chime.\n\n**Why it Works:** Built‑in pauses let fullness signals catch up.\n\n**How to Try It:**\n• Set a soft timer\n• Utensils down on each chime\n• Re‑rate hunger mid‑meal",
    "area": "eating",
    "goals": [
      "portion_control",
      "mindful_eating"
    ],
    "helps_with": [
      "portion_control",
      "mindless_eating"
    ],
    "contraindications": [],
    "mechanisms": [
      "habit_stacking",
      "mastery"
    ],
    "effort": "low",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any",
      "meal_time"
    ],
    "where": [
      "home",
      "work"
    ],
    "requires": [
      "timer"
    ],
    "features": [
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety"
    ],
    "difficulty": 2,
    "source": "dietitian_reviewed",
    "sustainability": "daily"
  },
  {
    "tip_id": "9f8a7b6c-1234-4abc-9def-000000000039",
    "micro_version": "Mentally name one flavor (e.g., \"salty\", \"sweet\") while chewing.",
    "summary": "Taste talk in your head.",
    "details_md": "**The Experiment:** Silently name flavors/textures during the meal.\n\n**Why it Works:** Keeps attention on eating vs. scrolling; boosts satisfaction.\n\n**How to Try It:**\n• Name 3 sensations per bite\n• Pause if distracted\n• Re‑rate fullness at 70%",
    "area": "eating",
    "goals": [
      "mindful_eating"
    ],
    "helps_with": [
      "mindless_eating"
    ],
    "contraindications": [],
    "mechanisms": [
      "time_ritual",
      "sensory",
      "mastery"
    ],
    "effort": "low",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any",
      "meal_time"
    ],
    "where": [
      "any"
    ],
    "features": [
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety"
    ],
    "difficulty": 2,
    "source": "dietitian_reviewed",
    "sustainability": "daily"
  },
  {
    "tip_id": "9f8a7b6c-1234-4abc-9def-000000000040",
    "micro_version": "Put your toothpaste on the counter so you see it after dinner.",
    "summary": "Minty full‑stop after planned last bite.",
    "details_md": "**The Experiment:** Brush, floss, or swish mouthwash right after you finish.\n\n**Why it Works:** Mint signals “eating is done” and reduces dessert grazing.\n\n**How to Try It:**\n• Keep kit near table\n• Do it immediately\n• Close kitchen",
    "area": "eating",
    "goals": [
      "evening_grazing",
      "portion_control"
    ],
    "helps_with": [
      "evening_grazing",
      "mindless_eating"
    ],
    "contraindications": [
      "Choose alcohol‑free mouthwash if sensitive; wait 30 minutes before brushing after acidic foods."
    ],
    "mechanisms": [
      "time_ritual",
      "habit_stacking",
      "closure",
      "sensory"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "evening",
      "night",
      "any",
      "post_dinner",
      "craving_event"
    ],
    "where": [
      "home",
      "work"
    ],
    "satisfies": [
      "minty"
    ],
    "requires": [
      "toothbrush"
    ],
    "features": [
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "difficulty": 1,
    "source": "dietitian_reviewed",
    "sustainability": "daily",
    "areas": [
      "nutrition",
      "organization"
    ]
  },
  {
    "tip_id": "9f8a7b6c-1234-4abc-9def-000000000041",
    "micro_version": "Drink one full glass of water right now.",
    "summary": "Big glass first (12–16 oz water).",
    "details_md": "**The Experiment:** Drink a full glass of water before any unplanned snack.\n\n**Why it Works:** Thirst is often misread as hunger; volume supports fullness.\n\n**How to Try It:**\n• Keep a filled bottle nearby\n• Sip steadily, wait 10 minutes\n• Re‑assess urge",
    "area": "eating",
    "goals": [
      "increase_hydration",
      "reduce_cravings"
    ],
    "helps_with": [
      "reduce_cravings",
      "mindless_eating"
    ],
    "contraindications": [
      "Adjust if you are on a fluid‑restricted plan."
    ],
    "mechanisms": [
      "behavioral_strategy",
      "convenience"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any",
      "craving_event",
      "snack_time"
    ],
    "where": [
      "any"
    ],
    "involves": [
      "water"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "difficulty": 1,
    "source": "dietitian_reviewed",
    "sustainability": "daily",
    "areas": [
      "nutrition",
      "organization"
    ]
  },
  {
    "tip_id": "9f8a7b6c-1234-4abc-9def-000000000042",
    "micro_version": "Locate the protein on your plate and eat one bite of it first.",
    "summary": "Protein‑first bite.",
    "details_md": "**The Experiment:** Take your first bite from a protein source.\n\n**Why it Works:** Protein blunts glucose swings that drive cravings.\n\n**How to Try It:**\n• Lead with eggs, yogurt, beans, fish or tofu\n• Then mix the rest\n• Pause halfway to re‑rate hunger",
    "area": "eating",
    "goals": [
      "reduce_cravings",
      "mindful_eating"
    ],
    "helps_with": [
      "portion_control",
      "reduce_cravings"
    ],
    "contraindications": [],
    "mechanisms": [
      "time_ritual",
      "mastery"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any",
      "meal_time"
    ],
    "where": [
      "home",
      "work",
      "restaurant"
    ],
    "involves": [
      "eggs",
      "yogurt",
      "beans",
      "fish",
      "tofu"
    ],
    "features": [
      "veggie_moderate",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety"
    ],
    "difficulty": 1,
    "source": "dietitian_reviewed",
    "sustainability": "daily"
  },
  {
    "tip_id": "9f8a7b6c-1234-4abc-9def-000000000043",
    "micro_version": "Put a handful of carrots on your plate to eat first.",
    "summary": "Veggie starter (cup of crunchy veg/soup first).",
    "details_md": "**The Experiment:** Begin meals with a cup of veg or broth‑based soup.\n\n**Why it Works:** Fiber/volume increase satiety and slow pace.\n\n**How to Try It:**\n• Keep pre‑cut veg ready\n• Plate starter first\n• Eat mindfully before mains",
    "area": "eating",
    "goals": [
      "produce_priority",
      "portion_control",
      "improve_gut_health"
    ],
    "helps_with": [
      "mindless_eating",
      "portion_control"
    ],
    "contraindications": [],
    "mechanisms": [
      "time_ritual",
      "habit_stacking",
      "sensory",
      "comfort"
    ],
    "effort": "minimal",
    "time": "5-15",
    "cost": "$",
    "when": [
      "lunch",
      "evening",
      "any",
      "meal_time"
    ],
    "where": [
      "home",
      "restaurant"
    ],
    "involves": [
      "vegetables",
      "soup"
    ],
    "satisfies": [
      "crunchy",
      "warm"
    ],
    "features": [
      "veggie_heavy",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety"
    ],
    "difficulty": 2,
    "source": "dietitian_reviewed",
    "sustainability": "daily"
  },
  {
    "tip_id": "9f8a7b6c-1234-4abc-9def-000000000044",
    "micro_version": "Sprinkle 1 teaspoon of chia seeds on your next meal.",
    "summary": "Omega‑3 boost today.",
    "details_md": "**The Experiment:** Add one omega‑3 source (salmon, sardines, chia, walnuts).\n\n**Why it Works:** Omega‑3s support mood regulation and may reduce anxiety‑driven urges.\n\n**How to Try It:**\n• Swap in salmon or sprinkle chia\n• Log mood/urge 0–10 after meals\n• Repeat 2–3×/week",
    "area": "eating",
    "goals": [
      "improve_mood",
      "reduce_cravings",
      "improve_gut_health"
    ],
    "helps_with": [
      "stress_eating",
      "anxiety"
    ],
    "contraindications": [
      "Fish/shellfish allergy; consult your clinician if on blood thinners."
    ],
    "mechanisms": [
      "planning_ahead",
      "mastery",
      "energy_boost"
    ],
    "effort": "low",
    "time": "5-15",
    "cost": "$",
    "when": [
      "any",
      "meal_time"
    ],
    "where": [
      "home",
      "restaurant",
      "work"
    ],
    "involves": [
      "salmon",
      "sardines",
      "chia",
      "walnuts"
    ],
    "requires": [
      "advance_planning"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "partner_resistant_ok",
      "diet_trauma_safe",
      "not_diety"
    ],
    "difficulty": 2,
    "source": "dietitian_reviewed",
    "sustainability": "weekly"
  },
  {
    "tip_id": "9f8a7b6c-1234-4abc-9def-000000000045",
    "micro_version": "Eat one spoonful of sauerkraut or yogurt.",
    "summary": "Ferment‑a‑day (yogurt, kefir, kimchi, sauerkraut).",
    "details_md": "**The Experiment:** Include one fermented food today.\n\n**Why it Works:** A healthy microbiome supports mood and cravings via the gut–brain axis.\n\n**How to Try It:**\n• Add plain yogurt to breakfast\n• Top bowls with kimchi\n• Note mood/urge changes",
    "area": "eating",
    "goals": [
      "improve_gut_health",
      "improve_mood"
    ],
    "helps_with": [
      "stress_eating"
    ],
    "contraindications": [
      "Consider histamine intolerance, active reflux, or GI sensitivities."
    ],
    "mechanisms": [
      "behavioral_strategy",
      "convenience",
      "comfort"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any",
      "meal_time"
    ],
    "where": [
      "home",
      "work",
      "restaurant"
    ],
    "involves": [
      "yogurt",
      "kefir",
      "kimchi",
      "sauerkraut"
    ],
    "features": [
      "veggie_moderate",
      "family_friendly",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "difficulty": 1,
    "source": "dietitian_reviewed",
    "sustainability": "daily"
  },
  {
    "tip_id": "9f8a7b6c-1234-4abc-9def-000000000046",
    "micro_version": "Put the kettle on to boil water for tea.",
    "summary": "Warm mug ritual at night (herbal).",
    "details_md": "**The Experiment:** Replace dessert grazing with a warm herbal drink.\n\n**Why it Works:** Warmth soothes the nervous system and provides comfort without a sugar spike.\n\n**How to Try It:**\n• Pick chamomile/peppermint/rooibos\n• Sit while you sip\n• Start 30–60 min before bed",
    "area": "eating",
    "goals": [
      "evening_grazing",
      "improve_sleep"
    ],
    "helps_with": [
      "evening_grazing",
      "stress_eating"
    ],
    "contraindications": [
      "Choose caffeine‑free varieties at night; consider reflux triggers."
    ],
    "mechanisms": [
      "time_ritual",
      "behavioral_strategy",
      "comfort",
      "sensory"
    ],
    "effort": "minimal",
    "time": "5-15",
    "cost": "$",
    "when": [
      "evening",
      "night",
      "post_dinner",
      "craving_event"
    ],
    "where": [
      "home"
    ],
    "involves": [
      "herbal_tea"
    ],
    "satisfies": [
      "warm"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "difficulty": 1,
    "source": "dietitian_reviewed",
    "sustainability": "daily"
  },
  {
    "tip_id": "9f8a7b6c-1234-4abc-9def-000000000047",
    "micro_version": "Set a \"Wind Down\" alarm for 1 hour before bed.",
    "summary": "Sleep guardrails (reverse alarm + dim lights).",
    "details_md": "**The Experiment:** Set an alarm 45 minutes before bed to start wind‑down.\n\n**Why it Works:** Better sleep improves ghrelin/leptin balance and trims next‑day cravings.\n\n**How to Try It:**\n• Dim lights\n• Screens off\n• Light stretch or reading",
    "area": "eating",
    "goals": [
      "improve_sleep",
      "reduce_cravings"
    ],
    "helps_with": [
      "reduce_cravings",
      "evening_grazing"
    ],
    "contraindications": [],
    "mechanisms": [
      "time_ritual",
      "planning_ahead",
      "comfort",
      "mastery"
    ],
    "effort": "low",
    "time": "0-5",
    "cost": "$",
    "when": [
      "night",
      "evening_grazing",
      "tired"
    ],
    "where": [
      "home"
    ],
    "requires": [
      "advance_planning"
    ],
    "features": [
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "diet_trauma_safe",
      "not_diety"
    ],
    "difficulty": 2,
    "source": "dietitian_reviewed",
    "sustainability": "daily",
    "areas": [
      "nutrition",
      "organization"
    ]
  },
  {
    "tip_id": "9f8a7b6c-1234-4abc-9def-000000000048",
    "micro_version": "Buy a bag of pistachios in the shell.",
    "summary": "Shell‑game snack (pistachios in shell or edamame in pod).",
    "details_md": "**The Experiment:** Choose an effortful snack with built‑in pauses.\n\n**Why it Works:** Slows pace and makes amounts visible, aiding satiety.\n\n**How to Try It:**\n• Portion into a small bowl\n• Keep shells/pods visible\n• Pair with water",
    "area": "eating",
    "goals": [
      "portion_control",
      "mindful_eating"
    ],
    "helps_with": [
      "mindless_eating",
      "portion_control"
    ],
    "contraindications": [
      "Allergy to nuts/soy; mind choking risk for small children."
    ],
    "mechanisms": [
      "behavioral_strategy",
      "sensory",
      "convenience"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any",
      "snack_time",
      "evening_grazing"
    ],
    "where": [
      "home",
      "work"
    ],
    "involves": [
      "pistachios",
      "edamame"
    ],
    "satisfies": [
      "salty"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "difficulty": 1,
    "source": "dietitian_reviewed",
    "sustainability": "daily"
  },
  {
    "tip_id": "9f8a7b6c-1234-4abc-9def-000000000049",
    "micro_version": "Pour sparkling water into a wine glass.",
    "summary": "Mocktail hour (sparkling water + citrus) in a fancy glass.",
    "details_md": "**The Experiment:** Make a zero‑alcohol, bubbly drink for evening ritual.\n\n**Why it Works:** Satisfies hand‑to‑mouth and ‘treat’ cues without sugar/fat.\n\n**How to Try It:**\n• Sparkling water + citrus/herbs\n• Sit to sip\n• Pair with kitchen‑closed card",
    "area": "eating",
    "goals": [
      "evening_grazing",
      "reduce_cravings"
    ],
    "helps_with": [
      "evening_grazing",
      "stress_eating"
    ],
    "contraindications": [
      "Citrus may aggravate reflux; use gentler flavors if needed."
    ],
    "mechanisms": [
      "time_ritual",
      "behavioral_strategy",
      "sensory",
      "comfort"
    ],
    "effort": "minimal",
    "time": "5-15",
    "cost": "$",
    "when": [
      "evening",
      "post_dinner",
      "screen_time"
    ],
    "where": [
      "home"
    ],
    "involves": [
      "sparkling_water",
      "citrus"
    ],
    "satisfies": [
      "bubbly"
    ],
    "features": [
      "veggie_free",
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "difficulty": 1,
    "source": "dietitian_reviewed",
    "sustainability": "daily"
  },
  {
    "tip_id": "9f8a7b6c-1234-4abc-9def-000000000050",
    "micro_version": "Stand outside or by a window for 2 minutes.",
    "summary": "Sunshine microdose (3–5 minutes midday).",
    "details_md": "**The Experiment:** Get outside for a few minutes of daylight.\n\n**Why it Works:** Supports circadian rhythm and mood, lowering stress‑driven appetite.\n\n**How to Try It:**\n• Step out after lunch\n• Look at the horizon (not the sun)\n• Take 5 slow breaths",
    "area": "eating",
    "goals": [
      "improve_mood",
      "improve_sleep",
      "stress_eating"
    ],
    "helps_with": [
      "stress_eating",
      "anxiety"
    ],
    "contraindications": [
      "Use sun protection as appropriate."
    ],
    "mechanisms": [
      "behavioral_strategy",
      "energy_boost",
      "relief"
    ],
    "effort": "minimal",
    "time": "0-5",
    "cost": "$",
    "when": [
      "afternoon",
      "any",
      "work_break",
      "stress"
    ],
    "where": [
      "outdoors",
      "work",
      "home"
    ],
    "features": [
      "family_friendly",
      "kid_approved",
      "partner_resistant_ok",
      "no_planning",
      "impulse_friendly",
      "diet_trauma_safe",
      "not_diety",
      "chaos_proof"
    ],
    "difficulty": 1,
    "source": "dietitian_reviewed",
    "sustainability": "daily"
  },
  {
    "tip_id": "eat-slowly-20-minutes",
    "micro_version": "Set a timer for 20 minutes when you start eating.",
    "summary": "Eat slowly: Take 20+ minutes for main meals",
    "details_md": "**The Experiment:** Take at least 20 minutes to eat your main meals today.\n\n**Why it Works:** Eating slowly gives your brain time to receive fullness signals from your stomach, which takes about 20 minutes. This helps prevent overeating and improves digestion. You'll likely feel more satisfied with less food.\n\n**How to Try It:**\n• Set a timer for 20 minutes when you start eating\n• Put your fork down between bites\n• Chew each bite thoroughly (aim for 20-30 chews)\n• Take sips of water throughout the meal\n• Engage in conversation if eating with others",
    "area": "eating",
    "goals": [
      "weight_loss",
      "improve_gut_health",
      "improve_energy"
    ],
    "helps_with": [
      "speed_eating",
      "mindless_eating",
      "portion_control"
    ],
    "contraindications": [],
    "mechanisms": [
      "mindset_shift",
      "time_ritual",
      "mastery",
      "comfort"
    ],
    "effort": "medium",
    "time": "15-30min",
    "cost": "$",
    "when": [
      "morning",
      "afternoon",
      "evening",
      "meal_time"
    ],
    "where": [
      "home",
      "work",
      "restaurant"
    ],
    "preserves": [
      "chocolate",
      "cheese",
      "bread",
      "pasta",
      "meat"
    ],
    "features": [
      "family_friendly",
      "partner_resistant_ok",
      "no_planning"
    ],
    "personalization_prompt": "What's your strategy for timing 20 minutes?",
    "personalization_type": "text",
    "personalization_config": {
      "placeholder": "e.g., Set a timer, put on a 20-minute playlist, watch a sitcom episode"
    },
    "difficulty": 3,
    "source": "dietitian_reviewed",
    "sustainability": "daily"
  },
  {
    "tip_id": "11111111-1111-4111-8111-111111111001",
    "summary": "Do a 10‑minute Clutter Audit + 'Why' Statement.",
    "details_md": "**The Experiment:** Walk through your space for 10 minutes noting stress points. Write a one‑sentence 'Why I want to be organized.'\n\n**Why it Works:** Naming *why* increases motivation and persistence; awareness reduces avoidance and guides effort.\n\n**How to Try It:**\n• Set a 10‑min timer and list 3 clutter hotspots\n• Write your one‑sentence 'why'\n• Post it where you'll see it daily",
    "area": "productivity",
    "goals": [
      "declutter",
      "motivation"
    ],
    "contraindications": [],
    "mechanisms": [
      "mindset_shift",
      "assessment",
      "identity",
      "mastery"
    ],
    "effort": "low",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any",
      "overwhelm"
    ],
    "where": [
      "home",
      "work"
    ],
    "features": [
      "solo_friendly",
      "no_planning",
      "impulse_friendly",
      "chaos_proof"
    ],
    "personalization_prompt": "What 3 reasons make being organized worth it for you?",
    "personalization_type": "multi_text",
    "personalization_config": {
      "items": [
        {
          "label": "Reason 1",
          "placeholder": "e.g., Less morning stress"
        },
        {
          "label": "Reason 2",
          "placeholder": "e.g., Easier to focus"
        },
        {
          "label": "Reason 3",
          "placeholder": "e.g., Ready for guests"
        }
      ]
    },
    "difficulty": 1,
    "source": "coach_curated",
    "areas": [
      "nutrition",
      "organization"
    ]
  },
  {
    "tip_id": "11111111-1111-4111-8111-111111111002",
    "summary": "Create a One‑Page Organizing Vision.",
    "details_md": "**The Experiment:** Draft a short description or mini vision board of your ideal space.\n\n**Why it Works:** A concrete target improves planning and effort; it also guides what to keep vs. let go.\n\n**How to Try It:**\n• Write a 3–5 sentence vision\n• Add 1–2 photos (optional)\n• Pick one area to align with that vision this week",
    "area": "productivity",
    "goals": [
      "declutter",
      "organize_home",
      "organize_workspace"
    ],
    "contraindications": [],
    "mechanisms": [
      "planning_ahead",
      "mindset_shift",
      "identity",
      "mastery"
    ],
    "effort": "low",
    "time": "5-15",
    "cost": "$",
    "when": [
      "any",
      "new_project"
    ],
    "where": [
      "home",
      "work"
    ],
    "features": [
      "solo_friendly",
      "no_planning",
      "impulse_friendly",
      "chaos_proof"
    ],
    "personalization_prompt": "Describe your ideal room/desk and how it should feel.",
    "personalization_type": "text",
    "personalization_config": {
      "placeholder": "e.g., Clear desk, only laptop + notebook; calm, easy to focus"
    },
    "difficulty": 1,
    "source": "coach_curated",
    "areas": [
      "nutrition",
      "organization"
    ]
  },
  {
    "tip_id": "11111111-1111-4111-8111-111111111003",
    "summary": "The 1‑Touch Rule: Don't put it down, put it away.",
    "details_md": "**The Experiment:** For one day, when you pick something up, put it directly in its home—not on the counter or floor.\n\n**Why it Works:** Eliminates double‑handling and prevents pile‑ups; it's a tiny habit with compounding benefits.\n\n**How to Try It:**\n• Start with one room or area\n• Each time you grab something, put it away immediately\n• Notice how much cleaner the space stays",
    "area": "productivity",
    "goals": [
      "declutter"
    ],
    "contraindications": [],
    "mechanisms": [
      "habit_stacking",
      "decision_ease",
      "mastery"
    ],
    "effort": "low",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any"
    ],
    "where": [
      "home",
      "work"
    ],
    "features": [
      "no_planning",
      "impulse_friendly",
      "chaos_proof"
    ],
    "personalization_prompt": "Which room will you practice the 1-Touch Rule in today?",
    "personalization_type": "text",
    "personalization_config": {
      "placeholder": "e.g., Kitchen, bedroom, or office"
    },
    "difficulty": 2,
    "source": "coach_curated"
  },
  {
    "tip_id": "11111111-1111-4111-8111-111111111004",
    "summary": "The 2‑Minute Pick‑Up: Clear one surface completely.",
    "details_md": "**The Experiment:** Set a 2‑minute timer and clear ONE surface entirely—counter, desk, or table.\n\n**Why it Works:** Creates immediate visual calm and proves you can make progress quickly.\n\n**How to Try It:**\n• Pick your most annoying cluttered surface\n• Set timer for 2 minutes\n• Put everything away or in a temporary basket\n• Enjoy the clear space!",
    "area": "productivity",
    "goals": [
      "declutter"
    ],
    "contraindications": [],
    "mechanisms": [
      "environment_design",
      "sensory",
      "mastery"
    ],
    "effort": "low",
    "time": "0-5",
    "cost": "$",
    "when": [
      "any"
    ],
    "where": [
      "home",
      "work"
    ],
    "features": [
      "no_planning",
      "impulse_friendly",
      "chaos_proof"
    ],
    "personalization_prompt": "Which surface drives you crazy when it's cluttered?",
    "personalization_type": "text",
    "personalization_config": {
      "placeholder": "e.g., Kitchen counter, desk, coffee table"
    },
    "difficulty": 1,
    "source": "coach_curated"
  },
  {
    "tip_id": "11111111-1111-4111-8111-111111111005",
    "summary": "Paper Triage: Sort into Action, Archive, or Toss.",
    "details_md": "**The Experiment:** Gather all loose papers and sort into 3 piles: Action (do something), Archive (keep), Toss (recycle).\n\n**Why it Works:** Most paper clutter is delayed decisions; this system forces quick choices.\n\n**How to Try It:**\n• Collect all visible papers\n• Sort quickly - don't read everything\n• Toss/recycle immediately\n• Put Action items in one folder\n• File or scan Archive items",
    "area": "productivity",
    "goals": [
      "paper_clutter",
      "organize_home",
      "organize_workspace"
    ],
    "contraindications": [],
    "mechanisms": [
      "planning_ahead",
      "environment_design",
      "decision_ease",
      "relief"
    ],
    "effort": "medium",
    "time": "15-30min",
    "cost": "$",
    "when": [
      "any"
    ],
    "where": [
      "home",
      "work"
    ],
    "features": [
      "solo_friendly",
      "no_planning"
    ],
    "personalization_prompt": "Where does paper pile up most in your space?",
    "personalization_type": "text",
    "personalization_config": {
      "placeholder": "e.g., Kitchen counter, desk drawer, entryway"
    },
    "difficulty": 3,
    "source": "coach_curated"
  }
];

export const SIMPLIFIED_TIPS: SimplifiedTip[] = [
  ...BASE_SIMPLIFIED_TIPS,
  ...NUTRITION_SIMPLIFIED_TIPS,
  ...FITNESS_SIMPLIFIED_TIPS,
  ...ORGANIZATION_SIMPLIFIED_TIPS,
  ...PRODUCTIVITY_SIMPLIFIED_TIPS,
  ...RELATIONSHIP_SIMPLIFIED_TIPS,
  ...MINDSET_SIMPLIFIED_TIPS,
  ...SLEEP_SIMPLIFIED_TIPS,
];

// Helper function to get a specific tip by ID
export function getTipById(tipId: string): SimplifiedTip | undefined {
  const found = SIMPLIFIED_TIPS.find(tip => tip.tip_id === tipId);
  if (!found) {
    console.log(`⚠️ getTipById: Tip with ID ${tipId} NOT FOUND in database`);
    console.log(`  Available tip IDs (first 5): ${SIMPLIFIED_TIPS.slice(0, 5).map(t => t.tip_id).join(', ')}`);
  }
  return found;
}

// Helper function to get safe tips for a user (no contraindications)
export function getSafeTips(userProfile: any): SimplifiedTip[] {
  const userConditions = userProfile.medical_conditions || [];

  return SIMPLIFIED_TIPS.filter(tip => {
    // If tip has no contraindications, it's safe for everyone
    if (!tip.contraindications || tip.contraindications.length === 0) {
      return true;
    }

    // Check if any of the user's conditions are contraindicated
    return !tip.contraindications.some(contra =>
      userConditions.some((condition: string) =>
        contra.toLowerCase().includes(condition.toLowerCase())
      )
    );
  });
}

export default SIMPLIFIED_TIPS;
